/** Bundle the Graph Memory host and browser halves. */
...
if (process.argv.includes('--check')) {
  for (const output of outputs) {
    ...
    if (current === undefined || digest(current) !== digest(output.contents)) {
      throw new Error(`Graph Memory build is stale: ${output.path}`);
    }
  }
  console.log('Graph Memory host and client builds match their sources.');
} else {
  await mkdir(join(root, 'dist'), { recursive: true });
  for (const output of outputs) await writeFile(output.path, output.contents);
  console.log(`Built ${manifest.name}: graph routes and tools + sidebar panel.`);
}
