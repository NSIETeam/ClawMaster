...
创建工作空间的路由会在建立目录前，使用与业务操作相同的 `GovernanceAccess` 检查当前身份与权限。企业模式要求有效的执行者或管理员角色，并检查其是否获准创建所请求类型的工作空间；身份缺失、成员权限撤销或授权服务不可用时，不会创建目录。本地模式是独立、显式的身份模式，不代表组织成员身份。
...
WatchDog 管理台通过“定时巡检”面板提供产品级持久计划，支持逐次批准、独立观察工作进程状态，以及人工核对不确定的投递。操作流程、认证接口、部署限制和恢复方式见[持久调度](scheduling/README.zh.md)。
