...
The workspace allocation route checks the same current `GovernanceAccess` used by business operations before creating directories. Enterprise requests need an active executor or administrator grant for the requested workspace kind; a missing identity, revoked membership or unavailable authority creates no directory. Local mode remains a separate explicit identity mode and does not establish organization membership.
