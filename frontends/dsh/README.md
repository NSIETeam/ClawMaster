...
Workspace allocation checks the current `GovernanceAccess` identity and scoped `workspace.create` permission before creating directories, then checks permission again before registering the Workspace. Shared desk allocations stay serialized through registration or rollback, so one denied request cannot remove another caller's Workspace. If the second check observes revoked access, registration stops and the newly created empty directory is removed; explicit local identity does not establish organization membership.
...
WatchDog home reports app-service connection, model verification, scheduled-worker observations, business-task attention and host component health separately. Component health comes from `/api/clawmaster/runtime`, which a local Host serves and a shared deployment refuses; it states the loaded component count and names any plugin this Host disabled, without a filesystem path, digest or credential. It is read once when the home loads and again on refresh, so a stale count is never presented as current. Every product surface — the main panel, the settings panels and each sidebar component — renders inside its own error boundary, so a panel that throws is replaced by a readable alert naming the failure, with a retry, instead of leaving its region blank or taking the window with it. A durable `assistant/message` in a loaded Session event window records a successful model response; this does not prove current provider availability; schedule status uses the latest timestamped worker response; business counts cover only the task page currently loaded. Missing schedule observations are shown as unobserved in a neutral color, not as healthy; read failures remain explicit. A line above the collapsed details names the layers that need attention without being expanded, so the single connection indicator cannot be read as a whole-system verdict; when no layer needs attention the line instead states that each layer is observed separately, which is not an all-clear. Management prioritizes durable tasks awaiting review, failed or overdue. Drafts record a goal, scope, owner, deadline, risk and acceptance criteria; an existing Session can be explicitly selected as their source. Starting a ready task saves its selected Session and request identifier, then submits the complete goal, scope, owner, deadline, risk and acceptance criteria to that Session. The browser can record only uncertainty; a Host observer correlates the durable Session request id with its turn and records failure for a claimed request that ends unsuccessfully or is canceled from the inbox. Success requires a matching persisted `user/message`, so an empty pre-step that ends `completed` does not count as execution. Host replay flushes a live Session before reading its durable log, reconstructs inbox requests and cancellation, and restores active turn correlations after an observer reload. A completed DSH turn confirms execution, not business acceptance. If admission is uncertain, the task can retry the same request identifier and exact brief; a terminal replay result prevents another prompt submission. Linking a Session alone sends no model request. In-progress work can record waiting conditions, failures or evidence submissions. Humans enter a comment to accept or reject results, and closed tasks can be reopened. Paged history preserves prior submissions and reviews. Evidence references are marked unverified and require inspection of their sources.
...
WatchDog occupies the main panel. Better Sidebar opens document editing, browsing, CRM and ERP in tabs beside the conversation, and terminals at the bottom. The main panel does not require Better Sidebar; if it is unavailable, WatchDog remains usable and only sidebar tools are unavailable. If Better Sidebar activates later, CRM and ERP tabs register automatically. Manage CRM and ERP with the other components in Settings → Side Cards; each component's feature settings opens its right-side tab. Components are enabled by default but open only on request. Opening an existing component selects its tab.
...
`watchdog_task_query` and `watchdog_task_command` manage durable tasks with an owner, deadline/timezone, risk, scope, acceptance criteria, evidence and linked DSH Sessions. Business states are draft, ready, in progress, awaiting review, accepted, failed and cancelled. Waiting and overdue indicators are independent of Session activity. Agents submit evidence and cannot accept, reopen or cancel tasks. Human rejection returns work to ready while preserving previous evidence and review history. Commands carry a task revision and idempotency key; explicit Session imports create drafts and never infer past success. `/api/clawmaster/tasks` reads a page, an `id`, or its `history=true`; `/api/clawmaster/tasks/command` accepts the shared command envelope. These records are excluded from CRM/ERP restore. The task form can reference a Session already linked to that task; the Host reports it available only while the linked Session exists and is authorized for the task. This checks access, not Session contents, which still require human inspection. Other locations, including external URLs, remain unchecked; the Host never fetches arbitrary evidence URLs. If the computed status would exceed the response-byte budget, the Host omits it and the client shows unchecked. See the [linked Session evidence decision](../../.agents/notes/implemented/feature/2026-09-18-watchdog-linked-session-evidence-status.md) for the authorization and availability rules.

Tasks can also persist StateCapsules in the same SQLite task revisions and immutable history. A capsule records decisions, SHA-256 file hashes, verification results, unfinished actions and memory IDs; its target and current task approval state are derived from the owning task. User capsules are keyed to the authenticated principal, Session capsules require a Session linked to the task, and project capsules require a Workspace with a task-linked Session. Reads and command responses return only the caller's capsules and recheck Session or Workspace membership. Capsule input has no credential fields or credential export path; stored text is not a secret scanner. Legacy tasks load with an empty capsule collection.

Local mode identifies a device operator and uses explicitly labelled local owners. An embedding Host may configure `governance: { mode: 'enterprise', organizationId, authority }` using the trusted `GovernanceAuthority` interface in `src/governance-access.ts`. That authority must authenticate HTTP requests independently of the DSH desktop token, bind agent Sessions to initiating members, read current membership on each operation and consume object/revision/digest-bound approvals. Each database is bound to one organization; an existing local database cannot silently become enterprise data. Every route, tool and schedule runtime checks that its database owner matches the configured organization before registering or running. Schedule consumers check both their ledger and responsibility database. The reserved `local` namespace requires local mode; a deployment mismatch fails without exposing those consumers. HTTP and tool consumers check the same roles and resource grants, including after approval waits. Order submission rechecks every referenced inventory grant after approval and records the latest checked policy version while retaining its approval receipt. Delegated grants cannot exceed the initiating member's grants. New or revised task owners are checked again after independent approval. Every enterprise record mutation, task write and restore requires a different active approver; task result review is performed directly by an authorized human, and a submitting member cannot accept their own result. Approver resource checks accept the same exact, family-wildcard and global grants as executor checks. Local DSH tool approval records only the `dsh-one-shot` outcome; DSH supplies no independent approver identity or approval ID. Committed task retries require current access and exact caller/content matching without consuming another approval. An unavailable authority fails the operation without local fallback.

The enterprise overview contains versions, collection counts and configured page limits; it requires organization-wide record and audit read access. Every HTTP write returns only command receipt metadata, including in local desktop mode. Resource-scoped readers use `/api/clawmaster/enterprise/query` or `enterprise_query` with an authorized record ID; audit reads require their own permission. Authority `resources` grants use canonical keys such as `record/contact/<encoded-id>`, `record/inventory/<encoded-id>`, `record/order/<encoded-id>`, `record/audit/<encoded-id>`, `task/<encoded-id>`, `schedule/<encoded-id>` and `workspace/<encoded-id>`; `<family>/*` grants one family, and `*` grants all organization resources. Order submission also requires write access to each referenced inventory item. Bare-ID grants are rejected because the same ID can name unrelated resource types; update the authority's grant mapping before enabling this version.

The Host validates every non-empty response from `GovernanceAuthority` at the provider boundary. A malformed HTTP or Session principal, membership, owner membership or approval response fails closed as service unavailability and cannot become a local operator or an enterprise denial. A missing response remains an ordinary unauthenticated or unapproved result. This distinction prevents a broken identity provider from granting access while keeping provider details out of HTTP errors and responsibility history. Authority identifiers are opaque Unicode text of 1 to 128 UTF-16 code units; task member owners, submitters, StateCapsule owners and durable responsibility identities use the same rule, so email identities survive validation without changing who the authority authenticated.
...
The authority interface is an integration requirement, not a bundled identity provider or a claim of a validated multi-user deployment. Enterprise identity comes only from the configured server-side `GovernanceAuthority`; request JSON, the local DSH desktop token and client-selected organization/member fields do not establish membership. The registered enterprise HTTP routes, DSH business tools, backup export and managed workspace allocation share the same access evaluator, and foreign or missing identities fail closed in route-level tests. There is no ClawMaster attachment-serving route or enterprise attachment store in this frontend, so `attachments.read` is only a permission vocabulary entry and cannot be presented as an implemented attachment authorization path. Organization login and IdP integration, attachment serving, explicit local-to-enterprise migration and the real desktop acceptance flow require deployment-specific integration and live enterprise verification. External CRM/ERP connectors are separate capabilities.
