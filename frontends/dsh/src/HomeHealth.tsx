...
export function HomeHealth({ locale, state }: { locale: ProductLocale; state: HomeHealthState }) {
  ...
  return <section className="cm-home-health" aria-label={copy.healthHeading}>
    ...
    <dl>
      ...
      <div><dt>{copy.healthSchedule}</dt><dd data-health={state.schedule.error || state.schedule.offline + state.schedule.degraded + state.schedule.failed + state.schedule.uncertain > 0 || (state.schedule.total !== null && state.schedule.online === 0) ? 'attention' : 'observed'}>{scheduleStatus}</dd>
