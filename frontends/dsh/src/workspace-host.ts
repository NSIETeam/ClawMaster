...
/**
 * Register an authenticated allocation route; no Workspace exists until requested.
 ...
 */
export function applyManagedWorkspaces(ctx: WorkspaceHostContext, managedRoot: string): () => Promise<void> {
  ...
  const allocate = async (request: Request): Promise<Response> => {
      ...
      if (closing || request.signal.aborted) return unavailable();
