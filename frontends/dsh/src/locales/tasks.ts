...
const zh = {
  ...
  session: '关联执行会话', selectSession: '选择已有会话', start: '开始并提交任务目标', resumeExecution: '补交任务目标', retryExecution: '用原请求重试提交', link: '关联会话', openSession: '打开执行会话',
  sessionHint: '开始会将目标、范围、负责人、期限、风险和验收项提交到所选会话。若提交结果不确定，可用同一请求标识重试。', executionFailed: '任务已记录为处理中，但会话未确认接收。请用原请求重试提交。',
  submit: '提交结果待验收', evidenceLocation: '证据位置', evidenceSession: '引用已关联会话', noEvidenceSession: '不引用会话', evidenceLocationHint: '可选择此任务已关联的 DSH 会话；其他位置不会由主机自动抓取。', evidenceLocationRequired: '请选择关联会话或填写证据位置。', evidenceSummary: '证据说明', evidenceObserved: '证据观测时间',
  checkedCriteria: '已完成的验收项', evidence: '结果证据', evidenceUnchecked: '未能自动确认该证据的位置；验收前请打开来源核对内容和可用性。外部链接不会由主机抓取。',
  evidenceAvailable: '已确认关联的 DSH 会话可访问；这不代表其中的内容已经核实，验收前仍须检查。',
  evidenceUnavailable: '关联的 DSH 会话不存在、未关联到此任务或当前不可访问；请修复引用后再验收。',
  ...
  nextStep: '下一步', nextDraft: '补全范围与验收标准', nextReady: '选择执行会话并开始', nextProgress: '提交结果证据或更新等待条件', nextWaiting: '处理等待条件后继续', nextReview: '核对证据并作出验收结论', nextAccepted: '查看验收记录；有变化时重新打开', nextFailed: '核对失败原因并修订任务', nextCancelled: '需要继续时重新打开任务',
...
const en: Copy = {
  ...
  session: 'Execution session', selectSession: 'Choose an existing session', start: 'Start and submit task goal', resumeExecution: 'Submit task goal', retryExecution: 'Retry with original request', link: 'Link session', openSession: 'Open execution session',
  sessionHint: 'Starting sends the goal, scope, owner, deadline, risk and acceptance criteria to the selected session. Retry uses the same request identifier if admission is uncertain.', executionFailed: 'The task is recorded as in progress, but the Session did not confirm admission. Retry with the original request.',
  submit: 'Submit results for review', evidenceLocation: 'Evidence location', evidenceSession: 'Reference a linked Session', noEvidenceSession: 'Do not reference a Session', evidenceLocationHint: 'Choose a DSH Session already linked to this task; the Host does not fetch other locations.', evidenceLocationRequired: 'Choose a linked Session or enter an evidence location.', evidenceSummary: 'Evidence summary', evidenceObserved: 'Evidence observed at',
  checkedCriteria: 'Completed acceptance criteria', evidence: 'Result evidence', evidenceUnchecked: 'This evidence location could not be checked automatically. Inspect its content and availability before acceptance. External URLs are never fetched by the Host.',
  evidenceAvailable: 'The linked DSH Session is accessible. Its content has not been verified; inspect it before acceptance.',
  evidenceUnavailable: 'The linked DSH Session is missing, not linked to this task, or currently inaccessible. Fix the reference before acceptance.',
  ...
  nextStep: 'Next step', nextDraft: 'Complete scope and acceptance criteria', nextReady: 'Choose a Session and start', nextProgress: 'Submit result evidence or update the waiting condition', nextWaiting: 'Resolve the waiting condition before continuing', nextReview: 'Check evidence and record an acceptance decision', nextAccepted: 'Review the acceptance record; reopen if circumstances change', nextFailed: 'Review the failure reason and revise the task', nextCancelled: 'Reopen the task if work should continue',
