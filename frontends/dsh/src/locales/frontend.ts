...
const zh = {
  ...
  healthHeading: '业务结果概览', systemDetails: '运行与服务详情', healthAttention: '需要处理：{layers}。展开“运行与服务详情”查看各层状态；这一行不代替对具体记录的核对。', healthObserved: '各层状态分别观测，不合并为一个结论；展开“运行与服务详情”逐个查看。', healthApp: '应用服务', healthModel: '模型', healthSchedule: '定时执行', healthBusiness: '业务结果', healthComponents: '组件健康', renderFailedTitle: '这块面板渲染失败，已就地隔离', renderFailedBody: '错误：{error}。其它面板与会话不受影响；修好后选“重试”即可恢复。', renderRetry: '重试', componentsLoaded: '本机已载入 {count} 个组件，未禁用任何插件', componentsDisabled: '本机有 {count} 个插件被禁用', componentsUnobserved: '尚未取得本机组件状态', componentsRefused: '组件状态只在本地桌面提供，未在此提供', componentsEvidenceLimit: '这来自本机运行时记录，不构成对每个组件功能的验证。', componentsNextStep: '被禁用的插件不会提供其功能；请查看运行时状态确认原因。', modelVerified: '已记录成功的模型响应', modelEvidenceLimit: '这证明会话中曾有成功响应，不代表模型此刻仍可用。', modelUnverified: '尚未在已载入会话中观察到成功响应', modelNextStep: '发送一次实际任务；收到模型响应后会更新此状态。', scheduleUnavailable: '读取失败，请刷新巡检状态', scheduleUnobserved: '尚未取得执行器心跳', scheduleNoWorker: '当前没有在线执行器', scheduleAttention: '有执行器离线或状态异常', scheduleOnline: '执行器在线', scheduleWorkers: '在线执行器', scheduleNeedsAttention: '需关注', scheduleOccurrencesAttention: '定时实例待处理：失败 {failed} 项，投递结果待核实 {uncertain} 项。请检查记录；系统不会自动重放不确定投递。', observedAt: '最近观测', businessUnavailable: '业务任务暂不可读取，请刷新后核对。', businessSummary: '已载入 {total} 项；待审核 {review}，失败 {failed}，逾期 {overdue}', businessNextStep: '下一步：先处理待审核、失败和逾期事项；责任人及截止时间见任务卡片。',
...
const en: ProductCopy = {
  ...
  healthHeading: 'Business outcomes', systemDetails: 'Runtime and service details', healthAttention: 'Needs attention: {layers}. Expand “Runtime and service details” for each layer; this line does not replace checking the records.', healthObserved: 'Each layer is observed separately and is not merged into one verdict; expand “Runtime and service details” to read them.', healthApp: 'App service', healthModel: 'Model', healthSchedule: 'Scheduled execution', healthBusiness: 'Business outcomes', healthComponents: 'Component health', renderFailedTitle: 'This panel failed to render and was isolated in place', renderFailedBody: 'Error: {error}. Other panels and Sessions are unaffected; fix it and choose Retry to recover.', renderRetry: 'Retry', componentsLoaded: '{count} components loaded, no plugin disabled', componentsDisabled: '{count} plugins disabled on this Host', componentsUnobserved: 'No component state observed for this Host', componentsRefused: 'Component state is served only to the local desktop, so it is absent here', componentsEvidenceLimit: 'This records the Host runtime record; it does not verify each component works.', componentsNextStep: 'A disabled plugin does not provide its features; check runtime status for why.', modelVerified: 'Successful model response recorded', modelEvidenceLimit: 'This proves a Session response succeeded before; it does not prove current availability.', modelUnverified: 'No successful response observed in loaded Sessions', modelNextStep: 'Run a real task; this state updates when a model response is recorded.', scheduleUnavailable: 'Could not read; refresh schedule status', scheduleUnobserved: 'No worker heartbeat observed', scheduleNoWorker: 'No worker is currently online', scheduleAttention: 'Some workers are offline or degraded', scheduleOnline: 'Workers online', scheduleWorkers: 'Workers online', scheduleNeedsAttention: 'Need attention', scheduleOccurrencesAttention: 'Scheduled instances need attention: {failed} failed, {uncertain} delivery outcomes uncertain. Inspect records; uncertain deliveries are not replayed automatically.', observedAt: 'Last observed', businessUnavailable: 'Business tasks are unavailable; refresh and review.', businessSummary: '{total} loaded; {review} awaiting review, {failed} failed, {overdue} overdue', businessNextStep: 'Next: handle reviews, failures and overdue items first; task cards show owners and deadlines.',
