...
The owner-local tests exercise real SQLite workers, crash recovery, the production DSH Jobs provider, AgentLoop and JSONL persistence, missing approval, revocation, bounded replay, stale-heartbeat observation and a fixed model-input snapshot. They use temporary state and a synthetic model. They do not validate installed desktop sleep/resume, server service management, actual provider charges, external delivery, multi-host network filesystems or authorized credential migration. The management UI must explicitly connect these interfaces before it can expose plan controls; existing Session-local reminder controls do not create these product plans.
