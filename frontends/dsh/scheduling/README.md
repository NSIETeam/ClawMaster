...
The owner-local tests exercise real SQLite workers, crash recovery, the production DSH Jobs provider, AgentLoop and JSONL persistence, missing approval, revocation, bounded replay, stale-heartbeat observation and a fixed model-input snapshot. A Host scenario closes and reopens the same SQLite ledger, simulates fifteen minutes of sleep, checks latest-only coalescing, and requires fresh human approval before the resumed DSH Jobs and AgentLoop path persists one message. Rendered panel tests use the registered Host and SQLite for approval, cancellation, uncertain resolution and response-loss retry; compiled client expectations include the management entry. Tests use temporary state and a synthetic model. They do not validate installed desktop sleep/resume, server service management, actual provider charges, external delivery, multi-host network filesystems or authorized credential migration. Session-local reminder controls remain separate from these product plans.
