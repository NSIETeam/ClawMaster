...
In WatchDog management, open Scheduled checks and choose Create check plan. Select an existing execution Session, describe the goal and scope, then choose a fixed interval or a future time and timezone. Creating a plan sends no model prompt. Select the saved plan to inspect its occurrences and approve a specific delivery. Stopped plans show no next due time. Stop future checks leaves existing occurrences intact; cancel those separately when needed. History shows localized action labels with raw codes collapsed under Technical details. While Scheduled checks is open and the app is visible and online, the panel refreshes every 30 seconds and when the window regains focus or network connectivity; the Home health view summarizes persisted failed and uncertain instances across all plan pages. Automatic refresh preserves the displayed plan and occurrence pages and loaded history; it waits for active reads and unresolved commands to settle. Every observation includes its read time and can be refreshed manually.
...
The Host dispatches only to an already live root agent belonging to the plan's Session. It never resumes a cold Session, transfers credentials or creates an operating-system service. `desktop` and `server` describe the explicitly configured Host deployment: desktop work stops when the application stops; server work requires an operator-managed, running DSH Host and live bound root. A powered-off or sleeping host executes nothing. The desktop ledger retains plans and occurrence history across application restarts; on the next launch, the selected missed-time policy materializes only its bounded recovery set. This does not run work while the desktop process is stopped. Distinct machines with separate database copies are not coordinated workers; use one authorized server and its local ledger for shared execution.
...
| --- | --- |
| `GET /api/clawmaster/schedules` | Plan page, actual deployment mode, independent worker status, and ledger-wide failed/uncertain occurrence counts. |
...
Observation uses stable row cursors and does not refresh heartbeats, recover leases, prune workers or append audit entries. `attentionSummary` counts all `failed` and `uncertain` instances, independent of the requested page. Worker pages contain at most 100 complete records and share the carrier byte budget with the requested plan or occurrence page. History pages contain no worker observation.
...
The owner-local tests exercise real SQLite workers, crash recovery, the production DSH Jobs provider, AgentLoop and JSONL persistence, missing approval, revocation, bounded replay, stale-heartbeat observation and a fixed model-input snapshot. A Host scenario closes and reopens the same SQLite ledger, simulates fifteen minutes of sleep, checks latest-only coalescing, and requires fresh human approval before the resumed DSH Jobs and AgentLoop path persists one message. Rendered panel tests use the registered Host and SQLite for approval, cancellation, uncertain resolution, response-loss retry, ledger-wide failed/uncertain summaries and refresh after connectivity returns. Tests use temporary state and a synthetic model. They do not validate installed desktop sleep/resume, server service management, external notification delivery, actual provider charges, multi-host network filesystems or authorized credential migration. Session-local reminder controls remain separate from these product plans.
