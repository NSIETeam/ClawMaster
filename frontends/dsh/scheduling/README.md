...
In WatchDog management, open Scheduled checks and choose Create check plan. Select an existing execution Session, describe the goal and scope, then choose a fixed interval or a future time and timezone. Creating a plan sends no model prompt. Select the saved plan to inspect its occurrences and approve a specific delivery. Stopped plans show no next due time. Stop future checks leaves existing occurrences intact; cancel those separately when needed. History shows localized action labels with raw codes collapsed under Technical details. While the app is open, the panel refreshes every 30 seconds and when the window regains focus or network connectivity; the Home health view summarizes persisted failed and uncertain instances across all plan pages. Every observation includes its read time and can be refreshed manually.
