...
在 WatchDog 管理台打开“定时巡检”，选择“建立巡检计划”，选定已有执行会话，填写目标与范围，再设置固定间隔或未来的时间及时区。创建计划不发送模型提示词。选中保存的计划后，可查看各次记录并单独批准投递。已停止的计划不显示下次到期时间。“停止后续巡检”会保留已有实例；需要时应分别取消。历史以当前语言显示操作名称，原始代码折叠在“技术详情”中。应用运行时，面板每 30 秒刷新一次，并在窗口重新获得焦点或网络恢复时刷新；首页健康区汇总所有计划中的持久化失败实例和投递结果待核实实例。每次观察都会记录读取时间，也可手动刷新。
