...
在 WatchDog 管理台打开“定时巡检”，选择“建立巡检计划”，选定已有执行会话，填写目标与范围，再设置固定间隔或未来的时间及时区。创建计划不发送模型提示词。选中保存的计划后，可查看各次记录并单独批准投递。已停止的计划不显示下次到期时间。“停止后续巡检”会保留已有实例；需要时应分别取消。历史以当前语言显示操作名称，原始代码折叠在“技术详情”中。定时巡检已打开且应用可见、联网时，面板每 30 秒刷新一次，并在窗口重新获得焦点或网络恢复时刷新；首页健康区汇总所有计划中的持久化失败实例和投递结果待核实实例。自动刷新保留当前计划页、实例页及已读取历史；正在读取或命令结果待核实时会暂缓刷新。每次观察都会记录读取时间，也可手动刷新。
...
每个实例首先处于 `waiting_approval`。已认证的人可以通过命令接口批准这个具体实例。Agent 可以通过 `watchdog_schedule_command` 请求同一授权；DSH 必须在已开启的对话回合中返回 `allowed-once`。在本地模式下，责任记录将其标记为 `dsh-one-shot`；DSH 不提供独立审批者 ID。审批响应者缺席、审批被禁用、治理服务不可用或授权过期都不会授权执行。批准的有效期截止于 `createdAt + approvalTimeoutMs`，包含等待所属会话上线或重试交付的时间。创建计划、恢复工作进程和重新打开桌面都不会批准实例。
...
| 接口 | 输入与结果 |
| --- | --- |
| `GET /api/clawmaster/schedules` | 计划分页、实际部署模式、独立工作进程状态，以及全账本失败/不确定实例数。 |
...
观察使用稳定的行游标，不刷新心跳、不恢复租约、不清理工作进程，也不追加审计条目。`attentionSummary` 会统计请求分页之外的所有 `failed` 和 `uncertain` 实例。工作进程每页至多包含 100 条完整记录，与请求的计划页或实例页共享传输层字节预算。历史分页不携带工作进程观察信息。
...
包内测试覆盖真实 SQLite 工作进程、崩溃恢复、正式 DSH Jobs 提供者、AgentLoop 与 JSONL 持久化、审批缺席、权限撤销、有界重放、过期心跳观察，以及固定模型输入快照。Host 场景会关闭并重新打开同一个 SQLite 账本，模拟休眠十五分钟，核对只合并最近一次到期任务，并要求重新人工批准后才通过恢复后的 DSH Jobs 和 AgentLoop 路径持久化一条消息。渲染后的面板测试通过注册 Host 和 SQLite 验证批准、取消、不确定记录处理、响应丢失重试、全账本失败/不确定汇总，以及网络恢复后的刷新。测试使用临时状态和合成模型，不验证已安装桌面的休眠唤醒、服务器服务管理、外部通知送达、实际模型计费、多主机网络文件系统或已授权的凭据迁移。会话内提醒控件与这些产品计划保持独立。
