...
在 WatchDog 管理台打开“定时巡检”，选择“建立巡检计划”，选定已有执行会话，填写目标与范围，再设置固定间隔或未来的时间及时区。创建计划不发送模型提示词。选中保存的计划后，可查看各次记录并单独批准投递。“停止后续巡检”会保留已有实例；需要时应分别取消。工作进程统计和心跳分页是界面所示读取时刻的观察结果；刷新才能获取当前状态。
