...
在 WatchDog 管理台打开“定时巡检”，选择“建立巡检计划”，选定已有执行会话，填写目标与范围，再设置固定间隔或未来的时间及时区。创建计划不发送模型提示词。选中保存的计划后，可查看各次记录并单独批准投递。已停止的计划不显示下次到期时间。“停止后续巡检”会保留已有实例；需要时应分别取消。历史以当前语言显示操作名称，原始代码折叠在“技术详情”中。工作进程统计和心跳分页是界面所示读取时刻的观察结果；刷新才能获取当前状态。
...
包内测试覆盖真实 SQLite 工作进程、崩溃恢复、正式 DSH Jobs 提供者、AgentLoop 与 JSONL 持久化、审批缺席、权限撤销、有界重放、过期心跳观察，以及固定模型输入快照。Host 场景会关闭并重新打开同一个 SQLite 账本，模拟休眠十五分钟，核对只合并最近一次到期任务，并要求重新人工批准后才通过恢复后的 DSH Jobs 和 AgentLoop 路径持久化一条消息。渲染后的面板测试通过注册 Host 和 SQLite 验证批准、取消、不确定记录处理与响应丢失重试；编译客户端预期包含管理入口。测试使用临时状态和合成模型，不验证已安装桌面的休眠唤醒、服务器服务管理、实际模型计费、外部交付、多主机网络文件系统或已授权的凭据迁移。会话内提醒控件与这些产品计划保持独立。
