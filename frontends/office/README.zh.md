...
从当前任务的文件树打开 Office 文件。请使用编辑器自带的保存按钮。关闭未保存的标签页、刷新或在原标签切换文件前，会询问是否放弃编辑；取消会保留当前文档。切换 Session、将标签移到其他分栏或退出应用前，请等待显示“已保存”。保存保留原文件名和格式。桌面安装包含此私有组合包。
...
[侧栏适配生成器](scripts/patch-sidebar-editor.mjs) 在记录的基础补丁之后，对唯一且由 SHA-256 固定的源码和产物片段进行转换；上游输入变化时会失败。新增 hook 与对话框标记从已审阅的 TypeScript 片段编译。组合侧栏补丁及溯源记录包含这些输出。
