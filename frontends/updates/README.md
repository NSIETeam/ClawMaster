...
Updater `0.1.2` reads the native v2 channel. Separately installed updaters `0.1.0` and `0.1.1`, and the published access kit `0.1.0`, retain their versions and legacy channel until separately upgraded. The kit's first-install operation cannot replace an existing updater. Its offline `repair` operation can, however, apply or recover staged updater operations written with the historical `0.1.0` and `0.1.1` journal fields after the Host exits; this repairs the pending operation, not the installed updater binary or its native-channel parser. The focused kit test covers historical journal fields through apply, exact load confirmation and rollback. Changing the endpoint alone does not teach an older parser to accept a release without Intel Mac files.
