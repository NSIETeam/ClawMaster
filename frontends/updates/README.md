...
Discovery includes durable operation progress. `staged` means approved and waiting for a supported desktop restart; `awaiting-health` means selected but not confirmed loaded; `completed` includes the observing Host identity. `selected-unverified` records a deliberate disabled selection or a legacy updater that cannot send a health receipt; it is terminal and does not claim the component loaded. Other unconfirmed switches are restored at the next startup, preserving their `rolled-back` record. Candidate verification failures and intervening edits produce `blocked` without replacing the profile. Ask the agent to use `clawmaster_update_rollback` with an operation token to request one approval for a previous verified updater version. This tool is limited to the updater, restores no business database, and refuses first-install removal because no previous updater version exists.

Updater `0.1.0` and `0.1.1` default to the withdrawn legacy native manifest URL; `0.1.2` defaults to the v2 URL. An approved component update can migrate a verified installed `0.1.0` or `0.1.1` updater by changing only its `name` entry in the updater-owned profile row. The migration preserves the user's `config` and `disabled` fields, including custom channel URLs and a disabled choice. It accepts only the known row keys and configuration schema; an unknown field or invalid configuration stops before writing. The error tells the operator to stop ClawMaster, back up the exact profile and updater data, restore only a known-good profile backup, or request an administrator-led offline repair when no backup exists. The published access kit `0.1.0` remains first-install-only, but its offline `repair` operation can apply or recover staged updater operations written with the historical `0.1.0` and `0.1.1` journal fields after the Host exits. This repairs the pending operation and does not migrate the updater binary. Tests cover the historical operation journal and the profile-row migration separately; real installed `0.1.0` and `0.1.1` platform upgrades still require acceptance evidence.
...
Native manifests require Windows x64, Apple Silicon, Linux x64 AppImage and Linux x64 DEB targets. Unknown targets and missing required targets are rejected. When the selected machine has no installer in a valid release, discovery reports that native update as unavailable and preparation stops before approval, downloading or writing files. The component catalog remains usable. Native file URLs must match the manifest directory's `versions/<version>/` subtree and exact target filename; legacy and v2 manifests cannot borrow each other's files.
...
The maintenance tests also kill a separate process after it selects the new updater but before a Host can report its health receipt. A subsequent maintenance pass restores the prior profile and records `rolled-back`. On POSIX, another test imposes a zero-byte process file-size quota while the transition journal is written; the failed atomic write leaves the operation `staged`, the live profile unchanged and no temporary journal behind. These checks exercise process death after selection and a constrained-filesystem write failure, but they do not simulate `ENOSPC` on a full volume or kill a process during filesystem rename. Native installer interruption recovery remains unverified on supported installed platforms.

An unreadable or malformed operation journal is reported as `invalid`. Its bytes remain untouched, automatic component switching pauses, and the Host can start with its existing profile. `/updates` identifies the affected token without exposing profile contents. Repair the record before attempting component switching; invalid records cannot authorize rollback or be reported as successfully loaded.
