...
确认安装后，工具在所选 profile 的 `cordis.patch.yml` 中添加更新器所属配置行。监视该 profile 的运行中 Host 可在不重启桌面的情况下加载该行。返回的 `activation-pending` 不代表加载成功：应先确认该 Host 中可使用 `/updates`，再视为安装完成。工具既不启动另一个 DSH 应用，也不修改已安装桌面编译内置的原生更新端点。
...
更新检查包含持久化操作进度。`staged` 表示已批准并等待受支持桌面重启；`awaiting-health` 表示已选择但尚未确认加载；`completed` 包含观测到的 Host 身份。`selected-unverified` 表示用户有意禁用更新器，或所选旧版更新器不支持健康回执；它是终态，不代表组件已加载。其他未确认切换会在下次启动时恢复，并保留 `rolled-back` 记录。候选校验失败和期间发生的编辑产生 `blocked`，不替换 profile。请 agent 使用 `clawmaster_update_rollback` 和操作令牌，为恢复先前已校验的更新器版本请求一次批准。此工具仅支持更新器，不恢复业务数据库；首次安装没有先前更新器版本，因此拒绝把移除当作版本回退。

更新器 `0.1.0`、`0.1.1` 默认请求已退役的旧原生 manifest 地址；`0.1.2` 默认使用 v2 地址。用户批准组件更新后，可以迁移已验证安装的 `0.1.0` 或 `0.1.1` 更新器：只修改更新器所属 profile 行的 `name` 入口字段。迁移会保留用户的 `config` 和 `disabled` 字段，包括自定义通道地址与停用选择。它只接受已知行字段及当前配置 schema；遇到未知字段或无效配置时，会在写入前停止。错误信息会要求操作人员退出 ClawMaster，备份 profile 和更新器数据；只恢复已知可用的 profile 备份，若没有备份则请求管理员离线修复。已发布的接入包 `0.1.0` 仍仅支持首次安装，但其离线 `repair` 操作可在 Host 退出后应用或恢复 `0.1.0`、`0.1.1` 写入的旧格式暂存操作；它修复待处理操作，不会迁移更新器二进制。测试分别覆盖旧操作日志和 profile 行迁移；真实安装的 `0.1.0`、`0.1.1` 跨平台升级仍须补验收证据。
...
原生清单必须包含 Windows x64、Apple Silicon、Linux x64 AppImage 和 Linux x64 DEB 目标。未知目标或缺失必需目标都会被拒绝。当有效版本未提供所选机器的安装包时，发现操作将该原生更新报告为不可用，准备操作会在审批、下载或写入文件前停止。组件目录仍可使用。原生文件 URL 必须匹配清单所在目录的 `versions/<version>/` 子目录及目标文件名；旧格式与 v2 清单不能引用彼此的文件。
...
维护测试还会在新更新器已被选中、Host 尚未提交健康回执时，强制杀死独立进程。后续维护会恢复旧 profile 并记录 `rolled-back`。POSIX 上的另一项测试会在写入切换日志时施加零字节进程文件大小配额；原子写入失败后，操作仍为 `staged`，当前 profile 不变，也不会留下临时日志。这些检查覆盖了选择更新后进程终止和受限文件系统写入失败，但没有模拟整个磁盘写满导致的 `ENOSPC`，也没有在文件系统重命名过程中杀死进程。支持平台上的原生安装器中断恢复仍未经验证。

无法读取或格式损坏的操作记录会报告为 `invalid`。原始字节保持不变，自动组件切换暂停，Host 仍可使用已有 profile 启动。`/updates` 显示受影响的令牌，不暴露 profile 正文。修复记录后才能继续组件切换；无效记录不能授权回滚，也不会被报告为成功加载。
...
即使已安装依赖目录是符号链接，三个 JavaScript 产物仍保留相对包目录的依赖模块名称。源码、编译器与固定依赖字节相同时，其 SHA-256 摘要不受检出路径影响。`npm test` 在两个不同目录深度实际构建，核对三个摘要并拒绝输出中的物理检出路径；`npm run check` 仍会拒绝被修改的产物。该保证仅覆盖这些 JavaScript bundle，不涵盖原生安装包或依赖版本变化。
