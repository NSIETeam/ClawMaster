...
更新器 `0.1.2` 读取原生 v2 通道。单独安装的更新器 `0.1.0`、`0.1.1` 以及已发布的接入包 `0.1.0` 在另行升级前保留其版本和旧通道。接入包的首次安装操作不能替换已有更新器。不过，Host 退出后，接入包的离线 `repair` 操作可以应用或恢复由 `0.1.0`、`0.1.1` 更新器写入的旧格式暂存操作记录；这只修复待处理操作，不会升级已安装的更新器二进制或其原生通道解析器。定向接入包测试以旧格式记录覆盖应用、精确加载确认及回滚。仅修改端点，不能使旧解析器接受缺少 Intel Mac 文件的版本。
