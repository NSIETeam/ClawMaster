...
function manifest() {
  return { version: '0.2.1', notes: '', pub_date: '2026-09-15T00:00:00Z', platforms: Object.fromEntries(Object.entries(suffixes).map(([target, suffix], index) => [target, { url: `https://updates.test/updates/clawmaster/versions/0.2.1/clawmaster-0.2.1-${suffix}`, signature: encode(signatures[index % signatures.length]) }])) }
}
...
test('native manifests reject missing targets, prereleases and unexpected origins or paths', async () => {
  ...
  for (const change of [
    value => { delete value.platforms['linux-x86_64-deb'] },
