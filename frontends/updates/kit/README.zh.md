...
接入包还提供已暂存更新的离线修复操作。桌面退出 Host 后，在解压目录运行 `update-kit.mjs repair --yes --dsh-home <DSH_HOME>`。工具先认证接入包，再调用有限维护工具，应用已批准的版本选择或恢复中断的切换，并返回持久化的操作状态。它不会下载候选、启动 DSH 或恢复业务数据。没有 `--yes` 时不会修改所选主目录；如果 Host 仍存活，工具会在修改 profile 前停止。
