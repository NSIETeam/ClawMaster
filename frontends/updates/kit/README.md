...
The kit also provides an offline repair operation for a staged updater change. After the desktop has exited its Host, run `update-kit.mjs repair --yes --dsh-home <DSH_HOME>` from the extracted kit. The utility authenticates the kit before invoking the finite maintenance helper, applies the approved selection or restores an interrupted switch, and reports the durable operation state. Kit `0.1.0` accepts the historical staged-operation journal fields written by updaters `0.1.0` and `0.1.1`; a source-level behavior test covers repair, exact load confirmation and rollback. This does not update the installed updater binary or its native-channel parser. The utility never downloads a candidate, launches DSH, or restores business data. Without `--yes`, it refuses to modify the selected home; if the Host is still alive, it stops before changing the profile.
