...
import { inspectKit, installKit, nativeKit } from './kit.ts';
...
try {
  ...
  if (values.help) {
    ...
    if (positionals.length > 1) throw new Error('只支持一个操作：inspect、install 或 native。');
    const action = positionals[0] ?? 'inspect';
    if (!['inspect', 'install', 'native'].includes(action)) throw new Error('未知操作。请使用 inspect、install 或 native。');
    ...
    const result = action === 'inspect' ? await inspectKit(options) : action === 'install' ? await installKit({ ...options, confirmed: values.yes,
