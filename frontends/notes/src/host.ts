  ...
  await ctx.effect(async () => {
    ...
    try {
      ...
      disposers.push(ctx.connection.fetch.register({
        ...
        fetch: handle(async () => {
          const [notes, proposals] = await Promise.all([watcher.recompute(), service.proposals.list()]);
          return { version: createHash('sha256').update(notes).update(JSON.stringify(proposals)).digest('hex') };
        }),
      }));
      disposers.push(ctx.connection.fetch.register({
      ...
      removals.push(ctx.tools.register({
        name: 'notes_query',
        description: `Read the built-in ClawMaster notes vault (${root}). Select one mode: tree lists every note, read returns one note with its revision and link facts, search matches text, backlinks lists notes linking to one note, tags counts tags, proposals lists stored drafts and their diffs. Read-only and never needs approval. Pass the revision from read into a later save so a concurrent edit is detected instead of overwritten.`,
      ...
      removals.push(ctx.tools.register({
        name: 'notes_propose',
        description: `Store a pending draft as local JSON under .clawmaster/proposals in the built-in ClawMaster notes vault (${root}), leaving the source Markdown note unchanged. Pass the complete proposed note body to receive the proposal id, its base revision, and a line diff for review. Storing draft metadata needs no note-write approval. Apply it later with notes_write action "apply-proposal" (approval-gated, and refused when the note moved in the meantime) or drop it with "discard-proposal". Read them back with notes_query mode "proposals".`,
        ...
        execute: (args, exec) => runTool(exec, async () => {
          const parsed = proposeSchema.parse(args);
          return service.propose(parsed.id, parsed.text);
        }),
