...
[实时刷新](src/watcher.ts)发布的笔记库版本号来自文件系统指纹与递归监听的结合。指纹是权威，因此漏掉、被合并或不支持的监听事件只会退化为较慢的刷新，而不是呈现过期视图；无法附加的监听会被记录而不是被吞掉。`revision` 路由按需重算指纹，因此在没有监听器可用的平台上客户端仍能感知外部改动。
