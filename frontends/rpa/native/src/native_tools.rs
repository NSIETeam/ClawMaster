...
pub fn dispatch_from_args(args: &[String]) -> Option<Result<(), String>> {
    ...
    let result = match args.get(1).map(String::as_str) {
        ...
        Some("pdf-merge") => {
            ...
            output.and_then(|output| crate::native_documents::merge_pdfs(&output, &args[3..]))
        }
        Some("pdf-optimize") => match (args.get(2), args.get(3)) {
            (Some(output), Some(input)) => crate::native_documents::optimize_pdf(Path::new(output), Path::new(input)),
            _ => Err("pdf optimize output and input are required".to_string()),
        },
        Some("docx-write") => match (args.get(2), args.get(3)) {
            (Some(output), Some(request)) => crate::native_documents::write_docx(Path::new(output), Path::new(request)),
        ...
        Some("pptx-write") => match (args.get(2), args.get(3), args.get(4), args.get(5)) {
            (Some(output), Some(title), Some(author), Some(content)) => {
                crate::native_documents::write_pptx_content(Path::new(output), title, author, content)
            }
