...
默认 `advisory` 只记录发现并放行；`enforce` 下**强制类**发现会变成审批请求，因此一个从未说明如何验证的计划，需要用户本人点头才能开始动工。满足规则的计划在两种模式下都不受打扰。若某个 profile 使用别的提交工具，用 `planTool` 指定（默认 `exit_plan_mode`）。
...
`npm --prefix frontends/guard test` 先构建再运行测试套件：84 条用例覆盖上表的风险判定、目标展开、`sudo`/`env` 前缀、子 shell 与命令链、引号内文字不得误报、决定映射、`observe` 模式、`allowPaths`/`denyPaths`、workdir 解析，以及挂载本身——包括「拒绝不会抵达管线」和「审查抛错时改为委派而不是弄坏智能体」。结果审查另有专属用例：从代表性事件还原事实、无法识别的载荷、命令只取首行、合成出的审查文本、按会话缓冲、`off` 默认值、笔记库缺失，以及写入失败。方案审查覆盖五条规则、强制与建议两种模式，以及它是否真的接在同一条 `tools/pre-execute` 管线上。
