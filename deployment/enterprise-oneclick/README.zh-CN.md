...
- 默认面向全新服务器。完全相同 build 重跑时只验收、不重启；检测到不同的现有 ClawMaster 安装会拒绝覆盖。
...
- 外层 SHA-256 与包内清单用于发现传输损坏；正式包还必须携带 Ed25519 `.sig`，并使用从独立可信渠道取得的 ClawMaster 发布公钥验签，不能信任签名文件自行提供的公钥。
- 正式写入前会创建 `/opt/clawmaster-enterprise/.installing` 事务标记；断电或 `SIGKILL` 后标记会保留，重跑将 fail closed，避免把半安装状态当成新服务器。

## 一、在旧服务器导出

先把本压缩包上传到旧服务器并解压，然后确认实际数据库位置。当前标准安装位置是 `/var/lib/clawmaster-enterprise/data.db`。

只读预检：

```bash
./export-migration.sh \
  --data-dir /var/lib/clawmaster-enterprise \
  --output /root/clawmaster-enterprise-migration.tar.gz \
  --dry-run
```
...
```text
/root/clawmaster-enterprise-migration.tar.gz
/root/clawmaster-enterprise-migration.tar.gz.sha256
```

导出不会停止或修改旧服务。为了最终切换时不丢写入，建议在短维护窗口内先停止旧服务，再做最后一次导出：

```bash
sudo systemctl stop clawmaster-enterprise
sudo ./export-migration.sh \
  --data-dir /var/lib/clawmaster-enterprise \
  --output /root/clawmaster-enterprise-final.tar.gz
```

如果新服务器没有通过验收，立即重新启动旧服务：

```bash
sudo systemctl start clawmaster-enterprise
```

不要把旧 `/etc/clawmaster-enterprise/enterprise.env` 放进迁移压缩包。短信密钥应通过你自己的安全渠道单独复制到新配置。
...
正式包必须同时带有 `.sig`。先从 ClawMaster 单独维护的可信渠道取得 Ed25519
公钥，再校验发布者身份；签名文件内不会携带或选择公钥：

```bash
node verify-enterprise-package-signature.mjs \
  clawmaster-enterprise-oneclick-v*-*.tar.gz \
  clawmaster-enterprise-oneclick-v*-*.tar.gz.sig \
  /安全路径/clawmaster-enterprise-release-public.pem
```

然后校验传输完整性：

```bash
sha256sum -c clawmaster-enterprise-oneclick-v*-*.tar.gz.sha256
```
...
```bash
tar -xzf clawmaster-enterprise-oneclick-v*-*.tar.gz
cd clawmaster-enterprise-oneclick-v*-*
```
...
这些可选项留空不会阻止报修记录写入，但对应的外部通知通道不会发送。安装器会把它们写入 `/etc/clawmaster-enterprise/enterprise.env`，不会放进迁移包或日志。
...
`CLAWMASTER_ENTERPRISE_ADMIN_TOKEN=auto` 会生成不输出到日志的随机平台令牌。迁移库已有管理员账号时不会重建账号；空库会生成一次性管理员密码，安装结束后只写到 `/root/clawmaster-enterprise-bootstrap-*.txt`。
...
./install.sh \
  ...
  --migration /root/clawmaster-enterprise-final.tar.gz \
  --dry-run
```
...
7. 安装专用 `clawmaster-enterprise` 用户、只读 release 和 0600 运行配置；
8. 启动 systemd 服务；
9. 可选安装/验证/重载 Caddy；
...
```bash
sudo /opt/clawmaster-enterprise/deploy/verify.sh
sudo systemctl status clawmaster-enterprise --no-pager
sudo journalctl -u clawmaster-enterprise -n 100 --no-pager
```
...
5. 用修复后的 ClawMaster 客户端完成一次“邀请链接 → 短信注册 → 进入工作区 → 展开企业组织树”；
...
1. 将 DNS 指回旧服务器；
2. `sudo systemctl start clawmaster-enterprise` 启动旧服务；
3. 保留新服务器 `/var/tmp/clawmaster-enterprise-deploy-*` 失败目录供排查。
...
```bash
sudo cat /opt/clawmaster-enterprise/.installing
sudo systemctl status clawmaster-enterprise --no-pager
sudo readlink -f /opt/clawmaster-enterprise/current
sudo ls -la /opt/clawmaster-enterprise /var/lib/clawmaster-enterprise /etc/clawmaster-enterprise
sudo ls -ld /var/tmp/clawmaster-enterprise-deploy-*
```

若服务已启用，先停止它；保留 `/var/tmp/clawmaster-enterprise-deploy-*` 和数据库副本。确认这是未接收任何业务写入的新服务器后，按事务目录中的失败文件恢复或清理，再移除标记。对状态没有把握时不要覆盖安装，直接把上述输出交给维护者。
...
```bash
sudo /opt/clawmaster-enterprise/deploy/backup-now.sh
```
...
```bash
sudo /opt/clawmaster-enterprise/deploy/restore-backup.sh \
  /var/lib/clawmaster-enterprise/backups/clawmaster-enterprise-*.clawmaster-backup
```

`CLAWMASTER_BACKUP_ENCRYPTION_KEY` 必须由客户和交付方按合同约定离线托管；只剩备份文件但
丢失该密钥时无法解密。需要异地副本时，将 NFS、对象存储网关或备份卷挂载到
`/var/backups/clawmaster-enterprise`，再设置
`CLAWMASTER_BACKUP_REPLICA_DIR=/var/backups/clawmaster-enterprise`。异地副本写入后会重新计算
SHA-256，上传或复制失败不会阻断 ClawMaster 业务，但会进入健康状态告警。
...
`clawmaster-enterprise` 服务账号必须可读；恢复时外部密钥与备份不一致会 fail closed，安装器
...
`/var/lib/clawmaster-enterprise/attachments`。大客户可把该目录映射到持久卷、MinIO/S3 网关
...
也可以填写 ClawMaster Control `GET /v1/signing-keyring` 返回的完整 JSON；客户端会接受
...
在线商业部署不再向旧的 `/v1/billing/usage/consume` 发送无签名用量。ClawMaster Server 会为
...
查看 health 中 `sms.configured`。若为 `false`，说明短信四项没有进入 `/etc/clawmaster-enterprise/enterprise.env`。不要用 `CLAWMASTER_ALLOW_SMS_DISABLED=1` 绕过正式迁移验收。
