...
usage() {
  cat <<'EOF'
用法：
  ./export-migration.sh [--data-dir /var/lib/clawmaster-enterprise] \
    --output /安全目录/clawmaster-enterprise-migration.tar.gz [--dry-run]
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --data-dir)
      [ "$#" -ge 2 ] || clawmaster_die "--data-dir 缺少值"
      DATA_DIR="$2"
      shift 2
      ;;
    --output)
      [ "$#" -ge 2 ] || clawmaster_die "--output 缺少值"
      OUTPUT="$2"
      shift 2
      ;;
    --dry-run)
...
[ -n "$OUTPUT" ] || clawmaster_die "必须明确提供 --output；空目标绝不回退到默认目录"
case "$OUTPUT" in
  /*) ;;
  *) clawmaster_die "--output 必须是绝对路径" ;;
esac
DB_PATH="${DATA_DIR%/}/data.db"
[ -f "$DB_PATH" ] || clawmaster_die "找不到数据库：${DB_PATH}" 3
[ ! -L "$DB_PATH" ] || clawmaster_die "数据库不能是符号链接：${DB_PATH}" 3
[ ! -e "$OUTPUT" ] || clawmaster_die "输出已存在，拒绝覆盖：${OUTPUT}"
[ ! -e "${OUTPUT}.sha256" ] || clawmaster_die "校验文件已存在，拒绝覆盖：${OUTPUT}.sha256"

clawmaster_log "导出计划"
printf '  数据库：%s\n  输出：%s\n  模式：SQLite Online Backup\n' "$DB_PATH" "$OUTPUT"
if [ "$DRY_RUN" -eq 1 ]; then
  clawmaster_log "dry-run 完成：没有写入文件、没有停止服务"
  exit 0
fi
...
PREFERRED_NODE="/opt/clawmaster-enterprise/runtime/current/bin/node"
if ! NODE_PATH="$(clawmaster_resolve_node "$PREFERRED_NODE")"; then
  NODE_PATH="$(clawmaster_install_node_runtime "${TEMP_DIR}/runtime")"
fi
...
clawmaster_log "创建在线一致性快照（服务无需停止）"
...
[ "$SOURCE_SCHEMA" -ge 2 ] && [ "$SOURCE_SCHEMA" -le "$RELEASE_SCHEMA_TO" ] \
  || clawmaster_die "本迁入包只支持导出 schema 2 至 ${RELEASE_SCHEMA_TO}，检测到 schema ${SOURCE_SCHEMA}；请先走受控升级" 5
...
await writeFile(output, JSON.stringify({
  format: 'clawmaster-enterprise-migration-v1',
...
chmod 600 "$OUTPUT"
ARCHIVE_SHA="$(clawmaster_sha256 "$OUTPUT")"
