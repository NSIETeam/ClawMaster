...
describe('ProductWorkspaceStore', () => {
  ...
  it('总公司签发引入子公司链接，子公司输入后持久化父子关系且不改变自身 CEO 身份', () => {
    ...
    const childRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-child-workspace-'))
    try {
  ...
  it('父子公司接入只允许 CEO，并校验用途、签名、过期时间和目标企业', () => {
    ...
    const issuerRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-issuer-workspace-'))
    const receiverRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-receiver-workspace-'))
    const wrongRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-wrong-workspace-'))
    try {
