 ...
 * `clawmaster:*` 仍是未上线的托管模型占位符，只能回退到当前个人模型。
 */
...
const defaultRuntimeFactory: RuntimeFactory = async (
  ...
  const config = createCoreConfig({
    sessionId,
    // 内部测试阶段一律 BYOK。旧企业会话可能持有 clawmaster:*，交给 coreConfig
...
export class ClawMasterServer {
  ...
  /**
   * MCP 服务器摘要：配置来自 ~/.clawmaster-user/settings.json 的 mcpServers；
  ...
  /** 追加一条记忆事实：写入项目级 CLAWMASTER.md（对齐 save_memory 工具的落点），成功后回推最新快照。 */
...
(() => {
  ...
  window.clawmaster = {
    ...
    saveTextFile(name, content) {
      ...
      a.href = url;
      a.download = name || 'clawmaster-export.txt';
      a.click();
    ...
    themeGet: () => Promise.resolve(localStorage.getItem('clawmaster-theme') || 'system'),
    themeSet: (value) => {
      localStorage.setItem('clawmaster-theme', value);
    ...
    appVersion: () => Promise.resolve('0.1.5-rc.2'),
    updateCheck: () => Promise.resolve({ status: 'up-to-date', currentVersion: '0.1.5-rc.2', latestVersion: null }),
