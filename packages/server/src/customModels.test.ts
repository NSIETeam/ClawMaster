 ...
 * BYO-key 自定义模型只读加载单测。
 *
 * 全程用临时 HOME 隔离（spy os.homedir），绝不碰真实 ~/.clawmaster-user。
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-custommodels-'))
  // os.homedir() 读 HOME（POSIX）/ USERPROFILE（Win）。ESM 下命名空间不可 spy，
  // 故用 stubEnv 隔离到临时目录，绝不碰真实 ~/.clawmaster-user。
...
describe('customModelsFilePath', () => {
  it('指向临时 HOME 下的 .clawmaster-user/custom-models.json', () => {
    expect(customModelsFilePath()).toBe(
      path.join(tmpHome, '.clawmaster-user', 'custom-models.json'),
    )
  })
})
...
describe('saveCustomModel Codex OAuth', () => {
  it('OAuth 哨兵不是密钥，保存时原样保留而不包装成 secret 文件引用', () => {
    ...
    expect(fs.existsSync(path.join(tmpHome, '.clawmaster-user', 'secrets'))).toBe(false)
  })
