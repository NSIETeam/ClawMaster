...
describe('会话运行时模型边界', () => {
  it('内部测试企业会话继续使用成员自己的 BYOK，只有未上线的 clawmaster:* 才回退', () => {
    ...
    expect(resolveSessionRuntimeModel('enterprise', 'clawmaster:managed')).toBeUndefined();
  });
});
...
describe('ClawMasterServer WS（v1.7 产品工作区）', () => {
  ...
  it('未绑定个人 API 时明确报错，不创建空白 assistant 或回退 mock', async () => {
    ...
    client.send({
      type: 'create_session',
      payload: { title: '企业工作台', agentProfileId: 'clawmaster-enterprise-work' },
    });
    const created = await client.waitFor(
      (f) => f.type === 'session_upsert' && f.payload.session.agentProfileId === 'clawmaster-enterprise-work',
    );
  ...
  it('自动 Skill 候选可读取并仅在明确确认后写入用户 Skill 目录', async () => {
    ...
    const projectSkillDir = path.join(workspace, '.clawmaster', 'skills', 'project-only');
  ...
  it('Agent profile 只传 id，并把固定 9 个工作 Agent 全部隔离到企业身份', async () => {
    ...
    client.send({
      type: 'create_session',
      payload: { title: '个人 ClawMaster', agentProfileId: 'clawmaster-personal' },
    });
  ...
  it('create_session 的基础 Agent 与个人、管理者、成员身份白名单一致', async () => {
    ...
    client.send({
      type: 'create_session',
      payload: { title: '个人首卡', agentProfileId: 'clawmaster-personal' },
    });
    const personal = await client.waitFor(
      (frame) => frame.type === 'session_upsert'
        && frame.payload.session.agentProfileId === 'clawmaster-personal',
    );
    ...
    client.send({
      type: 'create_session',
      payload: { title: '错误企业首卡', agentProfileId: 'clawmaster-enterprise-work' },
    });
    const personalDenied = await client.waitFor(
    ...
    client.send({
      type: 'create_session',
      payload: { title: '管理者首卡', agentProfileId: 'clawmaster-enterprise-work' },
    });
    const ceo = await client.waitFor(
      (frame) => frame.type === 'session_upsert'
        && frame.payload.session.agentProfileId === 'clawmaster-enterprise-work',
    );
    if (ceo.type !== 'session_upsert') throw new Error('unreachable');
    ...
    client.send({
      type: 'create_session',
      payload: { title: '成员首卡', agentProfileId: 'clawmaster-enterprise-work' },
    });
    ...
    client.send({
      type: 'create_session',
      payload: { title: '错误管理者首卡', agentProfileId: 'clawmaster-enterprise-ceo' },
    });
  ...
  it('中心认证成员不能通过 configure_enterprise 自升 CEO，admin/member 只能创建各自 profile', async () => {
    ...
    client.send({
      type: 'create_session',
      payload: { title: '成员工作台', agentProfileId: 'clawmaster-enterprise-work' },
    });
    ...
    client.send({
      type: 'create_session',
      payload: { title: '伪造 CEO 工作台', agentProfileId: 'clawmaster-enterprise-ceo' },
    });
    const memberDenied = await client.waitFor(
      (frame) =>
        frame.type === 'error' &&
    ...
    client.send({
      type: 'create_session',
      payload: { title: '管理员工作台', agentProfileId: 'clawmaster-enterprise-work' },
    });
  ...
  it('中心身份角色变化会取消旧 runtime，但统一企业工作会话仍可继续', async () => {
    ...
    client.send({
      type: 'create_session',
      payload: { title: '企业工作会话', agentProfileId: 'clawmaster-enterprise-work' },
    });
    const created = await client.waitFor(
      (frame) =>
        frame.type === 'session_upsert' &&
  ...
  it('中心会话绑定账号和组织；legacy/错租户会话不会被列出且所有 sessionId 操作统一拒绝', async () => {
    ...
    client.send({
      type: 'create_session',
      payload: { title: '当前成员会话', agentProfileId: 'clawmaster-enterprise-work' },
    });
    const created = await client.waitFor(
      (frame) =>
        frame.type === 'session_upsert' &&
  ...
  it('loopback 控制路由可向桌面 WS 推送增量更新检查通知', async () => {
    ...
    const missing = await fetch(`${baseUrl}/internal/incremental-update/push`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ manifestUrl: 'https://updates.example.com/clawmaster/incremental.json' }),
    });
    expect(missing.status).toBe(401);
    ...
    const invalid = await fetch(`${baseUrl}/internal/incremental-update/push`, {
      ...
      body: JSON.stringify({ manifestUrl: 'http://updates.example.com/clawmaster/incremental.json' }),
    });
    expect(invalid.status).toBe(400);
    ...
    expect(frame.payload.manifestUrl).toBe('https://updates.example.com/clawmaster/incremental.json');
...
describe('ClawMasterServer runtimeFactory（非 mock 路径）', () => {
  ...
  beforeEach(() => {
    ...
    const dir = path.join(tmpHome, '.clawmaster-user');
  ...
  it('设置面板的项目记忆按当前会话工作目录读取', async () => {
    ...
    fs.writeFileSync(path.join(workspace, 'CLAWMASTER.md'), '# workspace memory', 'utf8');
    fs.writeFileSync(path.join(tmpHome, 'CLAWMASTER.md'), '# legacy default memory', 'utf8');
    const store = new InMemorySessionStore({ defaultWorkspacePath: os.homedir() });
    ...
    if (snapshot.type === 'memory_snapshot') {
      expect(snapshot.payload.files[0]).toMatchObject({
        path: path.join(workspace, 'CLAWMASTER.md'),
    ...
    if (legacySnapshot.type === 'memory_snapshot') {
      expect(legacySnapshot.payload.files[0]).toMatchObject({
        path: path.join(tmpHome, 'CLAWMASTER.md'),
  ...
  it('A2A tool-free runtime 不注入企业组织与职位上下文', async () => {
    ...
    client.send({
      ...
      payload: {
        title: 'A2A',
        agentProfileId: 'clawmaster-enterprise-a2a',
...
describe('ClawMasterServer set_model 真实生效语义', () => {
  ...
  it('偏好落盘失败时仍确认已经真实切换的 runtime 与会话模型', async () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.clawmaster-user', 'custom-models.json.tmp'));
...
describe('ClawMasterServer 斜杠命令帧（P3）', () => {
  ...
  it('run_slash_command submit_prompt 形态（/init）在会话正忙时 → ok:false 拒绝，无矛盾双帧', async () => {
    // /init 依赖 cwd 是否存在 CLAWMASTER.md 决定 message/submit_prompt 分叉；
    // mock 到干净临时目录，确保走 submit_prompt 路径（不受仓库现状影响）。
    const tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-init-busy-'));
