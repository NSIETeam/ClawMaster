...
export const ENTERPRISE_POSTGRES_MIGRATIONS: readonly PostgresMigration[] = [
  {
    ...
    sql: `
CREATE TABLE clawmaster_sqlite_import_runs (
  id TEXT PRIMARY KEY,
...
CREATE UNIQUE INDEX clawmaster_sqlite_import_runs_verified_source
  ON clawmaster_sqlite_import_runs (source_sha256)
...
CREATE INDEX attachment_object_access_lookup
  ...
  },
  {
    ...
    sql: `
CREATE TABLE clawmaster_sqlite_import_tables (
  run_id TEXT NOT NULL REFERENCES clawmaster_sqlite_import_runs(id) ON DELETE CASCADE,
...
CREATE TABLE clawmaster_sqlite_import_rows (
  ...
  FOREIGN KEY (run_id, table_name)
    REFERENCES clawmaster_sqlite_import_tables(run_id, table_name) ON DELETE CASCADE
);
...
VALUES ('org_default', 'ClawMaster', 'clawmaster-default', 'enterprise', 'active')
...
CREATE TABLE clawmaster_sqlite_import_promotions (
  run_id TEXT PRIMARY KEY REFERENCES clawmaster_sqlite_import_runs(id) ON DELETE RESTRICT,
...
CREATE INDEX direct_message_attachment_objects_message
  ...
  },
  {
    ...
    sql: `
CREATE TABLE clawmaster_sqlite_import_attachment_objects (
  run_id TEXT NOT NULL REFERENCES clawmaster_sqlite_import_runs(id) ON DELETE CASCADE,
  attachment_id TEXT NOT NULL,
  message_id TEXT NOT NULL,
  organization_id TEXT NOT NULL,
...
CREATE UNIQUE INDEX clawmaster_sqlite_import_attachment_objects_s3_key
  ON clawmaster_sqlite_import_attachment_objects (s3_storage_key)
