...
function dryRunPool(
  ...
  const client: PostgresClientLike = {
    query: vi.fn(async (sql: string, values: readonly unknown[] = []) => {
      statements.push(sql)
      if (sql.includes('FROM clawmaster_sqlite_import_promotions')) return result()
      if (sql.includes('FROM clawmaster_sqlite_import_runs')) {
      ...
      if (sql.includes('FROM clawmaster_sqlite_import_tables')) {
      ...
      if (sql.includes('FROM clawmaster_sqlite_import_rows')) {
        const tableName = String(values[1])
          ...
          return result([
            {
              row_data: [
                ...
                'ab/cd/source.clawmaster-object',
      ...
      if (sql.includes('FROM clawmaster_sqlite_import_attachment_objects')) {
        return result(
          input.withAttachments
            ...
            ? [
              {
                ...
                source_storage_key: 'ab/cd/source.clawmaster-object',
                s3_storage_key:
      ...
      if (sql.includes('INSERT INTO clawmaster_sqlite_import_promotions')) {
        return result([
          {
            run_id: 'import_1',
            promoted_counts: {
...
describe('verified SQLite PostgreSQL promotion', () => {
  ...
  it('rolls back every MLS row when a staged event is invalid', async () => {
    ...
    expect(
      statements.some(sql =>
        sql.includes('INSERT INTO clawmaster_sqlite_import_promotions'),
      ),
    ).toBe(false)
