...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .status-row{display:flex;align-items:center;gap:14px;padding:18px 22px;border-radius:17px;background:#f7f5ef;margin-bclawmasterm:20px}
    .status-icon{width:42px;height:42px;border-radius:12px;background:#e8e4d8;display:grid;place-items:center;font-size:20px}
    .status-text{flex:1}
    .status-label{font-size:13px;font-weight:700;color:#6b7975;margin-bclawmasterm:4px}
...
<body>
          ...
          <div id="state-found" class="hidden">
            <div class="status-row">
              ...
              <div class="status-text">
                ...
                <div class="status-value">版本 <span id="clawmaster-version">—</span> · 实例 <span id="clawmaster-id">—</span></div>
              </div>
            </div>
  ...
  <!-- Discovery SDK -->
  <script src="/enterprise/sdk/clawmaster-discovery.js"></script>
  <script>
    ...
    function handleResult(result) {
      if (result.found) {
        ...
        document.getElementById('clawmaster-version').textContent = result.version || '—';
        document.getElementById('clawmaster-id').textContent = (result.instanceId || '').slice(0, 12) + '…';
      } else {
