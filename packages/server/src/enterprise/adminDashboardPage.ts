...
.header{display:flex;justify-content:space-between;align-items:baseline;margin-bclawmasterm:6px}
.header h1{font-size:24px;color:#60a5fa;letter-spacing:.5px}
.header span{color:#64748b;font-size:13px}
.note{color:#64748b;font-size:12px;margin-bclawmasterm:20px}
.note b{color:#fb923c}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin-bclawmasterm:26px}
...
.section{margin-bclawmasterm:26px}
.section h2{font-size:16px;color:#94a3b8;margin-bclawmasterm:11px}
.empty{color:#475569;font-size:13px;padding:14px;text-align:center}
.charts{display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:14px;margin-bclawmasterm:26px}
.chart-card{background:#1e293b;border:1px solid #334155;border-radius:12px;padding:16px 18px}
.chart-card h3{font-size:14px;color:#94a3b8;margin-bclawmasterm:12px;font-weight:600}
...
.bn .k{color:#94a3b8;font-size:12px;margin-bclawmasterm:6px}
...
<div class="note" id="discloseNote">数据全部存在本机 <b>~/.clawmaster-enterprise/data.db</b>，零云端。标 <b>估算</b> 的指标基于假设，非实测。</div>
...
<script>
const KEY='clawmaster.enterprise.admin.session';
...
async function load(){
  ...
  try{
    ...
    document.getElementById('discloseNote').innerHTML='数据全部存在本机 <b>~/.clawmaster-enterprise/data.db</b>，零云端。标 '+est+' 的指标基于假设（纯人工耗时按 <b>'+a.manualTimeMultiplier+'×</b> ClawMaster 折算、人力 <b>¥'+a.cnyPerHour+'/时</b>），非实测。省时 = ClawMaster 耗时 ×（倍率−1），只算净节省、不双算。';
    const cards=[
