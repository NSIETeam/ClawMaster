...
function poolFor(input: {
  ...
  const client: PostgresClientLike = {
    query: vi.fn(async (sql: string, values: readonly unknown[] = []) => {
      ...
      if (
        sql.includes('FROM clawmaster_sqlite_import_attachment_objects') &&
        sql.includes('SELECT')
      ) {
        return result(input.preparedRows ?? [])
      }
      if (sql.includes('INSERT INTO clawmaster_sqlite_import_attachment_objects')) {
        writes.push(values)
      }
      return result()
    }),
...
describe('verified SQLite attachment import', () => {
  ...
  it('redacts database URLs and both legacy and S3 object keys from CLI errors', () => {
    ...
    const message = safeAttachmentImportErrorMessage(
      new Error(
        `read D:\\legacy\\${legacyKey}: ${s3Key} at postgresql://clawmaster:secret@db.internal/clawmaster`,
      ),
    )
