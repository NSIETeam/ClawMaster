...
// 因此 connect() 前必须先按 appId 拿到 ~/.clawmaster-user/ 下的锁文件（O_EXCL 原子
...
export class FeishuGateway {
  ...
  /**
   * 将卡片原始 content JSON 追加落盘到 `~/.clawmaster-user/feishu-card-dumps.jsonl`，
   ...
   * 需要应用开通 `im:chat:read`（或 `im:chat`）权限——此权限属于 clawmaster
