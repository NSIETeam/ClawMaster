 ...
 *   `CLAWMASTER_DEBUG_FEISHU` is truthy. This keeps Ink's stdout clean by
...
const DEBUG_ENABLED = (() => {
  ...
  const v =
    process.env['CLAWMASTER_DEBUG_FEISHU'] ??
    process.env['CLAWMASTER_DEBUG_FEISHU'] ??
    process.env['CLAWMASTER_DEBUG_FEISHU'];
  if (v !== undefined) {
  ...
  return false; // 🚀 默认关闭所有原始 stderr 日志打印，防止破坏日志输出。需要调试时可设置 CLAWMASTER_DEBUG_FEISHU=1 开启。
})();
