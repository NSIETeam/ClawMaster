...
describe('服务端 Agent profile 白名单', () => {
  it('保留一个历史 CEO profile，但固定 9 个工作 Agent 全部属于企业版', () => {
    ...
    expect(resolveAgentProfile('clawmaster-personal')).toMatchObject({ edition: 'personal' })
    expect(resolveAgentProfile('clawmaster-enterprise-ceo')).toMatchObject({
      name: 'CEO Agent',
      edition: 'enterprise',
      roles: ['company_owner', 'company_admin'],
      legacyOnly: true,
    })
    expect(resolveAgentProfile('clawmaster-enterprise-work')).toMatchObject({
      edition: 'enterprise',
  ...
  it('个人只允许 ClawMaster，四种企业角色都允许固定 9 个企业工作 Agent', () => {
    expect(allowedProfileIds('personal', 'member')).toEqual(['clawmaster-personal'])
  ...
  it('A2A 使用不进入 9-Agent 目录的内部 tool-free profile', () => {
    expect(BUILTIN_AGENT_PROFILES.map(profile => profile.id))
      .not.toContain('clawmaster-enterprise-a2a')
    expect(resolveAgentProfile('clawmaster-enterprise-a2a')).toMatchObject({
      edition: 'enterprise',
      toolFree: true,
      roles: ['company_owner', 'company_admin', 'manager', 'member'],
    })
  ...
  it('三个基础身份使用独立白名单项并锁定 edition 与角色边界', () => {
    ...
    const ceo = resolveAgentProfile('clawmaster-enterprise-ceo')
    const work = resolveAgentProfile('clawmaster-enterprise-work')

    expect(personal).toMatchObject({
      id: 'clawmaster-personal',
      name: 'ClawMaster',
      scope: 'base',
      edition: 'personal',
    })
  ...
  it('基础 ClawMaster 与 PDF/Excel 专家也会用选项式问题引导办公文档任务', () => {
    ...
    const workPrompt = resolveAgentProfile('clawmaster-enterprise-work')?.systemPrompt ?? ''
