...
export interface EscalationFacadeOptions {
  ...
  senders?: Partial<Record<'clawmaster' | 'feishu' | 'sms', EscalationChannelSender>>;
...
export class TicketEscalationFacade {
  ...
  private readonly senders: Partial<Record<'clawmaster' | 'feishu' | 'sms', EscalationChannelSender>>;
  ...
  private async deliverImmediate(job: EscalationJob): Promise<void> {
    const results = await Promise.allSettled([
      this.tryChannel(job, 'clawmaster', job.recipientAccountId),
