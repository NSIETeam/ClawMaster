...
describe('企业公开引入链接', () => {
  it('使用显式 HTTPS 公网基址并保留部署路径前缀', () => {
    const baseUrl = resolveEnterprisePublicBaseUrl({
      configuredUrl: 'https://join.clawmaster.example/clawmaster/',
      host: '0.0.0.0',
      port: 7777,
    })
  ...
  it('拒绝非 HTTP(S)、凭据、query 与 fragment，避免生成可伪装链接', () => {
    for (const configuredUrl of [
      'javascript:alert(1)',
      'ftp://join.clawmaster.example',
      'https://user:pass@join.clawmaster.example',
      'https://join.clawmaster.example?next=https://evil.example',
      'https://join.clawmaster.example/#evil',
    ]) {
