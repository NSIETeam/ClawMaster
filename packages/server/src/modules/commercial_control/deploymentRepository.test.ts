...
function setup() {
  ...
  const control = createCommercialControlComposition({
    ...
    telemetryEndpoint: () => 'https://telemetry.clawmaster.example/v1/events',
  ...
  it('queues content-free module usage and retries it with License-bound billing credentials', async () => {
    ...
    try {
      ...
      const licensePayload = {
        ...
        modules: ['enterprise_tree'],
        offline: false,
        leaseEndpoint: 'https://control.clawmaster.example/v1/licenses/lic-billing/lease',
        billingEndpoint: 'https://control.clawmaster.example/v1/billing/usage/consume',
      ...
      const fetchImpl = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
        ...
        expect(String(url)).toBe(
          'https://control.clawmaster.example/v1/billing/execution-receipts',
        )
  ...
  it('locks an online license until a valid short lease is installed', () => {
    ...
    try {
      ...
      const licensePayload = {
        ...
        modules: ['enterprise_tree'],
        offline: false,
        leaseEndpoint: 'https://license.clawmaster.example/v1/lease',
        leaseToken: 'test-license-lease-token-at-least-32-characters',
      ...
      expect(storedLicense.raw_json).toContain('_clawmasterEncryptedSecretsV1')
  ...
  it('reports active seats and installs renewed License terms during lease refresh', async () => {
    ...
    try {
      ...
      const licensePayload = {
        ...
        modules: ['enterprise_tree'],
        offline: false,
        leaseEndpoint: 'https://license.clawmaster.example/v1/licenses/lic-lifecycle/lease',
        leaseToken: 'test-license-lease-token-at-least-32-characters',
        telemetryAllowed: true,
        telemetryToken: 'test-telemetry-token-at-least-32-characters',
        issuedAtMs: now,
    ...
    try {
      ...
      expect(
        control.ingestTelemetryBatch(
          ...
          {
            timestamp: uploadedHeaders['x-clawmaster-timestamp'],
            nonce: uploadedHeaders['x-clawmaster-nonce'],
            signature: uploadedHeaders['x-clawmaster-signature'],
          },
          now,
        ),
