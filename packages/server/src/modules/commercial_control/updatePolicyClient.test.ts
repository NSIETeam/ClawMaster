...
function policy(overrides: Record<string, unknown> = {}) {
  return {
    ...
    release: {
      ...
      notes: 'Green release',
      fullManifest: {
        url: 'https://updates.example.test/clawmaster-green/latest.json',
        sha256: 'a'.repeat(64),
      },
...
describe('deployment update policy client', () => {
  it('authenticates the request and accepts a correctly bound Ed25519 policy', async () => {
    ...
    const fetchImpl = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      const timestamp = headers.get('x-clawmaster-timestamp')
      const nonce = headers.get('x-clawmaster-nonce')
      ...
      expect(headers.get('x-clawmaster-signature')).toBe(
  ...
  it('fails closed for tampered, expired, or cross-distribution policies', async () => {
    ...
    for (const signedPolicy of [
      policy({ distributionId: 'clawmaster' }),
