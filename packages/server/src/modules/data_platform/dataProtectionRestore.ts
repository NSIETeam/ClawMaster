    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) visit(absolute);
        ...
        const relative = path
          ...
          .join('/');
        if (
          !/^[0-9a-f]{2}\/[0-9a-f]{2}\/[0-9a-f]{64}\.clawmaster-object$/.test(
            relative,
          )
        ) {
...
export async function verifyDataProtectionBackup(input: {
  ...
  const extractionDirectory = fs.mkdtempSync(
    path.join(temporaryRoot, '.clawmaster-verify-'),
  );
  try {
...
export async function restoreDataProtectionBackup(input: {
  ...
  const stagingDirectory = fs.mkdtempSync(
    path.join(path.dirname(dataDirectory), '.clawmaster-restore-staging-'),
  );
