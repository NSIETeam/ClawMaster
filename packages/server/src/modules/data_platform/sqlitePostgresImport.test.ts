...
function createSource(): Database {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'clawmaster-sqlite-postgres-import-'),
  );
...
class ImportTarget implements PostgresPoolLike, PostgresClientLike {
  ...
  async query<Row extends Record<string, unknown> = Record<string, unknown>>(
    ...
    if (
      sql.includes('FROM clawmaster_sqlite_import_runs') &&
    ...
    if (sql.includes('INSERT INTO clawmaster_sqlite_import_runs')) {
    ...
    if (sql.includes('INSERT INTO clawmaster_sqlite_import_rows')) {
    ...
    try {
      ...
      expect(
        target.statements.filter((sql) =>
          sql.includes('INSERT INTO clawmaster_sqlite_import_rows'),
        ),
      ).toHaveLength(4);
