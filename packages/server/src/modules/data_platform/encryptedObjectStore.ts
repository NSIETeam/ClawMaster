...
const OBJECT_MAGIC = Buffer.from('CLAWMASTEROBJ1')
...
function safeObjectKey(namespace: string, objectId: string): string {
  const digest = createHash('sha256')
    ...
    .update(objectId)
    .digest('hex')
  return `${digest.slice(0, 2)}/${digest.slice(2, 4)}/${digest}.clawmaster-object`
}

function assertSafeRelativeKey(key: string): string {
  const normalized = key.replace(/\\/g, '/')
  if (
    !/^[0-9a-f]{2}\/[0-9a-f]{2}\/[0-9a-f]{64}\.clawmaster-object$/.test(normalized)
  ) {
