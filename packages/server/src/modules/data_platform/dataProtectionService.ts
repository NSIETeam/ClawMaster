...
const BACKUP_NAME_PATTERN =
  /^clawmaster-enterprise-(\d{8}T\d{6}Z)-([0-9a-f]{8})\.clawmaster-backup$/
  ...
  async function runBackup(
    ...
    try {
      ...
      const timestamp = createdAt
        ...
        .replace(/[-:]/g, '')
        .replace(/\.\d{3}/, '')
      const archiveName = `clawmaster-enterprise-${timestamp}-${randomBytes(4).toString('hex')}.clawmaster-backup`
