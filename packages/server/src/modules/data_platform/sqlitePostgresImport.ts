  ...
  while (true) {
    const result = await input.client.query<{
      ...
      `SELECT row_index, row_sha256
       FROM clawmaster_sqlite_import_rows
  ...
  try {
    ...
    const existing = await client.query<{
      ...
      `SELECT id, state, source_schema_version, target_schema_version
       FROM clawmaster_sqlite_import_runs
    ...
    if (prior) {
      ...
      runOwned = true;
      await client.query(
        `UPDATE clawmaster_sqlite_import_runs
      ...
      await client.query(
        `INSERT INTO clawmaster_sqlite_import_runs
    ...
    for (const [tableIndex, descriptor] of descriptors.entries()) {
      ...
      await client.query(
        `INSERT INTO clawmaster_sqlite_import_tables
      ...
      while (true) {
        ...
        if (pending.length > 0) {
          await client.query(
            `INSERT INTO clawmaster_sqlite_import_rows
      ...
      await client.query(
        `UPDATE clawmaster_sqlite_import_tables
