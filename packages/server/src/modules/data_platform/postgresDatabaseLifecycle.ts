...
CREATE TABLE IF NOT EXISTS clawmaster_schema_migrations (
...
SELECT
  ...
  COALESCE((SELECT MAX(version) FROM clawmaster_schema_migrations), 0)::integer
    AS schema_version`
  ...
  async function initializeOnce(): Promise<PostgresDatabaseReadiness> {
    ...
    try {
      ...
      const appliedResult = await client.query<AppliedMigrationRow>(
        'SELECT version, name, checksum FROM clawmaster_schema_migrations ORDER BY version',
      )
      ...
      for (const migration of options.migrations) {
        ...
        await client.query(
          `INSERT INTO clawmaster_schema_migrations (version, name, checksum)
           VALUES ($1, $2, $3)`,
