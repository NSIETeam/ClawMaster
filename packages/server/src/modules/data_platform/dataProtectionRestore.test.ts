...
describe('data protection restore', () => {
  it('validates, restores and can roll back the protected data set', async () => {
    ...
    const restoredObject = path.join(sources, 'restored.clawmaster-object')
    ...
    const objectKey = `${'a'.repeat(2)}/${'b'.repeat(2)}/${'c'.repeat(64)}.clawmaster-object`
    const archivePath = path.join(root, 'backup.clawmaster-backup')
  ...
  it('restores deletion tombstones on a fresh host and removes them on rollback', async () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'clawmaster-restore-privacy-'),
    )
    temporaryDirectories.push(root)
    const dataDirectory = path.join(root, 'data')
    const sources = path.join(root, 'sources')
    ...
    const archivePath = path.join(root, 'privacy-backup.clawmaster-backup')
  ...
  it('refuses future schemas and a data directory used by a live server', async () => {
    ...
    const archivePath = path.join(root, 'future.clawmaster-backup')
  ...
  it('rejects a backup whose referenced attachment cannot be authenticated', async () => {
    ...
    const archivePath = path.join(root, 'corrupt-object.clawmaster-backup')
