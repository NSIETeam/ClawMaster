...
function createPaths(): {
  ...
  const dataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-db-life-'))
...
describe('enterprise database lifecycle', () => {
  ...
  it('fails before opening SQLite when the database path is a network share', () => {
    const lifecycle = createEnterpriseDatabaseLifecycle({
      dataDirectory: String.raw`\\server\share\clawmaster`,
      databasePath: String.raw`\\server\share\clawmaster\data.db`,
      schemaVersion: 1,
      initializeSchema() {},
    })
