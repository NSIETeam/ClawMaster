...
/**
 * setup 落盘：写入一个 BYO-key 自定义模型到 `~/.clawmaster-user/custom-models.json`。
 *
...
/** 拉取层级记忆文件内容（对齐 CLI /memory show：项目 CLAWMASTER.md + 全局 ~/.clawmaster/CLAWMASTER.md）。 */
export type GetMemoryMsg = Envelope<'get_memory', { sessionId?: string }>;

/** 追加一条记忆事实（对齐 save_memory 工具 / CLI /memory add），写入项目级 CLAWMASTER.md。 */
...
/** 单个记忆文件（层级：项目 CLAWMASTER.md / 全局 ~/.clawmaster/CLAWMASTER.md）。 */
...
export interface ExtensionSummary {
  name: string;
  version: string;
  /** 扩展所在目录（项目级或全局 ~/.clawmaster-user/extensions）。 */
  path: string;
}
 ...
 * 企业服务器网页用此接口检测用户机器上是否运行着 clawmaster。
 */
 ...
 * 仅允许这些来源的网页通过浏览器跨域探测本地 clawmaster。
...
export const DEFAULT_PORT = 7637; // 'CLAWMASTER' on a phone keypad-ish; 可被 env CLAWMASTER_SERVER_PORT 覆盖
