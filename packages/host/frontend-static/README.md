...
Root and configured-index responses call `ctx.connection.authorizeIndex` before reading HTML. A valid process token receives a 303 redirect plus the persistent browser cookie; an existing valid cookie serves the index; every other index request receives the Connection-owned 401 response. Successful index responses hash exact inline script and style blocks into their CSP and provide distinct, fresh script and style nonces for trusted DSH runtime consumers. The script nonce authorizes the dynamic Cordis runner to compile Host-authorized active-run source; it does not enable `eval`, inline event handlers, or arbitrary inline scripts. Script files and network requests remain same-origin; objects and remote frames are blocked. The `style-src-attr` exception supports runtime layout styles. Non-index files remain public static assets. Connection owns the token, cookie, expiry, and signing-record semantics.
