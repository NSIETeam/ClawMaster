...
使用脚本生成多格式交付。普通 `use_skill` 流程优先使用 ClawMaster 展示的
`copywriting` Skill 绝对目录；内置专家的默认安装目录是
`~/.clawmaster-user/skills/copywriting`。

macOS / Linux：

```bash
COPYWRITING_SKILL_DIR="$HOME/.clawmaster-user/skills/copywriting"
...
```powershell
$CopywritingSkillDir = Join-Path $HOME ".clawmaster-user\skills\copywriting"
