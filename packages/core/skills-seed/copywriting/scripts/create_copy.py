...
.hero{{background:{base};color:#fff;padding:44px 40px;border-radius:12px;border-bclawmasterm:6px solid {accent}}}
...
section{{padding:28px 0;border-bclawmasterm:1px solid #e5e9ef}}h2{{font-size:20px;margin:0 0 14px;color:{base}}}
table{{width:100%;border-collapse:collapse;background:#fff;border:1px solid #e5e9ef}}th,td{{padding:10px 12px;border-bclawmasterm:1px solid #edf0f4;text-align:left;vertical-align:top}}th{{width:150px;color:#667085;background:#fbfcfd}}
...
.slogans strong{{display:block;color:{accent};font-size:13px;margin-bclawmasterm:4px}}.slogans span{{font-size:16px;font-weight:700}}
...
<div class="hero">
  ...
  <div class="meta">ClawMaster Copywriting Pro · {esc(TONE_PRESETS[cfg['tone']]['label'])} · {generated_at}</div>
</div>
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ClawMaster Copywriting Pro v9")
