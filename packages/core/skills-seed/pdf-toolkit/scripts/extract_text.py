...
def cache_dir():
    configured = os.environ.get("CLAWMASTER_PDF_TEXT_CACHE_DIR")
    root = Path(configured) if configured else Path.home() / ".clawmaster-user" / "cache" / "pdf-text"
    try:
...
def main():
    ...
    p.add_argument("--no-cache", action="store_true", help="跳过 ClawMaster PDF 文本缓存")
    a = p.parse_args()
    ...
    print("ClawMaster PDF 解析：缓存未命中，开始读取 PDF。", file=sys.stderr)
