...
def apply_trusted_identity(meta, environ=None):
    ...
    author = re.sub(r"[\x00-\x1f\x7f]+", " ", env.get("CLAWMASTER_DOCUMENT_AUTHOR", "")).strip()[:160]
    department = re.sub(r"[\x00-\x1f\x7f]+", " ", env.get("CLAWMASTER_DOCUMENT_DEPARTMENT", "")).strip()[:160]
...
class R:
    ...
    def _setup(self):
        ...
        sec.top_margin=m; sec.bclawmasterm_margin=m; sec.left_margin=m; sec.right_margin=m
    ...
    def _styles(self):
        ...
        for lv in range(1,4):
            ...
            if lv==1:
                ...
                pPr.append(parse_xml(f'<w:pBdr {nsdecls("w")}><w:bclawmasterm w:val="single" w:sz="4" w:space="2" w:color="{self.t["accent"]}"/></w:pBdr>'))
    ...
    def cover(self):
        ...
        for w,sz in [(0.5,8),(0.2,2)]:
            ...
            pPr.append(parse_xml(f'<w:pBdr {nsdecls("w")}><w:bclawmasterm w:val="single" w:sz="{sz}" w:space="1" w:color="{accent}"/></w:pBdr>'))
        ...
        new.top_margin=m; new.bclawmasterm_margin=m; new.left_margin=m; new.right_margin=m
    ...
    def toc(self):
        ...
        pl._element.get_or_add_pPr().append(parse_xml(f'<w:pBdr {nsdecls("w")}><w:bclawmasterm w:val="single" w:sz="3" w:space="1" w:color="{self.t["accent"]}"/></w:pBdr>'))
    ...
    def table(self,hdrs,rows):
        ...
        for j,h in enumerate(hdrs):
            ...
            c._element.get_or_add_tcPr().append(parse_xml(f'<w:tcMar {nsdecls("w")}><w:top w:w="30"/><w:bclawmasterm w:w="30"/><w:left w:w="60"/><w:right w:w="60"/></w:tcMar>'))
        ...
        for i,row in enumerate(rows):
            for j,val in enumerate(row):
                ...
                c._element.get_or_add_tcPr().append(parse_xml(f'<w:tcMar {nsdecls("w")}><w:top w:w="20"/><w:bclawmasterm w:w="20"/><w:left w:w="60"/><w:right w:w="60"/></w:tcMar>'))
    ...
    def hf(self):
        ...
        hp._element.get_or_add_pPr().append(parse_xml(f'<w:pBdr {nsdecls("w")}><w:bclawmasterm w:val="single" w:sz="2" w:space="1" w:color="{self.t["accent"]}"/></w:pBdr>'))
