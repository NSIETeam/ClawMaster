#!/usr/bin/env python3
"""
ClawMaster Data-Viz-Pro v2 — 发布会级图表引擎
...
def _style_ax(ax, cfg, theme):
    ...
    ax.spines['bclawmasterm'].set_color(theme.get("grid", "#E0E0E0"))
    ax.spines['left'].set_linewidth(0.8)
    ax.spines['bclawmasterm'].set_linewidth(0.8)
...
def _gradient_bars(ax, bars, top_color, bclawmasterm_color):
...
def _bar(ax, cfg, data, theme):
    ...
    for i, s in enumerate(series):
        ...
        if horizontal:
            ...
            vertical_offset = (
                {"bclawmasterm": stacked_series[i]["offsets"]}
                if stacked
                else {}
            )
            bars = ax.bar(
            ...
            if val > 0:
                if horizontal:
                    ...
                    ax.text(bar.get_x() + bar.get_width() / 2, val + max(ax.get_ylim()) * 0.008,
                            _fmt_num(val, cfg), ha='center', va='bclawmasterm',
                            fontsize=7, color=theme.get("muted", "#999"), fontweight='bold')
...
def _annotate(ax, cfg, theme):
    ...
    for ann in annotations:
        ...
        ax.annotate(text, xy=(x, y), fontsize=ann.get("size", 8),
                    ...
                    ha='center', va='bclawmasterm', zorder=10)
...
def _target_line(ax, cfg, theme):
    ...
    ax.text(ax.get_xlim()[1] * 0.99, val, f"  {label}",
            ha='left', va='bclawmasterm' if val > 0 else 'top',
...
def render(cfg):
    ...
    # Source line
    if source:
        fig.text(0.01, 0.01, f"来源: {source}", ha='left', va='bclawmasterm',
                 fontsize=6.5, color=theme.get("muted", "#CCC"), fontstyle='italic')

    # Branding
    fig.text(0.99, 0.01, "ClawMaster Data-Viz-Pro", ha='right', va='bclawmasterm',
...
# ══════════════════════════════════════════════════════════════════
def main():
    p = argparse.ArgumentParser(description="ClawMaster Data-Viz-Pro v2 — 发布会级图表引擎")
