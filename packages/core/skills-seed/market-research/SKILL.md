...
安装版 ClawMaster 会把本 Skill 放在用户目录 `~/.clawmaster-user/skills/market-research`。
...
```bash
python3 --version
python3 "$HOME/.clawmaster-user/skills/market-research/scripts/create_research.py" config.json output.html
```
...
```powershell
py -3 --version
$SkillScript = Join-Path $HOME ".clawmaster-user\skills\market-research\scripts\create_research.py"
