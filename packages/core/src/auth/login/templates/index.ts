 ...
 * https://github.com/NSIETeam/clawmaster-new
...
export class AuthTemplates {
      ...
      <head>
        ...
        <style>
          ...
          .title {
            font-size: 28px;
            font-weight: 700;
            margin-bclawmasterm: 12px;
            color: #0f172a;
          }
          .subtitle {
            color: #64748b;
            margin-bclawmasterm: 32px;
            font-size: 16px;
          }
          .auth-button {
            display: flex;
      ...
      <body>
        <div class="container">
          ...
          <div id="auth-buttons-container" class="hidden">
            ...
            <button class="auth-button" onclick="startClawMasterAuth()" data-i18n="auth.clawmaster.button">
        ...
        <script>
          ...
          const translations = {
            en: {
              ...
              'auth.clawmaster.button': 'ClawMaster Login',
              'auth.china.restriction.title': 'Access Restricted',
              'auth.feishu.start.error': 'Failed to start Feishu authentication',
              'auth.clawmaster.start.error': 'Failed to start ClawMaster authentication'
            },
            zh: {
              ...
              'auth.clawmaster.button': 'ClawMaster 登录',
              'auth.china.restriction.title': '访问受限',
              'auth.feishu.start.error': '飞书认证启动失败',
              'auth.clawmaster.start.error': 'ClawMaster 认证启动失败'
            }
          };
          ...
          function startClawMasterAuth() {
            fetch('/start-clawmaster-auth', { method: 'POST' })
              .then(response => response.json())
              .then(data => {
                if (data.authUrl) {
                  window.location.replace(data.authUrl);
              ...
              .catch(error => alert(t('auth.clawmaster.start.error')));
          }
  ...
  private static generateClawMasterSuccessTemplate(): string {
    return `
      ...
      <html>
      <head>
        <title data-i18n="auth.clawmaster.success.title">ClawMaster Authentication Successful</title>
        <meta charset="utf-8">
        <style>
          * {
            margin: 0;
      ...
      <body>
        <div class="container">
          <div class="success">✓</div>
          <h1 class="title" data-i18n="auth.clawmaster.success.title">ClawMaster Login Successful</h1>
      ...
      <script>
        ...
        const translations = {
          en: {
            ...
            'auth.clawmaster.success.title': 'ClawMaster Login Successful',
          ...
          zh: {
            ...
            'auth.clawmaster.success.title': 'ClawMaster 登录成功',
