 ...
 * `.clawmaster` dir (logs, jwt-token, installation_id, etc.) may still be held open
...
function getLegacyMigrationUnits(projectRoot: string): LegacyMigrationUnit[] {
  ...
  return [
    ...
    {
      type: 'project',
      legacyDir: path.join(projectRoot, '.clawmaster'),
      newDir: path.join(projectRoot, '.clawmaster'),
    },
    // 2. User home directory configuration: ~/.clawmaster -> ~/.clawmaster-user
    {
      type: 'user',
      legacyDir: path.join(os.homedir(), '.clawmaster'),
      newDir: path.join(os.homedir(), '.clawmaster-user'),
    },
    // 3. System global public directory configuration: [GlobalBase]/.clawmaster -> [GlobalBase]/.clawmaster-global
    {
      type: 'global',
