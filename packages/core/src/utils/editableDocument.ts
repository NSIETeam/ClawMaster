...
async function writeDocxFromMarkdown(markdown: string, outPath: string): Promise<void> {
  ...
  zip.folder('word')?.file('document.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>' + body + '<w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1440" w:right="1440" w:bclawmasterm="1440" w:left="1440"/></w:sectPr></w:body></w:document>')
...
async function writePdfFromMarkdown(markdown: string, outPath: string): Promise<void> {
  ...
  const tmpDir = await fs.promises.mkdtemp(path.join(path.dirname(outPath), '.clawmaster-pdf-edit-'))
