...
describe('legacy directory migration', () => {
  ...
  beforeEach(() => {
    ...
    // user/global migration units from the developer's real ~/.clawmaster.
  ...
  // to .clawmaster -> .clawmaster (legacy === new), so it can no longer exercise a real
  // migration. The only unit with a distinct legacy source is the user unit
  // (~/.clawmaster -> ~/.clawmaster-user). os.homedir() is redirected to the sandbox via
  // HOME/USERPROFILE, so these tests drive the user-level unit safely without
  // touching the developer's real ~/.clawmaster.
  const legacyUserDir = () => path.join(fakeHome, '.clawmaster');
  const newUserDir = () => path.join(fakeHome, '.clawmaster-user');
  ...
  describe('needsLegacyMigration', () => {
    ...
    it('returns true when legacy user dir (.clawmaster) has real data', () => {
