 ...
 *
 * 日志存储：~/.clawmaster-user/memory/worklog/
...
export function resolveDefaultWorklogDir(): string {
  ...
  if (process.env['NODE_ENV'] === 'test' || process.env['VITEST']) {
    return path.join(tmpdir(), 'clawmaster-worklog-tests', String(process.pid))
  }
  return path.join(homedir(), '.clawmaster-user', 'memory', 'worklog')
}
