 ...
 * 3. 推送给用户确认（个人决定是否生成）
 * 4. 确认后写入 ~/.clawmaster-user/skills/<auto-skill-name>/SKILL.md
...
export function resolveAutoSkillUserDir(): string {
  ...
  if (process.env['NODE_ENV'] === 'test' || process.env['VITEST']) {
    return path.join(tmpdir(), 'clawmaster-auto-skill-tests', String(process.pid))
  }
  return path.join(homedir(), '.clawmaster-user')
}
...
function readProjectContext(cwd?: string): string {
  try {
    ...
    for (const filename of ['CLAWMASTER.md', 'CLAWMASTER.md']) {
    ...
    for (const name of fsSync.readdirSync(dir)) {
      try {
        ...
        const evidenceSignature = content.match(
          /<!--\s*clawmaster-auto-skill-evidence:([a-z0-9_-]+)\s*-->/iu,
        )?.[1]
        summaries.push({
          name,
...
export async function confirmAndSaveSkill(candidate: SkillCandidate): Promise<string> {
  ...
  const contentWithEvidence = candidate.evidenceSignature
    ? `${normalizedContent.trimEnd()}\n\n<!-- clawmaster-auto-skill-evidence:${candidate.evidenceSignature} -->\n`
    : `${normalizedContent.trimEnd()}\n`
