 ...
 *
 * 流程：
 * 1. 个人在 .clawmaster/skills/ 下有自己创建/自动生成的 Skill
...
export interface SkillShareRecord {
  id: string;
  /** 原始 Skill 名称（.clawmaster/skills/ 下的目录名） */
  skillName: string;
  /** 原始 SKILL.md 路径 */
  sourcePath: string;
  /** SKILL.md 内容 */
...
export class SkillShareManager {
  ...
  private get shareStorePath(): string {
    return path.join(this.config.getProjectRoot(), '.clawmaster', 'org', 'skill-shares.json');
  }

  /** 个人 Skills 目录 */
  private get personalSkillsDir(): string {
    return path.join(this.config.getProjectRoot(), '.clawmaster', 'skills');
  }

  /** 安装记录存储路径 */
  private get installRecordsPath(): string {
    return path.join(this.config.getProjectRoot(), '.clawmaster', 'org', 'skill-installs.json');
  }
   ...
   * 分享个人 Skill 到小组。
   *
   * 1. 读取 .clawmaster/skills/<skillName>/SKILL.md
   * 2. 创建分享记录
   ...
   * 安装小组共享的 Skill 到个人目录。
   *
   * 小组成员调用此方法，将团队共享的 Skill 复制到自己的 .clawmaster/skills/ 下。
   * 记录安装的版本号，用于后续更新检查。
   */
  ...
  /**
   * 列出当前用户可分享的个人 Skill（.clawmaster/skills/ 下的）。
   */
