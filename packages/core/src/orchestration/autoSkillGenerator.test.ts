...
beforeEach(async () => {
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'clawmaster-auto-skill-'));
  tempDirs.push(root);
...
describe('AutoSkillGenerator 个人 Skill 候选闭环', () => {
  ...
  it('stores portable pending paths and rehydrates them on another device', async () => {
    ...
    const restoredRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'clawmaster-auto-skill-restored-'));
