...
describe('WorkLogger 工作结果日志', () => {
  it('测试和企业自定义目录不会污染真实 ~/.clawmaster-user', () => {
    ...
    try {
      process.env['CLAWMASTER_WORKLOG_DIR'] = '/tmp/clawmaster-explicit-worklog';
      expect(resolveDefaultWorklogDir()).toBe('/tmp/clawmaster-explicit-worklog');
      delete process.env['CLAWMASTER_WORKLOG_DIR'];
      process.env['CLAWMASTER_USER_DIR'] = '/tmp/clawmaster-user-test';
      expect(resolveDefaultWorklogDir()).toBe(
        path.join('/tmp/clawmaster-user-test', 'memory', 'worklog'),
      );
      delete process.env['CLAWMASTER_USER_DIR'];
      process.env['VITEST'] = 'true';
      expect(resolveDefaultWorklogDir()).toContain(
        path.join('clawmaster-worklog-tests', String(process.pid)),
      );
    } finally {
  ...
  it('按本地日期落盘，而不是按 UTC 日期错位', async () => {
    ...
    await logger.log({
      toolName: 'clawmaster_work_result',
      action: '市场竞品调研',
      category: 'web',
      success: true,
