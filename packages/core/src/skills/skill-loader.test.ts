...
describe('SkillLoader', () => {
  ...
  beforeEach(async () => {
    ...
    testRoot = path.join(
      os.tmpdir(),
      `clawmaster-test-${Date.now()}-${Math.random().toString(36).substring(7)}`,
    )
...
`,
  ...
  // method to filter entries, so `ln -s <repo> ~/.clawmaster/skills/foo`
  ...
  describe('symlink discovery', () => {
    it('discovers symlinked skill directories alongside real ones', async () => {
      ...
      const projectSkillsDir = path.join(testRoot, '.clawmaster-user', 'skills')
