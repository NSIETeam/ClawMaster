...
const skillRoot = resolve(repoRoot, '.clawmaster/skills/data-viz-pro')
...
function pythonEnvironmentWithoutPlottingDependencies(): NodeJS.ProcessEnv {
  const blockers = makeTemporaryDirectory('clawmaster-data-viz-blocked-deps-')
  writeFileSync(
...
function runRenderHarness(mode: 'failed' | 'missing' | 'empty') {
  const outputDirectory = makeTemporaryDirectory('clawmaster-data-viz-render-')
  const harness = `
...
spec = importlib.util.spec_from_file_location("clawmaster_analyze_data", ${JSON.stringify(analyzeScript)})
...
spec = importlib.util.spec_from_file_location("clawmaster_create_chart", ${JSON.stringify(chartScript)})
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
...
`
  ...
  it('声明完整运行依赖，并提供跨平台 Python 3 命令', () => {
    ...
    expect(skillInstructions).not.toContain('.clawmaster/skills')
