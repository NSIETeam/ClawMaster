 ...
 * ~/.clawmaster-user/skills/，让**任何安装（含打包的桌面 App）开箱即有 skill 可用**。
 *
...
const managedStateFile = '.clawmaster-builtin-skill.json'
...
/**
 * 预置内置 skill 到 ~/.clawmaster-user/skills/。
...
export function seedDefaultSkills(): string[] {
  ...
  const target = join(homedir(), '.clawmaster-user', 'skills')
  const seeded: string[] = []
