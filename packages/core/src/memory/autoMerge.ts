 ...
 *   已有的记忆存储文件：
 *     - ~/.clawmaster-user/memory/global.md   (save_memory 写入 ClawMaster 记忆段落)
 *     - ~/.clawmaster-user/knowledge/entries.jsonl (knowledge_base 写入，JSONL)
 *
...
const MEMORY_ROOT = path.join(homedir(), '.clawmaster-user', 'memory')
const KNOWLEDGE_ROOT = path.join(homedir(), '.clawmaster-user', 'knowledge')
...
export class AutoMemoryEngine {
  ...
  private inferTopicsFromText(text: string): string[] {
    ...
    const hints: Array<[RegExp, string]> = [
      ...
      [/ClawMaster|桌面|Electron|desktop/i, 'clawmaster'],
