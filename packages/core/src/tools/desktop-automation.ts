...
public class Win32Api {
  ...
  public struct RECT { public int Left, Top, Right, Bclawmasterm; }
...
EXAMPLES:
  ...
  window_manager: {action:"window_manager", app_name:"Finder", window_operation:"tile_left"}
    operations: minimize, maximize, restore, close, front, tile_left, tile_right, tile_top, tile_bclawmasterm, fullscreen
...
DEPENDENCIES: macOS needs cliclick (brew install cliclick). Windows needs nothing extra.`;
    super(DesktopAutomationTool.Name, 'DesktopAutomation', desc, Icon.Terminal,
      {
        ...
        properties: {
          ...
          window_operation: { type: Type.STRING, enum: ['minimize','maximize','restore','close','front','tile_left','tile_right','tile_top','tile_bclawmasterm','fullscreen'], description: 'Window operation. Must combine with app_name.' },
  ...
  private async macWindow(app: string, op: string): Promise<string> {
    ...
    const scripts: Record<string,string> = {
      ...
      tile_bclawmasterm: 'tell application "System Events"\ntell process "'+a+'"\nset position of window 1 to {0,'+hh+'}\nset size of window 1 to {'+w+','+hh+'}\nend tell\nend tell',
    };
    const sc=scripts[op];
...
$h=$t.MainWindowHandle; if($h -eq [IntPtr]::Zero){throw "No main window: ${this.pe(n)}"}`;
    const ops: Record<string,string> = {
      ...
      tile_bclawmasterm:fw+'\n$w=[Win32Api]::GetSystemMetrics(0);$h2=[Math]::Floor([Win32Api]::GetSystemMetrics(1)/2);[Win32Api]::MoveWindow($h,0,$h2,$w,$h2,$true);"tiled-bclawmasterm"',
    };
  ...
  private async winGetWinPos(app: string): Promise<string> {
    const o=await this.ps('$t=Get-Process "'+this.pe(this.wpn(app))+'" -ErrorAction SilentlyContinue|Select -First 1;'+
      ...
      '$r=New-Object Win32Api+RECT;[Win32Api]::GetWindowRect($h,[ref]$r);"$($r.Left),$($r.Top),$($r.Right-$r.Left),$($r.Bclawmasterm-$r.Top)"');
