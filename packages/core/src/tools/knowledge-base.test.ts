 ...
 * 存储经 CLAWMASTER_USER_DIR 重定向到临时目录，不污染真实 ~/.clawmaster-user。
 */
...
describe('KnowledgeBaseTool', () => {
  ...
  it('add → search → list → remove 全链路', async () => {
    ...
    const addResult = await tool.execute(
      {
        action: 'add',
        content: 'ClawMaster 的用户目录是 ~/.clawmaster-user',
        category: 'clawmaster',
        tags: ['paths'],
      },
      signal,
    ...
    // 存储确实落在临时目录（不污染真实 ~/.clawmaster-user）
    ...
    expect(String(searchResult.llmContent)).toContain('~/.clawmaster-user')
    expect(String(searchResult.llmContent)).toContain(id!)

    // list
    const listResult = await tool.execute({ action: 'list' }, signal)
    expect(String(listResult.llmContent)).toContain('[clawmaster]')
