...
interface NativePptxTextOptions {
  ...
  w: number
  h: number
  valign?: 'top' | 'middle' | 'bclawmasterm'
...
export class ChromeHtmlToImageRenderer implements HtmlToImageRenderer {
  ...
  async render(request: HtmlToImageRenderRequest): Promise<void> {
    ...
    const browserProfilePath = fs.mkdtempSync(
      path.join(os.tmpdir(), 'clawmaster-chrome-profile-'),
    )
    try {
      const commonArgs = [
...
DEPENDENCIES: PPTX needs a local Chrome/Edge/Chromium browser and never runs Python. Markdown needs none. Slide PDF/HTML need marp-cli; other formats may need typst or pandoc. External engines run a doctor preflight and fail loud with an install command if missing (never faking output). macOS: brew install typst pandoc; npm i -g @marp-team/marp-cli. Windows: winget install typst pandoc; npm i -g @marp-team/marp-cli.`
    super(GenerateDocumentTool.Name, 'GenerateDocument', desc, Icon.Pencil,
      {
        type: Type.OBJECT,
        properties: {
          content: { type: Type.STRING, description: 'Markdown content. Slides: --- for page breaks, optional <!-- layout: xxx --> per page. ClawMaster auto-infers layout from content position, density, images, and data patterns. Bold numbers (**42%**) + pipe tables trigger native editable text & table objects.' },
  ...
  async execute(
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-doc-'))
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .eyebrow { display: block; margin-bclawmasterm: 24px; color: var(--primary); font-size: 18px; font-weight: 800; letter-spacing: .13em; }
    h1 {
    ...
    .visual-frame figcaption { position: absolute; left: 24px; bclawmasterm: 18px; color: var(--muted); font-size: 17px; }
    .footer {
      position: absolute; right: 112px; bclawmasterm: 52px; color: var(--muted);
    ...
    body[data-layout="cover"] .composition { display: flex; flex-direction: column; justify-content: flex-end; padding-bclawmasterm: 90px; }
    ...
    body[data-layout="section"]::before {
      width: 1160px; height: 480px; left: -210px; bclawmasterm: -250px;
    ...
    body[data-layout="statement"] .content strong { display: block; margin-bclawmasterm: 18px; font-size: 148px; line-height: .88; letter-spacing: -.055em; }
    body[data-layout="statement"]::before {
      width: 720px; height: 720px; right: 80px; bclawmasterm: -430px; border-radius: 50%;
    ...
    body[data-layout="timeline"]::before {
      width: calc(100% - 224px); height: 2px; left: 112px; bclawmasterm: 150px;
    ...
    body[data-layout="split"] .panel .panel-body p { margin-bclawmasterm: 16px; }
    ...
    body[data-layout="visual"] .composition {
      ...
      gap: 88px; align-items: stretch; padding-top: 54px; padding-bclawmasterm: 64px;
    }
    ...
    body[data-density="dense"] .content li { min-height: 86px; padding-top: 16px; padding-bclawmasterm: 16px; }
