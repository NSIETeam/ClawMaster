...
async function main() {
  ...
  //      使 nvm/homebrew 等 PATH 生效，确保 clawmaster 命令可找到）
 ...
 *
 * 重启方式（双轨）：
 *   - Windows: cmd.exe /c clawmaster --feishu（有 conpty，用户能看到界面）
 *   - Linux/macOS: login shell -l -c clawmaster --feishu（加载 .bashrc/.profile，
 *     使 nvm/homebrew 等 PATH 生效，确保 clawmaster 命令可找到）
 *
 * @returns 写出的脚本路径
 */
 ...
 *
 * 用法：
 *   - 默认：从 npm 安装 clawmaster-ai@latest 并重启
  ...
  constructor(_config: Config) {
    super(
      ...
      'Updates and/or restarts ClawMaster (Feishu/Lark gateway mode only). Use ONLY when the user ' +
        ...
        '- source: "npm" (default) installs clawmaster-ai@latest from npm; "local" installs a local .tgz ' +
        '(requires sourcePath).\n' +
        ...
        '`clawmaster --feishu` automatically. The bot is briefly offline (tens of seconds) and returns by ' +
      ...
      Icon.Hammer,
      {
        ...
        properties: {
          ...
          source: {
            ...
            enum: ['npm', 'local'],
            description:
              'npm (default) = install clawmaster-ai@latest; local = install a local .tgz (needs sourcePath). Ignored when action=restart_only.',
          },
          sourcePath: {
            type: Type.STRING,
            description:
  ...
  async execute(
    ...
    return {
      llmContent:
        ...
        'then relaunch `clawmaster --feishu`. The bot will be briefly offline and return automatically. ' +
