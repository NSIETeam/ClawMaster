...
const MEMORY_DIR = path.join(os.homedir(), '.clawmaster', 'memory')
...
FILES CREATED:
  ~/.clawmaster/memory/employee.markdown   - Personal habits + efficiency (LOCAL)
  ~/.clawmaster/memory/department.markdown - SOPs + templates (CLOUD SYNC)
  ~/.clawmaster/memory/role.markdown       - Role workflows (CLOUD SYNC)
  ~/.clawmaster/memory/workflows/*.md      - Task-specific templates
  ~/.clawmaster/memory/reports/*.md        - Token + efficiency reports`
  ...
  private async recallFromEnterprise(p: MemoryManagerToolParams): Promise<string | null> {
    ...
    try {
      ...
      if (token) headers['x-clawmaster-admin-token'] = token
...
`
  ...
  private sync(_p: MemoryManagerToolParams): string {
    ...
    for (const [name, fpath] of [['department', path.join(MEMORY_DIR, 'department.markdown')], ['role', path.join(MEMORY_DIR, 'role.markdown')]]) {
      if (fs.existsSync(fpath)) {
        ...
        const tmpFile = path.join(os.tmpdir(), `clawmaster_sync_${name}_${Date.now()}.md`)
  ...
  private export(p: MemoryManagerToolParams): string {
    const outPath = p.output_path || path.join(os.homedir(), 'Desktop', 'clawmaster_knowledge_export.md')
