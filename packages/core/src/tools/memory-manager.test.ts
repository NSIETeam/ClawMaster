...
describe('MemoryManagerTool recall — department/company knowledge unification', () => {
  ...
  it('merges knowledge from a real enterprise server (real HTTP, not a stub) into recall output', async () => {
    ...
    const result = await tool.execute({
      action: 'recall',
      task_type: 'clawmaster-recall-unification-test-' + Date.now(),
      department_id: 'sales',
    }, new AbortController().signal)
  ...
  it('sends the admin token header when CLAWMASTER_ENTERPRISE_ADMIN_TOKEN is set', async () => {
    ...
    const result = await tool.execute({
      action: 'recall',
      task_type: 'clawmaster-recall-token-test-' + Date.now(),
    }, new AbortController().signal)
  ...
  it('gracefully degrades (no crash, no error content) when the enterprise server is unreachable', async () => {
    ...
    const result = await tool.execute({
      action: 'recall',
      task_type: 'clawmaster-recall-unreachable-test-' + Date.now(),
    }, new AbortController().signal)
