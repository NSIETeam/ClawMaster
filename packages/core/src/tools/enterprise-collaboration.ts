...
4. Call ask_peer_clawmaster with recipientAccountId and question only when the user asks another employee's ClawMaster. The recipient controls permission and may allow explicitly selected current-chat segments, enterprise knowledge, work logs, and schedules, allow only selected categories, or deny the request. This scope does not include files, API keys, other chats, or unselected direct messages. Respect the returned scope; never bypass it.
      ...
      Icon.Info,
      {
        ...
        properties: {
          ...
          question: {
            type: Type.STRING,
            description:
              'Question or negotiation goal (1-4000 characters). Used only by ask_peer_clawmaster and consult_peer_clawmaster.',
          },
          department: {
            type: Type.STRING,
            description:
