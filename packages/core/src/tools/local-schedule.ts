 ...
 * `~/.clawmaster-user/schedules.json`，避免 UI 日历和 Agent 自主创建各存一套。
 */
...
export type LocalScheduleSource = 'user' | 'clawmaster'
...
export function localScheduleFilePath(): string {
  ...
  return configured || path.join(os.homedir(), '.clawmaster-user', 'schedules.json')
}
...
function isScheduleItem(value: unknown): value is LocalScheduleItem {
  ...
  return (
    ...
    (item.source === 'user' || item.source === 'clawmaster') &&
...
/** ClawMaster 可主动调用的本地日程工具。所有创建项都标记 source=clawmaster。 */
