...
describe('buildRelaunchScript', () => {
  const base = {
    parentPid: 12345,
    relaunchCommand: 'clawmaster',
    relaunchArgs: ['--feishu'],
    scriptPath: '/tmp/clawmaster-relaunch-12345.js',
  }

  const npmMode: RelaunchInstallMode = { type: 'npm', packageName: 'clawmaster-ai' }
  const noneMode: RelaunchInstallMode = { type: 'none' }
  const tgzMode: RelaunchInstallMode = { type: 'tgz', path: '/abs/clawmaster-ai-1.1.3.tgz' }
  ...
  it('tgz mode embeds the local tgz absolute path install', () => {
    ...
    expect(script).toContain('/abs/clawmaster-ai-1.1.3.tgz')
    expect(script).toContain('install')
    expect(script).toContain('-g')
  ...
  it('redirects relaunch output to a log file when logPath is provided', () => {
    const script = buildRelaunchScript({
      ...base,
      install: noneMode,
      logPath: '/home/u/.clawmaster-user/relaunch.log',
    })
