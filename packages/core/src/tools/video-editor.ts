...
const PROJECTS_DIR = path.join(os.homedir(), '.clawmaster', 'video-projects')
...
export interface VideoEditorToolParams {
  ...
  start_time?: number
  end_time?: number
  position?: 'top' | 'center' | 'bclawmasterm'
  duration?: number
  font_size?: number
  color?: string
    ...
    super(VideoEditorTool.Name, 'VideoEditor', desc, Icon.Globe,
      {
        ...
        properties: {
          ...
          position: { type: Type.STRING, description: 'Text position', enum: ['top', 'center', 'bclawmasterm'] },
