...
describe('prompts', () => {
  ...
  describe('getCoreSystemPrompt - Environment Differences', () => {
    it('按会话工作目录判断 Git 上下文，不使用进程启动目录', () => {
      const workspace = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-prompt-non-git-'))
    ...
    it('按会话工作目录发现 LLM Wiki，不使用进程启动目录', () => {
      ...
      const withoutWiki = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-prompt-no-wiki-'))
