...
export interface MCPResponseGuardConfig {
  ...
  /**
   * 临时文件目录
   * 默认: 项目根目录下的 .clawmaster/mcp-tmp
   * 如果未找到项目目录，则使用 HOME/.clawmaster/mcp-tmp
   * 最后降级到系统临时目录
   */
  tempDir?: string;
...
export class MCPResponseGuard {
  ...
  /**
   * 获取项目临时目录
   * 存储在 .clawmaster/mcp-tmp 下，避免系统临时目录问题
   */
  private getProjectTempDir(): string {
    // 尝试获取项目根目录，如果不可用则使用系统临时目录
    try {
      // 从当前工作目录向上查找 .clawmaster 文件夹
      let currentDir = process.cwd();
      let depth = 0;
      const maxDepth = 10; // 最多向上查找10层

      while (depth < maxDepth) {
        const clawmastercodePath = path.join(currentDir, '.clawmaster');
        if (fs.existsSync(clawmastercodePath)) {
          return path.join(clawmastercodePath, 'mcp-tmp');
        }
    ...
    // 备选：使用 HOME/.clawmaster/mcp-tmp
    const homeDir = process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH;
    if (homeDir) {
      return path.join(homeDir, '.clawmaster-user', 'mcp-tmp');
    }

    // 最后的备选：使用系统临时目录
    if (process.platform === 'win32') {
      return path.join(process.env.TEMP || process.env.TMP || 'C:\\Windows\\Temp', 'clawmastercode-mcp');
    } else {
      return path.join(process.env.TMPDIR || '/tmp', 'clawmastercode-mcp');
    }
  }
