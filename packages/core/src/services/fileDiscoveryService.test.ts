...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .clawmasterignore patterns even when not in a git repo', async () => {
      await createTestFile('.clawmasterignore', 'secrets.txt')
      const service = new FileDiscoveryService(projectRoot)
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'))
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist')
      await createTestFile('.clawmasterignore', 'logs/')
    });
  ...
  describe('shouldGitIgnoreFile & shouldGeminiIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'))
      await createTestFile('.gitignore', 'node_modules/')
      await createTestFile('.clawmasterignore', '*.log')
    });
