...
/** Default on-disk home for persisted delegate tasks: ~/.clawmaster-user/delegate-tasks */
function defaultStorageDir(): string {
  return path.join(os.homedir(), '.clawmaster-user', 'delegate-tasks');
}
...
export interface BackgroundTaskManagerOptions {
   ...
   * `~/.clawmaster-user/delegate-tasks`. Pass `null` to disable persistence
...
export function getBackgroundTaskManager(): BackgroundTaskManager {
  if (!globalTaskManager) {
    ...
    //   - Otherwise default to ~/.clawmaster-user/delegate-tasks.
    const override = (
