...
describe('diagnostic bundle', () => {
  ...
  it('生成 zip 且不包含 secrets 目录', async () => {
    ...
    await mkdir(path.join(home, '.clawmaster-user', 'logs'), { recursive: true })
    await mkdir(path.join(home, '.clawmaster-user', 'secrets'), { recursive: true })
    await writeFile(path.join(home, '.clawmaster-user', 'logs', 'server.log'), 'apiKey=sk-hidden')
    await writeFile(path.join(home, '.clawmaster-user', 'secrets', 'model'), 'sk-hidden')
  ...
  it('跳过符号链接日志，防止诊断包越界读取', async () => {
    ...
    await symlink(
      outside,
      path.join(home, '.clawmaster-user', 'logs', 'linked'),
