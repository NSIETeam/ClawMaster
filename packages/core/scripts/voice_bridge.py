#!/usr/bin/env python3
"""
ClawMaster Voice Bridge - lightweight voice input for ClawMaster Agent.
...
DEFAULT_WHISPER_MODEL = os.environ.get("CLAWMASTER_WHISPER_MODEL", "auto").strip() or "auto"
DEFAULT_WHISPER_LANGUAGE = os.environ.get("CLAWMASTER_WHISPER_LANGUAGE", "").strip() or None
DEFAULT_WHISPER_INITIAL_PROMPT = os.environ.get(
    "CLAWMASTER_WHISPER_INITIAL_PROMPT",
...
DEFAULT_WHISPER_TIMEOUT = int(os.environ.get("CLAWMASTER_WHISPER_TIMEOUT", "300"))
DEFAULT_WHISPER_BEAM_SIZE = int(os.environ.get("CLAWMASTER_WHISPER_BEAM_SIZE", "5"))
DEFAULT_WHISPER_TEMPERATURES = os.environ.get("CLAWMASTER_WHISPER_TEMPERATURES", "0,0.2").strip()
DEFAULT_WHISPER_NO_SPEECH_THRESHOLD = float(os.environ.get("CLAWMASTER_WHISPER_NO_SPEECH_THRESHOLD", "0.6"))
DEFAULT_WHISPER_LOGPROB_THRESHOLD = float(os.environ.get("CLAWMASTER_WHISPER_LOGPROB_THRESHOLD", "-1.0"))
DEFAULT_WHISPER_COMPRESSION_RATIO_THRESHOLD = float(os.environ.get("CLAWMASTER_WHISPER_COMPRESSION_RATIO_THRESHOLD", "2.4"))
DEFAULT_ASR_BACKEND = os.environ.get("CLAWMASTER_ASR_BACKEND", "auto").strip().lower() or "auto"
...
DEFAULT_FASTER_WHISPER_DEVICE = os.environ.get("CLAWMASTER_FASTER_WHISPER_DEVICE", "auto").strip() or "auto"
DEFAULT_FASTER_WHISPER_COMPUTE_TYPE = os.environ.get("CLAWMASTER_FASTER_WHISPER_COMPUTE_TYPE", "default").strip() or "default"
...
def whisper_model_candidates():
    ...
    configured = (os.environ.get("CLAWMASTER_WHISPER_MODEL") or DEFAULT_WHISPER_MODEL).strip()
...
def transcribe_with_local_whisper(audio_path):
    ...
    env.setdefault("CLAWMASTER_WHISPER_LANGUAGE", DEFAULT_WHISPER_LANGUAGE or "")
    env.setdefault("CLAWMASTER_WHISPER_INITIAL_PROMPT", DEFAULT_WHISPER_INITIAL_PROMPT)
    env["CLAWMASTER_WHISPER_MODEL_CANDIDATES"] = json.dumps(whisper_model_candidates())
    env.setdefault("CLAWMASTER_WHISPER_BEAM_SIZE", str(DEFAULT_WHISPER_BEAM_SIZE))
    env.setdefault("CLAWMASTER_WHISPER_TEMPERATURES", DEFAULT_WHISPER_TEMPERATURES)
    env.setdefault("CLAWMASTER_WHISPER_NO_SPEECH_THRESHOLD", str(DEFAULT_WHISPER_NO_SPEECH_THRESHOLD))
    env.setdefault("CLAWMASTER_WHISPER_LOGPROB_THRESHOLD", str(DEFAULT_WHISPER_LOGPROB_THRESHOLD))
    env.setdefault("CLAWMASTER_WHISPER_COMPRESSION_RATIO_THRESHOLD", str(DEFAULT_WHISPER_COMPRESSION_RATIO_THRESHOLD))
    env.setdefault("CLAWMASTER_ASR_BACKEND", DEFAULT_ASR_BACKEND)
    env.setdefault("CLAWMASTER_FASTER_WHISPER_DEVICE", DEFAULT_FASTER_WHISPER_DEVICE)
    env.setdefault("CLAWMASTER_FASTER_WHISPER_COMPUTE_TYPE", DEFAULT_FASTER_WHISPER_COMPUTE_TYPE)
    env["CLAWMASTER_NO_SPEECH_EXIT_CODE"] = str(NO_SPEECH_EXIT_CODE)
...
models = json.loads(os.environ.get("CLAWMASTER_WHISPER_MODEL_CANDIDATES", '["medium","small","base"]'))
language = os.environ.get("CLAWMASTER_WHISPER_LANGUAGE") or None
initial_prompt = os.environ.get("CLAWMASTER_WHISPER_INITIAL_PROMPT") or None
beam_size = int(os.environ.get("CLAWMASTER_WHISPER_BEAM_SIZE", "5"))
temperatures = tuple(float(t.strip()) for t in os.environ.get("CLAWMASTER_WHISPER_TEMPERATURES", "0,0.2").split(",") if t.strip())
no_speech_threshold = float(os.environ.get("CLAWMASTER_WHISPER_NO_SPEECH_THRESHOLD", "0.6"))
logprob_threshold = float(os.environ.get("CLAWMASTER_WHISPER_LOGPROB_THRESHOLD", "-1.0"))
compression_ratio_threshold = float(os.environ.get("CLAWMASTER_WHISPER_COMPRESSION_RATIO_THRESHOLD", "2.4"))
backend = os.environ.get("CLAWMASTER_ASR_BACKEND", "auto").lower()
faster_device = os.environ.get("CLAWMASTER_FASTER_WHISPER_DEVICE", "auto")
faster_compute_type = os.environ.get("CLAWMASTER_FASTER_WHISPER_COMPUTE_TYPE", "default")
no_speech_exit_code = int(os.environ.get("CLAWMASTER_NO_SPEECH_EXIT_CODE", "3"))
supported_backends = {"auto", "faster-whisper", "faster_whisper", "openai-whisper", "openai_whisper", "whisper"}
if backend not in supported_backends:
    print(f"Unsupported CLAWMASTER_ASR_BACKEND: {backend}", file=sys.stderr)
    sys.exit(2)
last_error = None
...
'''
    ...
    try:
        ...
        print(
            ...
            "Try CLAWMASTER_WHISPER_MODEL=small or CLAWMASTER_WHISPER_TIMEOUT=600.",
            file=sys.stderr,
        )
    except Exception as e:
...
def transcribe_with_user_api(audio_path):
    ...
    try:
        ...
        with open(audio_path, "rb") as f:
            files = {"file": f}
            data = {"model": os.environ.get("CLAWMASTER_ASR_MODEL", "whisper-1")}
...
def polish_to_command(raw_text, mode="polished"):
    """Polish raw speech into structured ClawMaster command via LLM."""
    if mode == "raw":
        return raw_text
    ...
    system_prompt = """You are ClawMaster Agent's voice input processor.
...
def main():
    parser = argparse.ArgumentParser(description="ClawMaster Voice Bridge")
    ...
    if not DEFAULT_ASR_BACKEND_VALID:
        print(
            f"ERROR: Unsupported CLAWMASTER_ASR_BACKEND={DEFAULT_ASR_BACKEND}. "
    ...
    if not text:
        ...
        print("  Low-spec computer: set CLAWMASTER_WHISPER_MODEL=small", file=sys.stderr)
        sys.exit(1)
