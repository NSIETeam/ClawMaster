...
`dsh-jobs-local` 在 harness 进程内运行后台任务：工作会在 agent（智能体）继续推进的同时保持运行，拥有它的 agent 可以读取、等待、列出和取消它；同时挂载 `dsh-tool-jobs` 时，完成以会话内通知送达。它用内存记录实现 `dsh-jobs` 约定，并且只交出全新快照，从不交出实时状态。所有者和进程级并发上限（均默认 10）共同约束各 agent 和无主工作的活动任务数量；任务会随 harness 进程终止而消失，无法跨重启持久。
...
加载插件即注册 `ctx.jobs`；`maxConcurrentJobsPerOwner` 与 `maxConcurrentJobs` 均可选，各自默认为 `10`。
...
| 字段 | 默认值 | 含义 |
|---|---|---|
| `maxConcurrentJobsPerOwner` | `10` | 每个精确所有者或共享无主桶中的最大活动任务数量 |
| `maxConcurrentJobs` | `10` | 全部所有者与无主桶合计的最大活动任务数量 |

生成的[配置目录](../../../docs/config-catalog.zh.md#deepseek-aidsh-jobs-local)是每个受支持字段的穷尽式真源。

### 每个所有者得到什么

两项上限均统计 `running` 与 `stopping` 记录，并在同步生产方启动器运行期间预留名额。全部无主任务共享一个所有者桶；进程上限还统计其他 agent 拥有的工作。终止历史不占用容量，只有生产方的 `done` 结算才释放一个停止中任务的名额。达到任一上限时，`start()` 会在生产方运行或获得 id 前失败。错误指出上限，并提示调用者等待活动工作结束后重试；注册表既不排队也不抢占。
...
`activeTaskCount` 从活动记录和同步启动器栈计算所有者及进程占用；启动器抛错时释放预留名额。`settle` 在存在挂起等待方时把任务标为已报告，解析每个等待方，记录终止快照，宣布可见集变更，然后通知完成监听器。挂起的等待会在监听器运行前把任务标为已报告，因此完成报告方不会重复通知；销毁时的取消出于同样理由标记——面向正在被销毁的所有者的通知不会有人读到。
...
- **任务数量不构成进程内存上限**——一个任务可能拥有多个子进程；WebView、外部应用和注册表以外的工作不计入其中。
