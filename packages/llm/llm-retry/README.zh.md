...
挂载 `@deepseek-ai/dsh-llm-retry`，可在持久 agent 步骤边界重试失败的模型请求。提供方的 `retryPolicy` 设置可选择有界的 normal mode 重试或仅针对暂时故障的 always mode 恢复；计划的尝试会在退避前写入会话日志，取消后历史仍保持一致。重试会在同一个打开的轮次内重跑失败步骤，而直接 `ctx.llm.stream()` 调用仍只尝试一次。每次重试都会产生另一次提供方请求计费。always mode 遇到凭据、配额、模型、能力或上下文超限等永久失败会停止，避免请求风暴。
...
省略 `retryPolicy` 时使用 normal mode：对 `EMPTY_RESPONSE`、`RATE_LIMIT`、`SERVER`、`TIMEOUT` 与 `TRANSPORT` 最多重试五次，退避从 500 毫秒到 10 秒、带 10% 抖动。normal mode 可以更改其有界预算、合格 code 与退避；always mode 先询问下游恢复，然后无尝试上限地重试暂时故障，只在成功、取消、插件释放或永久失败（`AUTH`、`QUOTA`、上下文超限、凭据缺失/无效、未知模型、适配器缺失、选项不支持）时停止。
...
HTTP 401/403 和已分类的永久故障在两种模式下都不会安排执行器重试，即使提供方将其标为暂时故障码。Always mode 保留下游恢复决策；如果下游在处理永久故障时抛出错误，执行器传播该错误，不会再次调用恢复逻辑。
...
- **always mode 没有暂时故障的尝试上限**——暂时故障会持续到成功、取消或释放；内置永久失败分类会阻止凭据、配额、模型、能力和上下文超限请求风暴，部署方仍负责提供方专属的成本与延迟控制。
