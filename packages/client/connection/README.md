...
The cookie signing secret is the owner-scoped `client-connection/browser-session` grant record in `ctx.credentials`. The local provider persists it in `$DSH_HOME/.credentials.yaml`; `BrowserAuth` loads or creates the record during Connection activation and retains the selected secret for the Host lifetime, so request authentication is synchronous and Connection reloads do not invalidate active cookies. If credential storage is unavailable, Connection logs a warning and uses a random process-only secret so the application can still open; those browser cookies become invalid when that Host exits. Recovery or failure of credential storage during that Host lifetime does not replace its selected secret. Deleting or replacing a persisted record takes effect on the next Host launch. Cookies carry an absolute issue/expiry interval, defaulting to 30 days through `cookieMaxAgeDays`, and bind the normalized hostname plus port in both their deterministic name and signed payload. They are host-only, `Path=/`, `HttpOnly`, and `SameSite=Strict`; they deliberately omit `Secure` because the shipped server uses loopback HTTP.
