...
`dsh-client-ui-settings-models` 是 dsh Web 客户端的 Models 设置页面：用户可以配置 API 密钥（以只写方式存入 profile 的凭据引用之下）、编辑每个提供方的模型列表，并手工声明自定义 pi-ai 路由；页面以提供方行展示，一次只展开一张编辑卡片。该页面把提供方目录、设置文档与凭据描述合并为一个共享快照，因此行的状态在三个方面始终一致。它还会带首次运行的用户走两个有序弹窗——版本化欢迎提示，以及按条件显示的官方 DeepSeek 凭据步骤。
...
ClawMaster 构建（`DSH_CLIENT_BUILD_PROFILE=clawmaster`）保留完整的模型页面，将两个自动弹窗交给 [WatchDog 教程](../../../frontends/dsh/README.zh.md#first-run-tutorial)。其他构建配置保留下述引导顺序。
...
声明步骤在 `src/client/locales.ts` 中持有精确文案，并在 `src/onboarding-copy.ts` 中持有确认版本；回环时它通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`，且只有显式点击「继续」才会记录当前版本。设置了 `DSH_CLIENT_TITLE` 的其他非官方构建显示带该产品名称的本地化欢迎提示；官方构建与未设置标题的构建显示 DSH 内测声明。非回环浏览器无法使用这个仅限宿主的 namespace，因此确认只保留在进程内，刷新后声明会再次出现。DeepSeek 步骤在共享引导模态框内以仅凭据模式渲染既有 `ProviderEditor`；`credentials.set` 仍是唯一的机密写入，且不改变任何提供方设置。
