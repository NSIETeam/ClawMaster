...
`DockSurface` 默认只渲染当前标签的正文。启用 `keepVisitedTabsMounted` 后，正文在首次访问时挂载，并在标签仍属于同一个已挂载面板期间保持挂载；未激活的正文会隐藏、禁用交互并失去焦点，保留编辑器的本地状态。未访问的标签不会挂载。关闭标签、将标签移出面板、关闭保留选项或卸载整个区域时，对应正文会释放。状态只保留在内存中，不跨应用重启保存。
