...
| [`src/windows-inspector.ts`](src/windows-inspector.ts) | 经 koffi 的 Windows Toolhelp32 进程表与常驻工作集检查 |
...
- **Windows 进程树 RSS 使用原生接口并在失败时收紧**——观察器把 Toolhelp32 父子链接与 PSAPI `GetProcessMemoryInfo` 工作集结合起来。受保护或正在退出的成员会让整棵树不可观测，因此运行时治理继续使用 Host RSS 检查，不编造较低的进程树值。
