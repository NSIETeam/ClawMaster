...
| [`src/windows-inspector.ts`](src/windows-inspector.ts) | Windows Toolhelp32 process-table and resident working-set inspection via koffi |
...
- **Windows process-tree RSS is native and fail-closed** — the observer combines Toolhelp32 parent links with PSAPI `GetProcessMemoryInfo` working sets. A protected or exiting member makes the whole tree observation unavailable, so runtime governance keeps the Host RSS check and does not invent a lower process-tree value.
