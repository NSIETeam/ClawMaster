...
- **单一 rejection 检查点。** `assertEntriesActivated` 会在激活诊断中写入完整 Loader 条目 ID（包括嵌套条目）与 pending 服务，并将确切原因保持到下一个进程级 rejection 检查点可见。`installFailLoud` 会合并 Loader 的重复通知，而所有无关的未处理 rejection 仍然致命。
