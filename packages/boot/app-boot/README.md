...
- **One rejection checkpoint.** `assertEntriesActivated` includes the full Loader entry id, including nested entries, and pending services in activation diagnostics, and keeps the exact reasons visible through the next process rejection checkpoint. `installFailLoud` coalesces Loader's duplicate notification while unrelated unhandled rejections remain fatal.
