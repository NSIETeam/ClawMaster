...
export function enterpriseE2eeDeviceVerification(
  ...
  return {
    ...
    qrPayload: `clawmaster-e2ee-verify:v1:${Buffer.from(canonical).toString('base64url')}`,
    deviceFingerprints: [
...
export function enterpriseFederationContactVerification(
  ...
  return {
    safetyNumber: groups.join(' '),
    qrPayload: `clawmaster-federation-verify:v2:${Buffer.from(canonical).toString('base64url')}`,
    deviceFingerprints: [
      identities[0]!.keyFingerprint,
      identities[1]!.keyFingerprint,
    ],
...
export class EnterpriseE2eeKeyVault {
  ...
  exportRecoveryBundle(
    ...
    cipher.setAAD(Buffer.from('clawmaster-e2ee-recovery-v1', 'utf8'))
  ...
  importRecoveryBundle(
    ...
    try {
      ...
      decipher.setAAD(Buffer.from('clawmaster-e2ee-recovery-v1', 'utf8'))
...
function deriveEnvelopeKey(
  ...
  return Buffer.from(
    hkdfSync(
      ...
      Buffer.from(`clawmaster-e2ee-envelope-key-v1:${accountId}:${deviceId}`, 'utf8'),
      32,
    ),
  )
}
