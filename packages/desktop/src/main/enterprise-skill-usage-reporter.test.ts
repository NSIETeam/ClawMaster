...
async function fixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'clawmaster-skill-market-usage-'));
  roots.push(root);
  const skills = path.join(root, 'skills');
  const marketSkill = path.join(skills, 'market-skill-1');
  await fs.mkdir(marketSkill, { recursive: true });
  await fs.writeFile(path.join(marketSkill, '.clawmaster-market.json'), JSON.stringify({
    skillId: 'skill-1',
