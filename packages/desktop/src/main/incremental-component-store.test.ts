...
async function tempRoot(): Promise<{ root: string; userDir: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'clawmaster-component-update-'))
  const userDir = path.join(dir, 'clawmaster-user')
...
function artifact(body: string, overrides: Partial<IncrementalUpdateArtifact> = {}): IncrementalUpdateArtifact {
  return {
    ...
    url: 'https://updates.example.com/clawmaster/component-skills-ppt-v2.bundle.json',
