...
describe('enterprise private-chat E2EE', () => {
  ...
  it('pins transparency heads and rejects a server rollback or fork', () => {
    const alice = createEndpoint('Alice laptop');
    const aliceDevice = alice.crypto.localDevice('https://clawmaster.test', 'alice');
    const second = createEndpoint('Alice phone').crypto.localDevice(
      'https://clawmaster.test',
      'alice',
    );
    ...
    expect(
      alice.crypto.verifyAndPinKeyTransparency({
        serverScope: 'https://clawmaster.test',
    ...
    const checkpointFiles = readFileSync(
      join(
        alice.root,
        `${createHash('sha256')
          .update('https://clawmaster.test\0org-a\0alice')
  ...
  it('rejects malformed transparency entries and inconsistent device directories', () => {
    ...
    const bob = createEndpoint('Bob laptop');
    const aliceDevice = alice.crypto.localDevice('https://clawmaster.test', 'alice');
    const bobDevice = bob.crypto.localDevice('https://clawmaster.test', 'bob');
  ...
  it('derives symmetric safety numbers and signs out-of-band device approvals locally', () => {
    ...
    const first = aliceOne.crypto.localDevice('https://clawmaster.test', 'alice');
    const second = aliceTwo.crypto.localDevice('https://clawmaster.test', 'alice');
    ...
    expect(forward.qrPayload).toMatch(/^clawmaster-e2ee-verify:v1:/);
  ...
  it('covers every active device and stops targeting a revoked device', () => {
    ...
    const devices = [
      alice.crypto.localDevice('https://clawmaster.test', 'alice'),
      bobOne.crypto.localDevice('https://clawmaster.test', 'bob'),
      bobTwo.crypto.localDevice('https://clawmaster.test', 'bob'),
    ];
  ...
  it('imports a passphrase recovery bundle as historical keys on a new device', () => {
    ...
    const alice = createEndpoint('Alice');
    const aliceDevice = alice.crypto.localDevice('https://clawmaster.test', 'alice');
    const oldBobDevice = oldBob.crypto.localDevice('https://clawmaster.test', 'bob');
    const encrypted = alice.crypto.encryptMessage({
      serverScope: 'https://clawmaster.test',
      organizationId: 'org-a',
      senderAccountId: 'alice',
    ...
    expect(newBob.crypto.localDevice('https://clawmaster.test', 'bob').deviceId).toBe(
  ...
  it('keeps raw private keys out of the vault file', () => {
    ...
    const device = endpoint.crypto.localDevice('https://clawmaster.test', 'alice');
    const files = readFileSync(
      join(
        endpoint.root,
        `${createHash('sha256').update('https://clawmaster.test\0alice').digest('hex')}.keyring`,
      ),
      'utf8',
    );
