...
export async function exportCustomerModuleData(root: string, moduleId: string): Promise<{
  format: 'clawmaster.customer-module-data.v1';
  moduleId: string;
  exportedAt: string;
  ...
  return { format: 'clawmaster.customer-module-data.v1', moduleId, exportedAt: new Date().toISOString(), values };
}
...
export async function installCustomerModule(input: {
  ...
  if (!isVersionCompatible(input.clawmasterVersion, manifest.minimumClawMasterVersion)) throw new Error(`客户模块要求 ClawMaster ${manifest.minimumClawMasterVersion} 或更高版本`);
