...
export interface EnterpriseOrganizationView {
  ...
  members: Array<{
    ...
    clawmasterLastSeenAt?: string | null
  }>
...
export class EnterpriseClient {
    ...
    try {
      const headers: Record<string, string> = {
        ...
        ...((init.method ?? 'GET').toUpperCase() === 'GET'
          ? {}
          : { 'x-clawmaster-idempotency-key': `desktop:${randomUUID()}` }),
  ...
  async sendFederationMessage(
    ...
    const temporaryDirectory = await fs.promises.mkdtemp(
      path.join(os.tmpdir(), 'clawmaster-federation-attachment-'),
    )
    try {
  ...
  async saveFederationMessageAttachment(input: {
    ...
    const temporaryDirectory = await fs.promises.mkdtemp(
      path.join(os.tmpdir(), 'clawmaster-federation-download-'),
    )
    try {
