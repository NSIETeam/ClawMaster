...
async function tempRoot(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'clawmaster-patch-update-'));
...
function artifact(body: string, overrides: Partial<IncrementalUpdateArtifact> = {}): IncrementalUpdateArtifact {
  return {
    ...
    url: 'https://updates.example.com/clawmaster/patch-renderer-css-enterprise-login.bundle.json',
