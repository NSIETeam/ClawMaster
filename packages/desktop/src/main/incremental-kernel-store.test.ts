...
async function tempRoot(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'clawmaster-kernel-update-'))
...
function artifact(body: string, overrides: Partial<IncrementalUpdateArtifact> = {}): IncrementalUpdateArtifact {
  return {
    ...
    url: 'https://updates.example.com/clawmaster/kernel-server-runtime-2026-07.bundle.json',
