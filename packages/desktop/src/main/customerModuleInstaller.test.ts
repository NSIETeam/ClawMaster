...
function packageFixture(permissions: CustomerModulePermission[] = [{ kind: 'model', paid: true }]) {
  ...
  const unsigned = {
    ...
    icon: 'icon.svg', entrypoint: 'module.wasm', hostApi: 'clawmaster.customer-module.v1' as const,
...
describe('customer module installer', () => {
  const clawmasterVersion = '1.15.3'
  ...
  it('fail-closes an installed version after marketplace suspension', async () => {
    ...
    const bundle = packageFixture()
    const client = { downloadCustomerModulePackage: vi.fn().mockResolvedValue(bundle), recordCustomerModuleInstall: vi.fn() }
    await installCustomerModule({ root, client: client as never, moduleId: bundle.manifest.id, version: bundle.manifest.version, approvedPermissions: bundle.manifest.permissions, clawmasterVersion })
  ...
  it('keeps declared background capability off until a separate explicit authorization', async () => {
    ...
    const client = { downloadCustomerModulePackage: vi.fn().mockResolvedValue(bundle), recordCustomerModuleInstall: vi.fn() }
    const installed = await installCustomerModule({ root, client: client as never, moduleId: bundle.manifest.id, version: bundle.manifest.version, approvedPermissions: bundle.manifest.permissions, clawmasterVersion })
