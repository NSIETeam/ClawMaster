...
describe('EnterpriseClient', () => {
  ...
  it('wires the inactive MLS ciphertext transport without enabling MLS chat', async () => {
    ...
    expect(fetchMock.mock.calls.slice(2).map(([url]) => url)).toEqual([
      'https://enterprise.clawmaster.test/enterprise/e2ee/mls/key-packages/inventory?deviceId=device-1',
      `https://enterprise.clawmaster.test/enterprise/e2ee/mls/key-packages/${keyPackageReference}?deviceId=device-1`,
      'https://enterprise.clawmaster.test/enterprise/e2ee/mls/key-packages',
      'https://enterprise.clawmaster.test/enterprise/e2ee/mls/key-packages/claim',
      'https://enterprise.clawmaster.test/enterprise/e2ee/mls/conversations/acc_peer/events',
      'https://enterprise.clawmaster.test/enterprise/e2ee/mls/conversations/acc_peer/events?afterSequence=0&limit=25',
      'https://enterprise.clawmaster.test/enterprise/e2ee/mls/inbound-conversations?deviceId=device-1&limit=500',
    ]);
  ...
  it('保留 HTTPS 部署路径前缀，并在前缀下请求全部企业接口', async () => {
    ...
    await client.loginWithPassword(
      'https://enterprise.clawmaster.test/company/',
      'staff01',
      'password',
    );

    expect(client.snapshot().serverUrl).toBe('https://enterprise.clawmaster.test/company');
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
  ...
  it('首次注册先请求挑战，再提交姓名、密码和验证码并保存会话', async () => {
    ...
    expect(fetchMock.mock.calls.map(([url, init]) => [url, (init as RequestInit).method])).toEqual([
      ['https://enterprise.clawmaster.test/enterprise/health', 'GET'],
      ['https://enterprise.clawmaster.test/enterprise/auth/register/sms/request', 'POST'],
      ['https://enterprise.clawmaster.test/enterprise/auth/register/sms/verify', 'POST'],
    ]);
  ...
  it('个人账号登录后用 Bearer 会话提交邀请码，并用服务端返回值刷新当前身份', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0])
      .toBe('https://enterprise.clawmaster.test/enterprise/auth/join-organization');
  ...
  it('uses the member session to read enterprise module update manifests', async () => {
    const manifest = {
      ...
      modules: [{
        ...
        notes: 'park update',
        minAppVersion: '1.9.5',
        manifestUrl: 'https://updates.example.com/clawmaster-incremental.json',
        sha256: 'a'.repeat(64),
    ...
    const client = new EnterpriseClient(fetchMock as typeof fetch);
    await client.loginWithPassword('https://enterprise.clawmaster.test', 'staff01', 'password');
  ...
  it('uses the member session to resolve a distribution-bound update policy', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0])
      .toBe('https://enterprise.clawmaster.test/enterprise/deployment/update-policy');
    const request = fetchMock.mock.calls[2]?.[1] as RequestInit;
  ...
  it('支持手机号验证码登录并保存会话', async () => {
    ...
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
      'https://enterprise.clawmaster.test/enterprise/health',
  ...
  it('登录后按消息幂等键上报 provider 返回的 Token 用量', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0]).toBe('https://enterprise.clawmaster.test/enterprise/usage');
  ...
  it('未登录时不发送 Token 用量', async () => {
    const fetchMock = vi.fn();
    const client = new EnterpriseClient(fetchMock as typeof fetch);
    client.restore({ serverUrl: 'https://enterprise.clawmaster.test', token: null });
  ...
  it('读取 Token 画像时在客户端收敛统计周期', async () => {
    const fetchMock = vi.fn();
    const client = new EnterpriseClient(fetchMock as typeof fetch);
    client.restore({ serverUrl: 'https://enterprise.clawmaster.test', token: 'token' });
  ...
  it('登录后把自动提炼的知识条目写入组织知识库', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0]).toBe('https://enterprise.clawmaster.test/enterprise/knowledge');
    const init = fetchMock.mock.calls[2]?.[1] as RequestInit;
    expect(init.method).toBe('POST');
    expect(init.headers).toMatchObject({ authorization: 'Bearer session-token' });
    expect(JSON.parse(init.body as string)).toEqual({
  ...
  it('登录后从组织知识库读取企业记忆并映射字段', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0]).toBe('https://enterprise.clawmaster.test/enterprise/knowledge?q=%E5%90%88%E5%90%8C&department=%E7%A0%94%E5%8F%91%E9%83%A8');
    const init = fetchMock.mock.calls[2]?.[1] as RequestInit;
  ...
  it('企业管理员可修订知识并读取版本历史', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0])
      .toBe('https://enterprise.clawmaster.test/enterprise/knowledge/12');
  ...
  it('登录成员通过 main 内的会话令牌读取完整组织架构', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0])
      .toBe('https://enterprise.clawmaster.test/enterprise/organization/view');
    expect((fetchMock.mock.calls[2]?.[1] as RequestInit).headers).toMatchObject({
      authorization: 'Bearer session-token',
    });
  });
  ...
  it('企业在线心跳使用成员会话并要求服务端支持 presence capability', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0])
      .toBe('https://enterprise.clawmaster.test/enterprise/presence/heartbeat');
  ...
  it('登录成员使用会话令牌读取 A2A 待处理请求', async () => {
    ...
    expect(fetchMock.mock.calls[2]?.[0])
      .toBe('https://enterprise.clawmaster.test/enterprise/atoa/inbox');
    expect((fetchMock.mock.calls[2]?.[1] as RequestInit).headers).toMatchObject({
      authorization: 'Bearer session-token',
    });
  });
  ...
  it('企业管理员可读取并手动换新 7 天中心引入链接', async () => {
    ...
    const client = new EnterpriseClient(fetchMock as typeof fetch);
    await client.loginWithPassword('https://enterprise.clawmaster.test', 'admin', 'password');
  ...
  it('服务端 401 时清除已恢复的失效会话', async () => {
    ...
    client.restore({ serverUrl: 'https://clawmaster.example.com', token: 'expired-token' });
  ...
  it('恢复会话遇到断网时保留服务器地址和 token，返回可重试的连接错误', async () => {
    ...
    client.restore({ serverUrl: 'https://enterprise.clawmaster.test', token: 'restored-token' });
  ...
  it('园区视图读取直接请求业务接口，不因旧 health capabilities 缺 park_membership_v1 隐藏入口', async () => {
    ...
    const client = new EnterpriseClient(fetchMock as typeof fetch);
    client.restore({ serverUrl: 'https://enterprise.clawmaster.test', token: 'session-token' });

    await expect(client.getParkView()).resolves.toEqual(park);

    expect(fetchMock).toHaveBeenCalledOnce();
    expect(fetchMock.mock.calls[0]?.[0]).toBe('https://enterprise.clawmaster.test/enterprise/park/view');
  ...
  it('同一服务器复用成功握手，切换服务器地址后重新验证', async () => {
    ...
    await client.loginWithPassword('https://a.clawmaster.test', 'staff01', 'password');
    await client.loginWithPassword('https://a.clawmaster.test/', 'staff01', 'password');
    await client.loginWithPassword('https://b.clawmaster.test', 'staff01', 'password');

    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
      'https://a.clawmaster.test/enterprise/health',
      'https://a.clawmaster.test/enterprise/auth/login',
      'https://a.clawmaster.test/enterprise/auth/login',
      'https://b.clawmaster.test/enterprise/health',
      'https://b.clawmaster.test/enterprise/auth/login',
    ]);
    expect(client.snapshot()).toEqual({
  ...
  it('切换服务器时取消旧健康检查，绝不把旧登录凭据发往新服务器', async () => {
    ...
    const fetchMock = vi.fn((input: string | URL | Request) => {
      const url = String(input);
      if (url === 'https://a.clawmaster.test/enterprise/health') return firstHealth.promise;
      if (url === 'https://b.clawmaster.test/enterprise/health') {
        return Promise.resolve(jsonResponse(200, API_V2_HEALTH));
      }
      if (url === 'https://b.clawmaster.test/enterprise/auth/login') {
    ...
    await expect(client.loginWithPassword('https://b.clawmaster.test', 'staff-b', 'password-b'))
    ...
    expect(fetchMock.mock.calls.map(([url]) => String(url))).not
      .toContain('https://a.clawmaster.test/enterprise/auth/login');
    expect(client.snapshot()).toEqual({
      serverUrl: 'https://b.clawmaster.test',
      token: 'server-b-token',
    });
  ...
  it('旧登录响应晚到时不能覆盖较新的服务器 token 和账号', async () => {
    ...
    const fetchMock = vi.fn((input: string | URL | Request) => {
      ...
      if (url === 'https://a.clawmaster.test/enterprise/auth/login') return firstLogin.promise;
      if (url === 'https://b.clawmaster.test/enterprise/auth/login') {
        return Promise.resolve(jsonResponse(200, {
    ...
    await vi.waitFor(() => expect(fetchMock.mock.calls.some(
      ([url]) => String(url) === 'https://a.clawmaster.test/enterprise/auth/login',
    )).toBe(true));
    await client.loginWithPassword('https://b.clawmaster.test', 'staff-b', 'password-b');
  ...
  it('旧注册响应晚到时不能覆盖更新的登录会话', async () => {
    ...
    const fetchMock = vi.fn((input: string | URL | Request) => {
      ...
      if (url === 'https://a.clawmaster.test/enterprise/auth/register/sms/request') {
        return Promise.resolve(jsonResponse(200, {
          challengeId: 'sms_a',
          expiresAt: '2099-01-01',
          retryAfterSeconds: 60,
      ...
      if (url === 'https://a.clawmaster.test/enterprise/auth/register/sms/verify') {
    ...
    await vi.waitFor(() => expect(fetchMock.mock.calls.some(
      ([url]) => String(url) === 'https://a.clawmaster.test/enterprise/auth/register/sms/verify',
    )).toBe(true));
    await client.loginWithPassword('https://b.clawmaster.test', 'staff-b', 'password-b');
  ...
  it('旧恢复请求返回 401 时不能清除后来登录的新会话', async () => {
    ...
    const fetchMock = vi.fn((input: string | URL | Request) => {
      ...
      if (url === 'https://a.clawmaster.test/enterprise/auth/me') return staleSession.promise;
      if (url === 'https://b.clawmaster.test/enterprise/auth/login') {
        return Promise.resolve(jsonResponse(200, {
          account: accountB, token: 'server-b-token', expiresAt: '2099-01-01',
        }));
    ...
    await vi.waitFor(() => expect(fetchMock.mock.calls.some(
      ([url]) => String(url) === 'https://a.clawmaster.test/enterprise/auth/me',
    )).toBe(true));
    await client.loginWithPassword('https://b.clawmaster.test', 'staff-b', 'password-b');
  ...
  it('通过受版本保护的企业 Skill 市场协议查询、投稿、安装、评分和读取榜单', async () => {
    ...
    expect(fetchMock.mock.calls.slice(2).map(([url]) => url)).toEqual([
      'https://enterprise.clawmaster.test/enterprise/skills?scope=department&q=%E6%9C%88%E6%8A%A5&sort=rating',
      'https://enterprise.clawmaster.test/enterprise/skills',
      'https://enterprise.clawmaster.test/enterprise/skills/skill-1/install',
      'https://enterprise.clawmaster.test/enterprise/skills/skill-1/rating',
      'https://enterprise.clawmaster.test/enterprise/skills/skill-1/usage',
      'https://enterprise.clawmaster.test/enterprise/skills/leaderboard',
    ]);
  ...
  it('uploads E2EE ciphertext before sending PostgreSQL attachment references', async () => {
    ...
    expect(fetchMock.mock.calls[6]?.[0]).toBe(
      'https://enterprise.clawmaster.test/enterprise/attachments/inline',
    );
  ...
  it('uploads an MLS attachment through resumable presigned multipart requests', async () => {
    ...
    try {
      ...
      expect(fetchMock.mock.calls[3]?.[0]).toBe(
        'https://enterprise.clawmaster.test/enterprise/attachments/uploads',
      );
  ...
  it('streams and verifies an MLS object download before local decryption', async () => {
    const directory = fs.mkdtempSync(
      path.join(os.tmpdir(), 'clawmaster-mls-download-'),
    );
    const ciphertextPath = path.join(directory, 'ciphertext.bin');
      ...
      .mockResolvedValueOnce(jsonResponse(200, {
        attachment: {
          ...
          attachment: {
            ...
            download: {
              method: 'GET',
              url: 'https://objects.clawmaster.test/signed-object',
