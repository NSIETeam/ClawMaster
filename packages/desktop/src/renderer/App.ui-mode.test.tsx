...
vi.mock('./state/useClawMasterStore.js', () => ({
  useClawMasterStore: () => ({
    state: {
...
vi.mock('./components/ParkServicesPlugin.js', () => ({
  ...
  PARK_STATE_EVENT: 'clawmaster:park-services-state',
...
describe('App UI mode integration', () => {
  it('keeps the workspace mounted when an older host returns null for module records', async () => {
    ...
    expect(document.querySelector('.claw-app')).toBeTruthy();
  });
  ...
  it('starts first-time users in conversational mode and persists their work UI choice', () => {
    ...
    const app = document.querySelector<HTMLElement>('.claw-app');
  ...
  it('does not reuse one account UI preference for another account', () => {
    localStorage.setItem(preferenceKey(accountA), 'work');
    const view = render(<App />);
    expect(document.querySelector<HTMLElement>('.claw-app')?.dataset.uiMode).toBe('work');
    ...
    expect(document.querySelector<HTMLElement>('.claw-app')?.dataset.uiMode).toBe('conversational');
    expect(screen.getByTestId('ui-mode-guide')).toBeTruthy();
