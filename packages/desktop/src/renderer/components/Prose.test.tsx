...
describe('Prose 轻量 Markdown', () => {
  ...
  it('识别 Windows、macOS 和 Linux 的绝对输出路径，不误判普通命令', () => {
    ...
    expect(normalizeLocalOutputPath('/Users/clawmaster/Desktop/report.pdf')).toBe(
      '/Users/clawmaster/Desktop/report.pdf',
    );
    expect(normalizeLocalOutputPath('/home/clawmaster/report.xlsx')).toBe(
      '/home/clawmaster/report.xlsx',
    );
  ...
  // 每个用例后清掉注入到 window 的 clawmaster 桩，避免相互污染。
  afterEach(() => {
  ...
  it('[文本](url) → <a>，显示文本、href 指向 url', () => {
    const { container } = render(
      <Prose text={'见 [ClawMaster 仓库](https://github.com/Felix201209/clawmaster) 了解'} />,
    );
    const a = container.querySelector('a.claw-prose__link');
    expect(a?.textContent).toBe('ClawMaster 仓库');
    expect(a?.getAttribute('href')).toBe('https://github.com/Felix201209/clawmaster');
    // 链接前后正文保留。
