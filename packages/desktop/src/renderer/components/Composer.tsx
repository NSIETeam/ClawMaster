...
export function Composer({
  ...
  const [globalAuto, setGlobalAuto] = useState(
    () => localStorage.getItem('clawmaster.authorization.global-auto') === '1',
  )
  ...
  const pickAuthorization = (kind: 'manual' | 'session' | 'global'): void => {
    if (!sessionId) return
    if (kind === 'global') {
      localStorage.setItem('clawmaster.authorization.global-auto', '1')
      setGlobalAuto(true)
...
function ModelMenuPopover({
  ...
  const updatePosition = useCallback(() => {
    ...
    const below = anchorRect.bclawmasterm + gap
