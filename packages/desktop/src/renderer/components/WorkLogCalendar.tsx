...
import type { WorkLogDisplayEntry } from 'clawmaster-server';
...
export function WorkLogCalendar({
  ...
  return (
    <div className="claw-wcal">
      <div className="claw-wcal__title">
      ...
      <div className="claw-wcal__grid">
        {['一', '二', '三', '四', '五', '六', '日'].map((weekday) => <div key={weekday} className="claw-wcal__weekday">{weekday}</div>)}
          ...
          return (
            <button
              ...
              title={entries.length
                ...
                : '点击查看/新增当日日程'}
            >
              {day}{entries.length ? <span className="claw-wcal__dot" /> : null}
              {entries.length ? (
                <span className="claw-wcal__pop" role="tooltip">
                  <span className="claw-wcal__pop-title">
                    {month + 1} 月 {day} 日 · {entries.length} 条
                  </span>
                  {orderedEntries.slice(0, 12).map((entry, entryIndex) => (
                    <span className="claw-wcal__pop-item" key={`${entry.time}-${entryIndex}`}>
                      <span className="claw-wcal__pop-time">{entry.time}</span>
                      <span className="claw-wcal__pop-copy">
                        <span className="claw-wcal__pop-action">
                          • {entry.entryType === 'work_result' ? '完成' : entry.category || '操作'} · {entry.action}
                          {entry.success === false ? '（失败）' : ''}
                        </span>
                        {entry.details ? (
                          <span className="claw-wcal__pop-detail">
                  ...
                  {entries.length > 12 ? (
                    <span className="claw-wcal__pop-more">…还有 {entries.length - 12} 条</span>
                  ) : null}
                </span>
              ) : null}
            </button>
