...
export function PlatformWorkspace({
  ...
  const insecure = (() => {
    ...
    try { return !isSecurePlatformUrl(new URL(target.url)); } catch { return true; }
  })();

  useLayoutEffect(() => {
    const host = browserHostRef.current;
    const url = target.url;
    if (!host || !url || insecure) return undefined;
  ...
  return (
    <section className="claw-platform-workspace" aria-label={`${target.label}平台工作区`}>
      ...
      {!target.url || insecure ? <div className="claw-platform-workspace__config" role="group" aria-label="平台地址配置">
        ...
        <div role={validationError ? 'alert' : 'status'}>{validationError ?? 'ClawMaster 不会内置或猜测生产地址，也不会要求在对话中发送凭据。'}</div>
      </div> : null}
      {insecure ? <div className="claw-platform-workspace__warning" role="alert">已阻止打开该平台：远程 HTTP 会暴露登录和业务数据，请先部署 HTTPS。</div> : null}
      {target.url && !insecure ? <div ref={browserHostRef} className="claw-platform-workspace__browser" aria-label={`${target.label}内置浏览器`}>
