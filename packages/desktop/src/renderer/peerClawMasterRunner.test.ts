...
import {
  ...
  buildPeerClawMasterPrompt,
  normalizePeerClawMasterQuestion,
  type PeerClawMasterTransport,
} from './peerClawMasterRunner.js';

describe('真实企业 A2A 本机 Agent 执行器', () => {
  it('把员工问题和明确限定上下文交给真实 A2A Agent，而不是拼固定回复', () => {
    const prompt = buildPeerClawMasterPrompt('今天 15:00 能评审吗？', '今天有 3 项工作，15:00 有项目复盘。');
  ...
  it('双方 ClawMaster 协商时明确比较发起方提案与接收方授权资料', () => {
    const prompt = buildPeerClawMasterPrompt(
      '为项目评审协商时间和合作计划',
      '日程：16:00 后有空。',
      {
        mode: 'consult',
  ...
  it('发起协商时由自己的 ClawMaster 先基于本方授权资料形成提案', () => {
    const prompt = buildPeerClawMasterPrompt(
      '协商项目评审时间',
  ...
  it('确认框与模型只使用同一份规范化问题，不发送未展示尾部', () => {
    ...
    const normalized = normalizePeerClawMasterQuestion(`  ${visible}未展示尾部  `);
    const prompt = buildPeerClawMasterPrompt(normalized, '无私有上下文');
  ...
  it('创建隔离的 tool-free A2A 会话，等待服务端完成并自动回收后返回答案', async () => {
    ...
    const transport: PeerClawMasterTransport = {
      isConnected: () => true,
      onFrame: (next) => {
        handler = next;
        return vi.fn();
          ...
          handler?.({
            ...
            payload: {
              ...
              session: {
                ...
                status: 'idle', agentProfileId: 'claw-enterprise-a2a',
    ...
    await expect(askLocalPeerClawMaster({
      question: '15:00 能评审吗？',
  ...
  it('本机 ClawMaster 未连接时明确失败，不发送伪造回答', async () => {
    const transport: PeerClawMasterTransport = {
      isConnected: () => false,
      onFrame: vi.fn(() => vi.fn()),
      send: vi.fn(),
  ...
  it('登出或切换账号时取消正在运行的会话', async () => {
    ...
    const pending = askLocalPeerClawMaster({
