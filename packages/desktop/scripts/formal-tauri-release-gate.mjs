...
export function evaluateFormalTauriReleaseGate({
  ...
  if (asset.kind === 'unsupported') {
    note(
      `no local download artifact required for unsupported platform ${platform}-${arch}`,
    );
  } else if (
...
if (path.resolve(process.argv[1] ?? '') === fileURLToPath(import.meta.url)) {
  ...
  for (const message of result.notes)
    console.log(`[formal-tauri-release-gate] note: ${message}`);
}
