...
describe('portable USB distribution', () => {
  it('assembles the green runtime and redirects every user directory', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'clawmaster-portable-'));
    ...
    try {
      ...
      expect(launcher).toContain('set "CLAWMASTER_USER_DIR=%~dp0clawmaster-data"');
      expect(launcher).toContain('set "CLAWMASTER_USER_DATA_DIR=%~dp0clawmaster-data\\electron"');
      expect(launcher).toContain('set "USERPROFILE=%~dp0clawmaster-home"');
