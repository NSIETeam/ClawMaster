...
describe('desktop packaging contract', () => {
  ...
  it('keeps package managers and development tools out of user installers', async () => {
    ...
    expect(verifier).toContain('@clawmaster\\/native\\/target');
  ...
  it('keeps update manifest download URLs bound to the no-proxy update mirror', async () => {
    ...
    expect(script).not.toContain(
      'github.com/Felix201209/clawmaster-releases/releases/download',
    );
  });
  ...
  it('provides a reproducible Windows x64 Tauri release workflow', async () => {
    ...
    expect(workflow).toContain('verify-tauri-node-runtime.mjs');
    expect(workflow).toContain('verify-tauri-sqlcipher-asset.mjs');
    expect(workflow).not.toContain('npm run doctor');
  ...
  it('discovers every packaged LibreOffice bundle before signing ClawMaster', async () => {
    const appPath = await mkdtemp(path.join(os.tmpdir(), 'clawmaster-after-pack-'));
    try {
