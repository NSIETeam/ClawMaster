...
describe('product asset budget', () => {
  it('keeps the retired animated pet atlas out of the lean renderer', () => {
    const atlas = path.join(assetsRoot, 'clawmaster-pet-atlas.png');
    ...
    expect(component).not.toContain("../assets/clawmaster-pet-atlas.png");
  ...
  it('keeps the shared avatar vector-only in the lean renderer', () => {
    expect(existsSync(path.join(assetsRoot, 'clawmaster-avatar.png'))).toBe(false);
  ...
  it('does not retain the unreferenced legacy avatar source set', () => {
    ...
    expect(releaseGate).not.toContain('clawmaster-avatar.png');
    expect(releaseGate).not.toContain("'clawmaster-avatar-1.png'");
  });
});
