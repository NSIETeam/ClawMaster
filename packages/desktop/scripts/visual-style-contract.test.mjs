...
describe('desktop visual style contract', () => {
  ...
  it('keeps right-rail module names readable instead of squeezing three cards across', async () => {
    ...
    expect(css).toMatch(/\.otto-module-workspace--panel \.otto-module-group__grid\s*\{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/su);
    expect(css).toMatch(/\.otto-module-workspace--panel \.otto-module-group__grid--rows-4\s*\{[^}]*max-height: 306px;/su);
    expect(css).toMatch(/\.otto-module-tile > span:last-child\s*\{[^}]*overflow-wrap: anywhere;[^}]*word-break: normal;/su);
    expect(css).not.toMatch(/\.otto-module-tile > span:last-child\s*\{[^}]*word-break: break-word;/su);
  });
  ...
  it('keeps marketplace controls readable while only the catalog scrolls at minimum height', async () => {
    ...
    expect(css).toMatch(/\.otto-module-marketplace__header,\s*\.otto-module-marketplace__footer\s*\{[^}]*flex: 0 0 auto;/su);
    expect(css).toMatch(/\.otto-module-marketplace__body\s*\{[^}]*min-height: 0;[^}]*display: flex;[^}]*overflow-y: hidden;/su);
    expect(css).toMatch(/\.otto-module-marketplace__catalog\s*\{[^}]*min-height: 0;[^}]*overflow-y: auto;/su);
    expect(css).toMatch(/\.otto-module-marketplace__tabs button,\s*\.otto-module-marketplace__filters button\s*\{[^}]*display: inline-flex;[^}]*min-height: 34px;[^}]*padding: 7px 13px;[^}]*line-height: 1\.2;/su);
