...
describe('ClawMaster / Otto Green distribution isolation (NSI-07)', () => {
  it('resolves ClawMaster by default and Green via OTTO_GREEN=1', () => {
  ...
  it('produces distinct artifact names', () => {
    const o = OTTO_IDENTITY;
    const g = OTTO_GREEN_IDENTITY;
    expect(winArtifactName('1.2.3', 'x64', o)).toBe(
      'ClawMaster-Setup-1.2.3-win-x64.exe',
    );
    expect(winArtifactName('1.2.3', 'x64', g)).toBe(
      'OttoGreen-Setup-1.2.3-win-x64.exe',
    );
    expect(macArtifactName('1.2.3', 'arm64', o)).toBe(
      'ClawMaster-1.2.3-arm64.dmg',
    );
    expect(macArtifactName('1.2.3', 'arm64', g)).toBe('OttoGreen-1.2.3-arm64.dmg');
  });

  it('Green blockmap stays distinct from ClawMaster', () => {
    const o = OTTO_IDENTITY;
    const g = OTTO_GREEN_IDENTITY;
