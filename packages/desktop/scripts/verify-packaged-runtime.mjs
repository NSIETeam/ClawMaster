...
const FORBIDDEN_RUNTIME_ENTRY_PATTERNS = [
  /^\/node_modules\/\.bin(?:\/|$)/u,
  /^\/node_modules\/@clawmaster\/native\/target(?:\/|$)/u,
  /^\/node_modules\/@clawmaster\/native\/src(?:\/|$)/u,
  /^\/node_modules\/@clawmaster\/native\/Cargo\.(?:toml|lock)$/u,
  /^\/node_modules\/@clawmaster\/native\/tsconfig\.json$/u,
...
export function verifyPackagedRuntime(
  ...
  if (
    ...
    property('clawmaster:target') !== manifest.target ||
    property('clawmaster:buildCommit') !== manifest.buildCommit ||
    property('clawmaster:sourceRevision') !== manifest.sourceRevision
  ) {
