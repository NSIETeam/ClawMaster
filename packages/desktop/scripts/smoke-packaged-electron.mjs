 ...
 *   2. preload 暴露真实 window.clawmaster；
...
function assertSmokeResult(result) {
  ...
  if (!result?.hasBridge) failures.push('window.clawmaster 未注入');
...
const tempRoot = mkdtempSync(resolve(tmpdir(), 'clawmaster-packaged-smoke-'));
const mountPoint = resolve(tempRoot, 'mount');
const homeDir = resolve(tempRoot, 'home');
const userDataDir = resolve(tempRoot, 'user-data');
const clawmasterUserDir = resolve(tempRoot, 'clawmaster-user');
mkdirSync(mountPoint);
mkdirSync(homeDir);
mkdirSync(userDataDir);
mkdirSync(clawmasterUserDir);
  ...
  appProcess = spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        CLAWMASTER_USER_DIR: clawmasterUserDir,
