...
const BUILD_PROVENANCE_PATH = path.join(
  RELEASE_DIR,
  '.clawmaster-build-provenance.json',
);
...
const SOURCE_REPO = 'NSIETeam/clawmaster-new';
const SOURCE_UPSTREAM = 'origin/internal';
const RELEASES_REPO = process.env.CLAWMASTER_RELEASES_REPO || 'NSIETeam/clawmaster-new';
...
async function main() {
  console.log('');
  log('CLAWMASTER', `ClawMaster Desktop v${VERSION} 构建发布工具`);
  log('CLAWMASTER', `工作目录: ${DESKTOP_DIR}`);
  console.log('');
