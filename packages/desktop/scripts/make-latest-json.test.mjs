...
async function fixtureDir() {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'clawmaster-update-manifest-'));
  tempDirs.push(dir);
  const version = '9.8.7';
  const files = [
...
describe('make-latest-json', () => {
  ...
  it('supports a private deployment mirror override', async () => {
    ...
    execFileSync(process.execPath, [scriptPath, version, notes, dir], {
      env: {
        ...process.env,
        CLAWMASTER_UPDATE_ASSET_BASE_URL: 'https://updates.example.com/clawmaster/',
      },
      stdio: 'pipe',
    });
    const manifest = JSON.parse(
    ...
    for (const asset of Object.values(manifest.assets)) {
      expect(asset.url).toBe(`https://updates.example.com/clawmaster/${asset.name}`);
    }
  });
