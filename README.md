...
- **原生桌面 RPA**：经确认后先通过 Windows UI Automation 或 macOS Accessibility 读取角色、标签、状态、动作和坐标组成的有界文本树，再通过 Rust 执行真实鼠标键盘操作；截图视觉仅作为语义树无法定位时的后备路径。
...
- **右侧通用工作区**：内置浏览器、思维导图阅览器和文件工作区；文本、代码与 Markdown 可另存新稿，DOCX/PPTX 可按段落另存原格式副本，PDF/XLSX 提供只读预览。
- **电商市场分析**：内置可追溯的分析工作区，覆盖客户研究、竞品矩阵、渠道与价格、机会缺口、风险及 30/60/90 天行动计划。
- **企业协作入口**：飞书、钉钉和企业微信智能机器人支持 Rust 原生双向消息链路、自动重连与系统钥匙串凭据存储；企业微信自建应用模式继续提供主动通知。
- **个人版企业能力**：个人版可使用本地企业级工具和自有凭证，不设置版本级工具付费墙；组织数据、部门角色和园区功能仍要求真实身份授权。
...
| 智信鸽 | 全能 AI 客服 | <https://47.116.30.60/pigeon/> |
