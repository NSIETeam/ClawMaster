...
ClawMaster 是基于 Otto 成熟运行时演进的桌面优先 AI coworker / coding agent。它以独立产品身份继续发展，同时保留原有内核接口的兼容性，避免在品牌迁移阶段破坏工具、技能、记忆、企业服务和桌面工作区。
...
构建或发布流程以 [build workflow](docs/build-workflow.md) 和 [GitHub Actions](.github/workflows/README.md) 为准；README 不替代发行门禁。
