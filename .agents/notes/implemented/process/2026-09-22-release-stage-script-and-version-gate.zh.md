...
`node --test apps/desktop-tauri/scripts/release-version-guard.test.mjs apps/desktop-tauri/scripts/release-workflow.test.mjs` 通过，`apps/desktop-tauri` 的 `npm run test:update-manifest` 已包含这两个文件。在 `d8eb54e47f` 的 `main` 上，`node apps/desktop-tauri/scripts/release-version-guard.mjs --check desktop-v0.2.3` 以非零退出并输出 `Desktop version 0.2.3 is behind the published 0.2.7`，这正是守卫应当拒绝的状态；只有把 `version.json` 向前推进之后，该检查才会通过。
