...
The source answers it, and the answer is that the current client does not need what the narrowed policy withholds. No browser bundle evaluates source at runtime: a scan of all 723 sources under `packages/client/**/src` and `frontends/dsh/src` finds no `new Function(` and no `eval(` (the one `new Function` in the tree sits in `packages/experimental/webworker-runtime`, which no product frontend references). Client plugin bundles load through a same-origin `<script src>` (`packages/client/modules/src/client/system.ts`), which `script-src 'self'` covers. Both injected styles take the nonce the host hands out — `frontends/dsh/src/client.tsx` and the plugin CSS injector in `packages/client/tsdown.client.ts` read `meta[name="dsh-style-nonce"]` — and the Cordis runner compiles with `meta[name="dsh-script-nonce"]` (`packages/client/.../evaluator.ts`), which the `'nonce-…'` sources cover. So the blank window belongs to the beta.1 payload that predates those consumers, and the narrowed policy is compatible with the current client by construction.

The app is a chrome around that page, which is why the probe measures the right thing. `apps/desktop-tauri/src-tauri/src/chrome.rs` builds the `main` window over the packaged `shell.html` and loads the product UI in a child webview from the local host URL, so the browser probe's target is the page the app displays; the packaged chrome is the only surface the Tauri window policy governs.
...
`client-eval-posture.test.mjs` pins the two halves of the host/client bargain so neither drifts silently: no browser bundle under `packages/client/**/src` or `frontends/dsh/src`, and no packaged chrome script (`shell.js`, `splash.js`, which run under the Tauri window policy's `'self'`-only sources), may contain `new Function(` or `eval(`; the host page must keep `'unsafe-eval'` out of `script-src` while handing out both nonce metas; and the window policy may grant neither `unsafe-eval` nor `unsafe-inline`. Either half regressing is what blanks the window, so the gate fails on the source instead of waiting for a browser run.
...
`node --test apps/desktop-tauri/scripts/render-verify-workflow.test.mjs` passes: the committed workflow satisfies the input, download, verdict and evidence rules, and each negative fixture returns its expected violation. `node --test apps/desktop-tauri/scripts/client-eval-posture.test.mjs` passes: the scan finds no runtime evaluator across the 723 browser sources or the packaged chrome scripts, the host page denies `'unsafe-eval'` while injecting both nonce metas, the window policy grants neither `unsafe-eval` nor `unsafe-inline`, and the negative fixtures show a `new Function(` or a granted eval is rejected. `npm run test:update-manifest` in `apps/desktop-tauri` includes both files, and the desktop release build runs that script — so the eval posture is checked on every release build. The workflow's inputs were compared against the runs whose states this note cites: `35477587606` (blank) and `35478591257` (rendered with the patched payload).
