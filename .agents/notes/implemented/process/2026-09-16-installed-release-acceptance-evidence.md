...
The validator provides a repeatable refusal point and retained artifact integrity. Its tests prove rejection of missing platforms, stale source, modified files, substituted signatures, omitted scenarios and unconsented account claims, including the real CLI exit status. It does not create certificates, run device acceptance or publish a release. The release workflow invokes the strict checker before update metadata generation and release upload; its shell-step regression rejects missing and incomplete manifests. Native installation evidence and its collection remain separate requirements before an Issue or release can be accepted.
