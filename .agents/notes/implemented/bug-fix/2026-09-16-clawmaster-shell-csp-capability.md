...
The Host page is served from the DSH frontend distribution and carries its own response CSP because Tauri's packaged-page CSP does not set headers on this external loopback page. Its policy hashes the exact boot-time inline script and style blocks, allows same-origin script files and requests, permits only the local preview frames and workers, and blocks inline event handlers and native objects. The style-attribute exception remains limited to layout values rendered by the DSH UI.
