...
The [reviewed-write decision](2026-09-13-enterprise-reviewed-writes-and-bounded-queries.md) still owns one-shot model approvals and exact replay. This decision changes the pre-stable browser HTTP responses and updates the client and desktop startup/restart smoke together. Database schema 3 and portable backup format 1 are unchanged; added indexes do not prevent an older owner from opening the database. A Host and browser bundle must be deployed together because the overview/receipt protocol does not support old snapshot consumers.
...
Ordinary list and save responses are independent of total audit history. The [capacity report](../../../../frontends/dsh/benchmarks/README.md) records three synthetic tiers through production HTTP handlers, including actual serialization, with source and worker provenance. Search/count time and startup validation still grow with data size. Explicit backup/restore remains synchronous and whole-store; the recorded memory and scheduling stalls preclude a claim that all capacity work is finished. Background backup, cancellation after synchronous execution starts, process-tree budgets and installed-platform acceptance require separate work.
