...
The restore transaction suspends incremental full-text triggers while replacing rows, rebuilds every index from the restored tables once, and reinstates the triggers before the final authorization check. Index data, triggers, records and version metadata commit or roll back together. Existing WAL readers retain their earlier snapshot; subsequent readers see only the committed generation. Ordinary record writes continue to update indexes incrementally.
