...
Host 页面由 DSH 前端发布目录提供。由于 Tauri 的打包页面 CSP 不会为这个外部 loopback 页面设置响应头，Host 页面使用自身的响应 CSP。策略会对启动时精确的内联脚本和样式区块计算摘要，为可信运行时脚本与样式分别提供每个响应独立的 nonce，只允许同源脚本文件和请求，仅允许本地预览 frame 与 worker，并阻止内联事件处理器及原生对象。样式属性例外仅支持 DSH 界面渲染的布局值。

动态 Cordis 浏览器代码通过 `getClientCode` 取自所属 Session 已授权的活动运行。求值器用脚本 nonce 将该源码编译为闭包，删除临时脚本与回调，然后执行闭包。不授予 `unsafe-eval` 权限，也不暴露通用全局求值器；形似 HTML 的源码仍保留为脚本文本。可信页面代码可以访问 nonce，它不承担获批插件之间的隔离职责。
