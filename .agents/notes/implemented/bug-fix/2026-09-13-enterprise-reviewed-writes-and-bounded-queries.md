...
The [command](../../../../frontends/dsh/tests/command-input.test.mjs) and [backup carrier tests](../../../../frontends/dsh/tests/enterprise-backup-carrier.test.mjs) send unfinished chunked uploads through the production HTTP bridge. Oversize and idle uploads must receive complete structured refusals followed by a server-owned socket close. A subsequent HTTP command must acquire the released capacity; the client does not close the rejected upload to satisfy the test.
