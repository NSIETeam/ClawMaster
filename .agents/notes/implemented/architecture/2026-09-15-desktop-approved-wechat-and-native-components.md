...
Native cancellation acceptance requires a readiness handshake from the owned child before cancellation and confirms process exit before completion; filesystem notification behavior is not a prerequisite. Secret Service cleanup predicates return explicit success or failure so a failing shell exit cannot suppress termination of an owned daemon.
