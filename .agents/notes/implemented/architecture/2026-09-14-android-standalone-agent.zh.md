...
移动执行器仅开放笔记搜索、读取及带修订检查的写入工具。每次模型发起的写入都等待一次性原生审批，审批绑定准确的提议值。取消与提交串行执行；取消不能授权尚未提交的写入。中断后缺少持久化工具回执表示结果未知，不表示应当重试。
...
手机可以在没有 ClawMaster 后端的情况下完成模型、工具和审批循环，但仍需配置模型服务商并保持联网。仅前台执行避免了在缺少前台服务设计时宣称 Android 后台可靠性。移动记录与凭据保存在应用私有空间，卸载时删除。
