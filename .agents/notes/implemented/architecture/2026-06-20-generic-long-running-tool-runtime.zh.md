...
进程内 Service Provider 还拥有有界准入，其理由记录在[有界后台任务准入决策](../../archived/bug-fix/2026-08-11-bounded-background-job-admission.md)中。它的 `maxConcurrentJobsPerOwner` 与 `maxConcurrentJobs` 配置均为正的安全整数，各自默认 `10`。前者限制每个确切 `Agent` 对象及共享无主桶，后者限制整个注册表，防止新增 Session 成倍扩大进程额度。`start()` 从 `running` 与 `stopping` 记录以及同步启动器持有的预留名额派生两项计数；嵌套启动无法复用已预留的名额。容量拒绝发生在 `run()` 与 id 分配之前，处于 stopping 的任务只有在生产方 `done` 结算时才释放名额。Service Provider 不排队或抢占任务，也不保留已注册任务的第二份可变计数。启动器抛错时释放预留名额，不消耗 id。
...
单元覆盖固定预检原子性、按 kind 分配的 id、按确切 owner、无 owner 桶和整个进程执行的准入、同步启动器重入、`stopping` 占位、终态释放、输出上限的校验与投影、完整结果的 UTF-8 字节上限、流式与最终读取、等待超时与中止竞态、取消、首次结果优先的结算、监听器隔离、通知压制、所有者隔离、陈旧的所有者实例、所有者清理、服务资源销毁和无控制器防线。生产方测试覆盖 bash 进程映射、subagent 启动取消、终止映射与释放。快照覆盖固定控制工具 schema、提示词指导，以及组合完整的 headless 路径：任一配置上限拒绝第二个真实后台 Bash 任务，阻止其写入文件，并允许通过 `job_kill` 恢复。
...
单个确切 owner 无法无限增加进程内由 Task 承载的工作。各 owner 保留独立桶，同时共享进程总额度；打开更多 Session 不会增加容量。这些计数不测量或限制总 RSS、WebView 或外部应用。取消请求会继续占用容量，直到生产方真正释放资源，因此用新工作替换缓慢停止的任务不会突破已配置的实时资源预算。
