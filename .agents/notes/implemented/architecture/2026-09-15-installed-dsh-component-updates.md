...
The component bundler preserves dependency symlink identities relative to the package. Resolving links to another checkout would otherwise embed that machine path in module labels and change authenticated artifact bytes for identical source and dependency versions. Host, installer and maintenance bundles use the same compiler setting; output bytes are not rewritten after bundling. Real subprocess builds at different checkout depths compare all three digests and keep the stale-artifact rejection check.
...
The portable access kit exposes the same maintenance operation as `update-kit.mjs repair --yes --dsh-home <DSH_HOME>`. It authenticates the extracted kit before invoking maintenance, so an older desktop without the bundled helper can finish or recover an already approved updater operation after its Host exits. Historical journal fields written by updater `0.1.0` and `0.1.1` are exercised through the `0.1.0` kit repair, exact loaded-entry confirmation and rollback. This compatibility repairs staged operations; it does not replace those updater binaries or their native-channel parsers. The command never selects a new candidate, downloads bytes, launches DSH or restores business data; a live recorded Host or missing recovery evidence stops the operation before profile changes.
...
The maintenance path requires both a desktop containing the helper and an updater candidate capable of recording load confirmation. It retains operation status separately from download and selection, never restores business databases, and refuses to reinterpret first-install removal as version rollback. Historical `0.1.0` and `0.1.1` operation journals can be applied and rolled back by the `0.1.0` offline kit in a source-level test; migrating the installed updater binary and its native-channel parser remains separate. WSL and real platform acceptance also remain unverified. Filesystem tests cover interruption on both sides of replacement, missing health, loaded-entry mismatch, modified candidate bytes, a still-live Host, later profile edits and approved version rollback.
