...
The process-local provider also owns bounded admission, whose rationale is recorded in the [bounded background job admission decision](../../archived/bug-fix/2026-08-11-bounded-background-job-admission.md). Its positive-safe-integer `maxConcurrentJobsPerOwner` and `maxConcurrentJobs` settings each default to `10`. The first bounds each exact `Agent` object and a shared unowned bucket; the second bounds the entire registry so additional Sessions cannot multiply the process allowance. `start()` derives both counts from `running` and `stopping` records plus reservations held by synchronous starters; nested starts cannot reuse a reserved place. Capacity rejection occurs before `run()` and id allocation, and producer `done` settlement is the only event that releases a stopping task's place. The provider does not queue, preempt, or retain a second mutable count of registered jobs. A throwing starter releases its reservation without consuming an id.
...
One exact owner cannot grow process-local Task-backed work without bound. Owners retain independent buckets while competing for the shared process allowance; opening more Sessions does not add capacity. These counts do not measure or cap aggregate RSS, WebViews or external applications. A cancellation request keeps capacity occupied until the producer actually releases its resource, so replacing slow-stopping work cannot exceed the configured live-resource budget.
