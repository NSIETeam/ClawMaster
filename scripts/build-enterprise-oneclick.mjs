...
console.log('[bundle] 构建 clawmaster-core 与 clawmaster-server');
run(npmCommand, ['run', 'build', '--workspace', 'clawmaster-core'], {
  shell: process.platform === 'win32',
});
run(npmCommand, ['run', 'build', '--workspace', 'clawmaster-server'], {
  shell: process.platform === 'win32',
});
...
const temporaryRoot = mkdtempSync(
  path.join(os.tmpdir(), 'clawmaster-enterprise-oneclick-'),
);
try {
  const packageNameBase = `clawmaster-enterprise-oneclick-v${version}`;
  ...
  writeFileSync(
    path.join(releaseRoot, 'node_modules', 'clawmaster-core', 'dist', 'index.js'),
...
`,
  ...
  writeFileSync(
    path.join(releaseRoot, 'node_modules', 'clawmaster-core', 'package.json'),
    `${JSON.stringify(
      {
        name: 'clawmaster-core',
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: 'clawmaster-enterprise-runtime',
        version,
        private: true,
        type: 'module',
  ...
  if (!enterpriseSigningPrivateKey) {
    ...
    writeFileSync(
      ...
      `${JSON.stringify(
        {
          format: 'clawmaster-enterprise-package-signature-v1',
          algorithm: 'Ed25519',
