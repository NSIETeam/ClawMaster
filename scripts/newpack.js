...
async function main() {
  console.log(chalk.bold.magenta('\n🚀 ClawMaster CLI Packaging Process'));
  ...
  try {
    ...
    if (shouldInstall) {
      ...
      try {
        run('npm uninstall -g clawmaster-ai', { stdio: 'pipe' });
      ...
      // Test clawmaster startup
      const testSpinner = ora('🚀 Testing new version startup...').start();
      try {
        run('clawmaster --version');
        testSpinner.succeed(chalk.green('✅ clawmaster startup successful!'));
      } catch {
        testSpinner.warn(chalk.yellow('⚠️ clawmaster startup failed, please test manually'));
      }
    ...
    if (!shouldInstall) {
      ...
      console.log(`\n${chalk.green('🎉')} ${chalk.bold('clawmaster')} is now updated and ready for use!\n`);
    }
...
Features:
  ...
  ✅ Generate tgz file (clawmaster-ai-{new-version}.tgz)
