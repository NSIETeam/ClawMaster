...
const SYSTEMD_SERVICE = path.resolve(
  'deployment/enterprise-oneclick/templates/clawmaster-enterprise.service',
);
...
describe('enterprise one-click service layout', () => {
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const sandbox = mkdtempSync(path.join(tmpdir(), 'clawmaster-oneclick-layout-'));
      try {
        const installRoot = path.join(sandbox, 'opt', 'clawmaster-enterprise');
        ...
        const result = spawnSync(
          ...
          [
            '-c',
            'source "$1"; shift; clawmaster_prepare_service_layout "$@"',
            'bash',
            COMMON_SH,
            installRoot,
            release,
  ...
  it('applies the service layout before systemd starts ClawMaster', () => {
  ...
  it('loads License trust only from the signed release and enforces it in production', () => {
    ...
    expect(service).toContain('Environment=CLAWMASTER_LICENSE_ENFORCE=true');
    expect(service).toContain('license-public-keys.json');
    expect(installer).toContain('export CLAWMASTER_LICENSE_TRUST_FILE=');
  });
});
...
describe('enterprise one-click schema contract', () => {
  it('verifies redacted public health through local database and authenticated status', async () => {
    const sandbox = mkdtempSync(path.join(tmpdir(), 'clawmaster-health-check-'));
        ...
        `
          ...
          const server = createServer((request, response) => {
            ...
            if (
              ...
              && request.headers['x-clawmaster-admin-token'] === process.env.ADMIN_TOKEN
            ) {
  ...
  it('derives one enterprise schema contract from the server source and release manifest', () => {
    ...
    expect(healthCheck).toContain("'x-clawmaster-admin-token': adminToken");
  ...
  it(`accepts v3-v${ENTERPRISE_SCHEMA_VERSION} databases and rejects a future v${ENTERPRISE_SCHEMA_VERSION + 1} database`, () => {
    const sandbox = mkdtempSync(path.join(tmpdir(), 'clawmaster-oneclick-schema-'));
    try {
  ...
  it(`backs up v3 before migration and verifies that v${ENTERPRISE_SCHEMA_VERSION} preserves every row`, () => {
    const sandbox = mkdtempSync(path.join(tmpdir(), 'clawmaster-oneclick-upgrade-'));
    try {
  ...
  it('rejects a release manifest that omits its channel or has an inconsistent schema range', () => {
    const sandbox = mkdtempSync(path.join(tmpdir(), 'clawmaster-oneclick-manifest-'));
    try {
      const manifest = {
        format: 'clawmaster-enterprise-release-v1',
        version: '1.9.0-test',
  ...
  it(`accepts only a completed v${ENTERPRISE_SCHEMA_VERSION} migration readiness result`, () => {
    const sandbox = mkdtempSync(
      path.join(tmpdir(), 'clawmaster-oneclick-readiness-'),
    );
    try {
...
describe('enterprise one-click runtime configuration contract', () => {
  ...
  it('preserves data governance, telemetry and external encryption key settings', () => {
    ...
    const keys = [
      ...
      'CLAWMASTER_FIELD_ENCRYPTION_KEY_FILE',
      'CLAWMASTER_TELEMETRY_ENDPOINT',
      'CLAWMASTER_TELEMETRY_RETENTION_DAYS',
      'CLAWMASTER_FEDERATION_ENABLED',
      'CLAWMASTER_FEDERATION_GATEWAY_URL',
      'CLAWMASTER_FEDERATION_DISPLAY_NAME',
      'CLAWMASTER_FEDERATION_POLL_INTERVAL_MS',
      ...
      'CLAWMASTER_STORAGE_VOLUME_ENCRYPTED',
