...
function productionFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'clawmaster-e2ee-gate-'));
  roots.push(root);
  ...
  writeJson(root, 'artifacts/adversarial.json', {
    format: 1,
    type: 'clawmaster-e2ee-adversarial-verification',
  ...
  const auditPath = writeJson(
    root,
    'security/audit.json',
    signedEnvelope('clawmaster-e2ee-external-audit', auditStatement, auditor),
  );
  ...
  const approval = signedEnvelope(
    'clawmaster-e2ee-production-approval',
    approvalStatement,
    security,
  );
  approval.signatures.push(
    signedEnvelope('clawmaster-e2ee-production-approval', approvalStatement, release)
      .signatures[0],
  );
...
describe('E2EE production release readiness gate', () => {
  it('allows ordinary ClawMaster releases only while the checked-in candidate remains disabled', () => {
  ...
  it('rejects a forged external audit or an approval without both independent roles', () => {
    ...
    expect(result.blockers).toEqual(
      expect.arrayContaining([
        'clawmaster-e2ee-external-audit signature is invalid',
        'clawmaster-e2ee-production-approval is missing a valid release-approver signature',
      ]),
    );
  });
