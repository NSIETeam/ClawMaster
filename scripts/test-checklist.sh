#!/usr/bin/env bash
# ============================================================
# ClawMaster Delivery Test Checklist — Interactive Test Runner
# ============================================================
# Usage:
...
TEST_ITEMS=(
    # ── macOS Apple Silicon ──
    "MAC-AS-01|macOS Apple Silicon|全新安装 ClawMaster .app（未签名，右键打开）|安装成功，应用图标出现在 /Applications"
    ...
    # ── macOS Intel ──
    "MAC-INTEL-01|macOS Intel|全新安装 ClawMaster .app（未签名，右键打开）|安装成功，无架构不兼容提示"
    ...
    # ── Windows 10 ──
    "WIN10-01|Windows 10|下载并安装 ClawMaster 安装包 (.exe/.msi)|安装成功，快捷方式创建"
    "WIN10-02|Windows 10|双击启动 ClawMaster，主窗口正常打开|窗口无报错，首屏渲染正确"
    "WIN10-03|Windows 10|发送消息并接收流式 AI 回复|消息收发正常，编码无乱码"
    "WIN10-04|Windows 10|通过对话操作本地文件|文件操作正常，路径处理正确"

    # ── Windows 11 ──
    "WIN11-01|Windows 11|下载并安装 ClawMaster 安装包 (.exe/.msi)|安装成功，兼容 Windows 11"
    "WIN11-02|Windows 11|双击启动 ClawMaster，主窗口正常渲染|窗口无报错，与现代窗口管理兼容"
    "WIN11-03|Windows 11|发送消息并接收流式 AI 回复|消息收发正常"
    "WIN11-04|Windows 11|通过对话操作本地文件|文件操作正常，权限模型兼容"

    # ── 低性能机器 ──
    "LOW-01|低性能机器|监控 ClawMaster 运行时内存占用峰值|内存 < 500MB（不含模型推理）"
    "LOW-02|低性能机器|测量冷启动时间|冷启动 < 10 秒"

    # ── 飞书私聊 ──
    "FEISHU-DM-01|飞书私聊|飞书用户向 ClawMaster 发送私聊消息|ClawMaster 收到消息并在 app 中实时显示"
    "FEISHU-DM-02|飞书私聊|在 app 内对飞书私聊会话回复|回复推送飞书，对方收到（双向）"
    "FEISHU-DM-03|飞书私聊|飞书私聊中发送附件（图片/文件）|ClawMaster 接收附件并在 app 中展示"

    # ── 飞书群聊 ──
    "FEISHU-GROUP-01|飞书群聊|多用户在群聊中同时发消息|正确区分用户，不串会话"
    "FEISHU-GROUP-02|飞书群聊|群聊中 @mention ClawMaster|正确响应 @mention"
...
echo ""
echo -e "${BOLD}${CYAN}╔═══════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║     ClawMaster 交付测试清单 — 交互式测试执行器         ║${NC}"
