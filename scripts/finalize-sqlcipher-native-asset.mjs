...
function smokeTest(bindingPath) {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'clawmaster-sqlcipher-smoke-'),
  );
...
function main() {
  ...
  const sbom = {
    ...
    metadata: {
      ...
      properties: [
        { name: 'clawmaster:target', value: target },
        { name: 'clawmaster:buildCommit', value: buildCommit },
        { name: 'clawmaster:sourceRevision', value: sourceRevision },
        { name: 'clawmaster:runtime', value: runtime },
        { name: 'clawmaster:runtimeVersion', value: expectedRuntimeVersion },
        { name: 'clawmaster.moduleAbi', value: process.versions.modules },
        { name: 'clawmaster:cryptoProvider', value: cryptoProvider },
        ...(runtime === 'electron'
          ? [
              { name: 'clawmaster:electronVersion', value: expectedRuntimeVersion },
              { name: 'clawmaster:electronModuleAbi', value: process.versions.modules },
            ]
          : []),
      ],
      component: {
