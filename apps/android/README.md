...
Core tests replay recorded model exchanges against the shipped loop and file stores, including Office round trips, revision conflicts, persisted approvals and schedule recovery. The [Android Cloud Validation workflow](../../.github/workflows/android-cloud-validation.yml) builds and tests the release variant on GitHub-hosted Android 8 and Android 15 emulators. Its instrumentation checks native approvals, Activity recreation, Keystore operations, Office containers, foreground continuation and system-scheduled execution. The workflow creates a disposable CI signing key, records the APK checksum and signer certificate, and retains a validation artifact for seven days. It does not test upgrades from the released 0.2.1 APK, ordinary launcher relaunch or separate-process cold start. Recorded providers prove local execution, not live provider availability.
