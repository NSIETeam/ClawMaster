...
Desktop 0.2.2 bundles updater 0.1.1 with Windows cache-collision handling and original archive-path validation. An updater already mounted by the user retains its own version and configuration; native desktop upgrades do not silently replace that component. The published 0.1.0 component and portable kit remain immutable.
