...
Downloaded Node archives must match the official SHA-256 pinned for the selected Node version and platform. A configured mirror changes the download location, not the expected digest. Provisioning reuses only verified cache files, downloads replacements into private temporary files, and atomically publishes them after verification; interruption, cancellation or a wrong digest preserves any existing cache and leaves the installed Node untouched. A subsequent attempt downloads an invalid cache again. Native and WSL provisioning share this check. These archive checks do not attest host-provided Node binaries or replace installed-platform acceptance.

Desktop package version: **0.0.1**. `build:harness` selects the ClawMaster client profile, sets the browser title before plugins load, and projects the existing product icon into the built favicon and PWA manifest. It records the resulting client digest; packaging rejects a different title, profile, manifest name, icon, or digest. Upstream Web asset sources retain their default branding.

The Tauri package is `@deepseek-ai/dsh-desktop-tauri`, independent of upstream Electron. The Host launch URL passes only in memory to a separate WebView, where upstream authentication issues the login cookie. Native capabilities match only the packaged `main` shell WebView; the splash, separate Host content and its document frames receive no native commands. The Host main view stays on its exact loopback origin, including port. Credential-free external HTTP(S) links with `target="_blank"` open in the system browser; loopback destinations and executable schemes are rejected. The same-origin `/clawmaster/office/runtime/NOTICE.html` page without query parameters opens in a separate `office-notice` WebView whose label has no native permissions. Boot logs omit the launch token. The trimmed bundle includes `native/system` and permits unused development-tool patches only in that tree; patch application failures still stop installation.

The packaged shell's CSP permits only same-origin scripts and styles, same-origin images, and Tauri IPC. Shell behavior and styles are external local assets, so the policy does not need `unsafe-inline`; script evaluation from content, remote resources, frames, object embeds, form actions and base-URL overrides are disallowed. This policy applies to packaged shell assets; the separate Host WebView owns its web resources. [Desktop isolation](../../.agents/notes/implemented/architecture/2026-09-16-clawmaster-native-privilege-isolation.md) defines the trusted-plugin limit and verification scope.
...
| **Host** | — | `node apps/cli/lib/bin.js web --host 127.0.0.1`; unavailable optional bundles are logged and omitted, and allowlisted feature plugins that fail startup are disabled before retry. Core UI, permission, approval and Guard failures stop startup |
...
The installation includes the built `frontends/dsh` package and the exact plugin versions below. Native and WSL launches preload [desktop-defaults.mjs](scripts/desktop-defaults.mjs) before `dsh web`, validate artifacts, and append missing Web profile bundles while preserving existing dependencies and user patches. Optional bundles are checked for patch parse errors, malformed inserted Loader rows, effective Loader id collisions with installed and user layers, and preset destination conflicts; unavailable optional bundles are omitted and logged, while required application and security bundles remain fatal. Native launch requires an absolute preload path, converts it to a `file:` URL, and reports conversion failure before starting Node; the [preload decision](../../.agents/notes/implemented/bug-fix/2026-09-13-desktop-node-preload-file-url.md) explains the Windows and filename-encoding requirements. Source-checkout `dev:local` skips this installation-specific preload. Installed plugins reuse the current DSH workspace dependencies, preventing prerelease peer ranges from installing another core version.
...
Installer output: `src-tauri/target/release/bundle/nsis/ClawMaster_0.0.1_x64-setup.exe`
...
The release matrix excludes Intel Mac installers. Existing Intel Mac installations retain their version; supported desktop targets are Apple Silicon, Windows x64 and Linux x64. The [installed-release acceptance reference](acceptance/README.md) defines the evidence required before publication. Android is outside this desktop release workflow.

Pushing a `desktop-v*` tag runs [the desktop release workflow](../../.github/workflows/desktop-release.yml) in build-only mode. It creates Windows x64 NSIS installers, macOS Apple Silicon DMGs, Linux x64 AppImage/deb packages, and retained native smoke evidence; it never publishes automatically. Manual dispatch also defaults to build-only. Publication is a separate dispatch with `publish` enabled, the successful source build run ID, and a full 40-character commit containing `release-evidence/acceptance-manifest.json` plus its referenced evidence files. The publisher downloads the original artifacts from that run, verifies its repository, workflow, result, source commit and release tag, then checks every evidence digest against those unchanged installers before the strict acceptance and final provenance gates. It also imports that run's native macOS and Windows reports; the acceptance manifest must cite them in each lane's `exit-restart` evidence. Keep these two generated reports out of `release-evidence/` because the publisher retrieves them from the original run. The `desktop-release` GitHub environment must require an independent reviewer; its protection is repository configuration and cannot be proven by workflow unit tests. The gate checks evidence integrity and completeness, not whether a human observation is truthful. Stable desktop releases require acceptance for Apple Silicon, Windows x64 and both Linux x64 installer formats; Android is outside this workflow. Program version `0.0.1` publishes as the stable release `desktop-v0.0.1`; prerelease program versions stay outside GitHub Latest. Existing stable releases cannot be overwritten. Each updater artifact carries a Tauri signature; the versioned release includes `latest.json`, `clawmaster-release-signing.pub`, and `SHA256SUMS.txt`. The manifest maps DEB installations to a separately signed DEB and retains AppImage for the generic Linux target. The [release-channel decision](../../.agents/notes/implemented/architecture/2026-09-13-desktop-stable-confirmed-updates.md) owns version and update-consent rules.
...
The bundled [update component](../../frontends/updates/README.md) adds `/updates`, `clawmaster_updates` and the approval-controlled `clawmaster_update` tool. It checks signed server metadata every minute by default; downloads and profile changes require one-time approval. Packaging adds desktop-only `dsh.bundle.patch` metadata and the [desktop patch](updates/cordis.patch.yml) to the copied updater package; its published module and original package stay unchanged. The profile declares `@clawmaster/dsh-updates`, so the existing portable kit rejects a duplicate first installation. The desktop patch uses the same row id as that kit. When an existing profile, home patch or other bundle inserts that updater, startup omits the desktop overlay and preserves the user patch. The native desktop applies approved updater changes with its trusted maintenance helper before starting the Host, confirms the loaded updater entry and restores an unconfirmed selection at the next start. Other restart-only components, legacy updater migration and WSL retain the [documented limitations](../../frontends/updates/README.md).

GitHub is the build and release source; the update server mirrors verified updater files without rebuilding or signing them. Native updates use the four-target v2 endpoint. The withdrawn legacy native endpoint returns 404, and withdrawn native asset URLs are unavailable. Installed builds that point to those URLs require manual installation of a supported release; changing the server or DSH profile cannot change a compiled endpoint.
...
Beta tags in the exact form `desktop-vX.Y.Z-beta.N` build and accept only the Windows x64 NSIS installer and Apple Silicon macOS DMG; stable and other tag lanes retain the full existing platform matrix and acceptance requirements. Every platform build records both workspace lockfile digests, and the final release verifier requires identical lockfile bytes across that release. `package-size-report.json` records each installer size; 20 MiB is an optimization target, so larger files are reported without blocking publication. Beta publication requires the Actions variable `CLAWMASTER_DOWNLOAD_PAGE_URL` to point to the public download page. The page must expose `data-clawmaster-release-version="X.Y.Z-beta.N"` on an HTML element and direct GitHub Release download anchors for both exact installer URLs. The workflow checks the page before and after publication; this repository does not contain the live Pages source, so that external configuration and page contract require verification in the publishing repository.
