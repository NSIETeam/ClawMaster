...
function fixture() {
  ...
  const run = (pid, stamp) => ({
    ...
    desktopIdentity: { pid, startTimeUnixMs: stamp, path: 'C:\\CI\\install\\dsh-desktop.exe' },
    desktopIdentityAtLaunch: { pid, startTimeUnixMs: stamp, path: 'C:\\CI\\install\\dsh-desktop.exe' },
    hostIdentity: { pid: pid + 1, parentPid: pid, startTimeUnixMs: stamp + 1, path: 'C:\\CI\\node.exe' },
    hostIdentityAtRecord: { pid: pid + 1, parentPid: pid, startTimeUnixMs: stamp + 1, path: 'C:\\CI\\node.exe' },
...
test('stale, foreign and abandoned processes cannot satisfy native acceptance', () => {
  for (const mutate of [
    ...
    e => { e.runs[0].desktopIdentity.startTimeUnixMs++ },
    e => { e.runs[0].desktopIdentity.path = 'C:\\CI\\other.exe' },
    e => { e.runs[0].hostIdentity.parentPid++ },
    e => { e.runs[0].hostIdentity.startTimeUnixMs++ },
