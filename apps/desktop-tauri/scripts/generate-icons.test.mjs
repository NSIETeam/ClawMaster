...
test('normalizeIcns refuses a file that is not a well-formed ICNS container', () => {
  ...
  try {
    ...
    const incomplete = join(root, 'incomplete.icns')
    writeFileSync(incomplete, Buffer.from('icns'))
    assert.throws(() => normalizeIcns(incomplete), /Not an ICNS container/)

    const trailing = join(root, 'trailing.icns')
    const trailingBytes = Buffer.concat([container([['ic07', PNG]]), Buffer.from([0])])
    trailingBytes.writeUInt32BE(trailingBytes.length, 4)
    writeFileSync(trailing, trailingBytes)
    assert.throws(() => normalizeIcns(trailing), /Incomplete ICNS entry header/)
    assert.deepEqual(readFileSync(trailing), trailingBytes)
  } finally { rmSync(root, { recursive: true, force: true }) }
})
