...
test('captures component versions, emitted bytes, lockfiles and plugin patch provenance', t => {
  ...
  write('apps/desktop-tauri/patches/memory.integrity.json', '{"integrity":"fixture"}\n')
  ...
  assert.equal(first.patches.length, 3)
  assert.ok(first.patches.some(entry => entry.path.endsWith('memory.integrity.json')))
