...
test('installed plugins use reviewed versions, applied patches and one DSH workspace runtime', async t => {
  ...
  assert.deepEqual(entries.filter(entry => entry.id === 'clawmaster-update-component-updates').map(entry => entry.name), ['@clawmaster/dsh-updates'])
  const updater = await import(pathToFileURL(resolver.resolve('@clawmaster/dsh-updates')).href)
  assert.equal(updater.name, 'clawmaster-updates')
  assert.equal(updater.Config.parse(undefined).checkIntervalMs, 60_000)
