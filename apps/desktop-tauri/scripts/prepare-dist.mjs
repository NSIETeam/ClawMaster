...
const version = spawnSync(process.execPath, [join(root, 'scripts', 'desktop-version.mjs'), '--check'], { stdio: 'inherit', cwd: root })
...
for (const name of ['dsh', 'guard', 'notes', 'graph-memory', 'office', 'rpa']) {
...
for (const name of ['desktop-i18n', 'shell', 'splash']) cpSync(join(root, `${name}.js`), join(dist, `${name}.js`))
for (const name of ['shell', 'splash']) cpSync(join(root, `${name}.css`), join(dist, `${name}.css`))
