...
test('release builds verify publisher signatures while keeping the reset exception version-scoped', () => {
  ...
  const macBundleVerify = steps.findIndex(step => step.name === 'Verify macOS signature')
  ...
  assert.match(steps[buildIndex].run, /\$env:RELEASE_TAG -eq 'desktop-v0\.0\.1'/u)
  ...
  assert.equal(steps[macBundleVerify].if, "runner.os == 'macOS'")
