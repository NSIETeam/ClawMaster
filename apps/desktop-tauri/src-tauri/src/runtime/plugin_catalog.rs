...
pub fn begin_from_tray(app: &AppHandle) {
    if INSTALLING
        ...
        .is_err()
    {
        notify::toast(app, "ClawMaster", i18n::t(Msg::CatalogBusy));
        return;
    }

    let Some(runtime) = app.try_state::<DesktopRuntime>() else {
        INSTALLING.store(false, Ordering::SeqCst);
        notify::toast(app, "ClawMaster", i18n::t(Msg::CatalogNotReady));
        return;
    };
    let target = runtime.plugin_target.clone();
    notify::toast(app, "ClawMaster", i18n::t(Msg::CatalogInstalling));
    let app = app.clone();
    tauri::async_runtime::spawn(async move {
        let result = tokio::task::spawn_blocking(move || install_catalog(&target)).await;
        match result {
            Ok(Ok(())) => {
                notify::toast(&app, "ClawMaster", i18n::t(Msg::CatalogRestarting));
