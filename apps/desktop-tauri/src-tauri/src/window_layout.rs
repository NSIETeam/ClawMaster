//! Native window-control metadata for the desktop shell.
//!
//! The main window uses the platform's native decorations, so no in-window
//! title bar is drawn and `titlebar_height` is zero. The native title bar stays
//! outside the embedded Web UI, including on macOS where its title text is
//! hidden. The button-layout parsing remains for the `__DSH_CHROME__` payload.
//!
