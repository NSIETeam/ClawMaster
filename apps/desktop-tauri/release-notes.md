# ClawMaster WatchDog 0.2.1

## 中文

开启AI时代的企业协作。WatchDog 是使用 Tauri 桌面壳与 DSH 运行时的本地企业协作工作台，复用现有模型凭证、会话、工具、审批和插件。程序版本为 `0.2.1`，发布名称为 `0.2.1`。

- **任务跟进**：首次教程围绕业务目标、责任、验收与巡检。工作台优先展示待审批、待回答和待审核计划，点击回到原会话。目标草稿在导航时保留；受理状态未确认时锁定原目标与频率，重试同一请求，避免重复创建。当前未运行不等于业务已经完成。
- **本版新增**：笔记批注、笔记与记忆图谱检索、危险操作复核及任务事实归档；本地业务备份恢复使用单调递增的恢复代次，旧表单和审批不能误写回恢复后的数据。RPA 控制层提供检查点与操作回执；原生控制需另行配置 helper 和系统权限，未配置时明确不可用。
...
ClawMaster WatchDog is a local enterprise collaboration workspace with a Tauri desktop shell and the DSH runtime. It reuses model credentials, conversations, tools, approvals and plugins. The program version is `0.2.1`; the release is named `0.2.1`.
...
- **New in this version**: Note annotations, graph retrieval across notes and memory, destructive-operation review and turn-fact archiving. Enterprise backup restoration uses a monotonic restore generation to reject stale forms and approvals. The RPA control plane provides checkpoints and action receipts; native control needs a separately configured helper and system permissions and reports unavailability without them.
