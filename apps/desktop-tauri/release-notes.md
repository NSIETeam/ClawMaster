...
本版需要手动安装，不支持从已撤回的 0.2.x 自动升级。macOS 安装包未经过 Apple 公证，Windows 安装包未使用 Authenticode 签名。下载前请阅读[首次启动安全提示处理教程](https://github.com/NSIETeam/ClawMaster/blob/desktop-v0.0.1/docs/user/guide/desktop-first-launch-security-warnings.zh.md)，并先核对官方发布页提供的 SHA-256 校验和。
...
原生桌面 RPA 执行本版仍未启用；相关操作会报告不可用，不作为本版已支持能力。
...
This version must be installed manually and does not automatically upgrade withdrawn 0.2.x installations. The macOS installer is not notarized by Apple, and the Windows installer is not signed with Authenticode. Before downloading, read the [first-launch security warning guide](https://github.com/NSIETeam/ClawMaster/blob/desktop-v0.0.1/docs/user/guide/desktop-first-launch-security-warnings.md) and verify the SHA-256 checksum published on the official release page.
...
Native desktop RPA execution remains disabled in this release; affected actions report unavailable and are not presented as supported.
