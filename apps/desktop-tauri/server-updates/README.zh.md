...
已安装的桌面版本继续使用编译进程序的更新端点。指向已退役端点的版本须手动安装受支持的桌面版本；服务器无法重定向该程序，也不会修改其 DSH 配置。
...
根路径 `/updates/clawmaster/latest.json` 和 `/updates/clawmaster/versions/` 原生通道已退役。Nginx 配置会对这些路径返回 404，原生更新只由 v2 提供；已撤下版本的文件路径返回 404 或 410，客户端无法再下载。更新组件 `0.1.0` 和 `0.1.1` 在自身升级前仍可能请求已退役的根端点。组件目录、组件产物与便携更新包保留现有根 URL。
...
[manifest 生成器](../scripts/generate-updater-manifest.mjs)要求显式选择四目标发布的 `--target-set current`，或在校验旧五目标 manifest 时选择 `--target-set legacy`。文件缺失不会自动选用更小的集合。它还接受 `--asset-base-url`，指定完整 HTTPS 版本目录，例如 `https://8.140.52.117/updates/clawmaster/v2/versions/0.0.1/`。它不同于同步器的通道级 `--base-url`。省略生成器选项时保留 GitHub 下载 URL；提供该选项时保留文件名与签名，只替换 URL 前缀。含凭据、查询参数、片段或不明确路径段的地址均被拒绝。
...
部署后必须确保已退役的根通道无法访问。使用管理员权限执行以下步骤：

1. 停止 `clawmaster-updates.timer`，停止正在执行的服务调用，并确认发布锁已释放。将现有单元、启用的 Nginx include 与服务脚本备份到管理员拥有的目录。
2. 将同步器、生成器和验签器安装到 `/opt/clawmaster-updates-v2/scripts`，代码归 root 所有且服务不可写。保留现有公钥，安装 v2 服务配置并重新加载 systemd。不要将旧状态复制或移动到新目录。
3. 将提供的 Nginx location 合并到现有虚拟主机，保留无关路由与组件/更新包别名，同时移除两个根原生更新 location。重新加载 Nginx 前先运行 `nginx -t`。
4. 确认已退役的根 manifest 和版本路径返回 404，再执行一次 v2 服务同步。按照选定的 GitHub 发布核对回执、来源签名与 HTTPS 响应，之后启用每小时定时器。

部署失败时，停止服务与定时器，恢复备份的单元与 Nginx 配置，验证 Nginx 后再重新加载。将已退役通道的恢复副本保存在公开目录之外；服务能接受所选发布前，保持同步停止。将正式指针指向旧版本不等于应用自动降级：服务器与客户端都会拒绝较低版本。修正后的发布使用新的正式版本。不可覆盖不可变文件、关闭 TLS 验证，或通过更换签名公钥让被拒版本通过。

[发布决策](../../../.agents/notes/implemented/architecture/2026-09-15-desktop-self-hosted-update-channel.zh.md)记录信任职责。[版本化通道决策](../../../.agents/notes/implemented/architecture/2026-09-15-versioned-native-update-targets.zh.md)说明历史解析器兼容性。[发布撤回决策](../../../.agents/notes/implemented/architecture/2026-09-24-withdraw-native-release-history.zh.md)记录公开版本重置与服务器清理限制。
