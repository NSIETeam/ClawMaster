...
The legacy `/updates/clawmaster/latest.json` and `/updates/clawmaster/versions/` retain the five-target 0.2.1 channel and its immutable files. Updater components 0.1.0 and 0.1.1 require that complete legacy set; changing their server response would not update their parser. The v2 service writes a separate state directory. Component catalogs, component artifacts and portable kits keep their existing root URLs.
...
The [manifest generator](../scripts/generate-updater-manifest.mjs) requires explicit `--target-set current` for the four-target release or `--target-set legacy` for all five historical targets. Missing files never select a smaller set automatically. It also accepts `--asset-base-url` for an exact HTTPS version directory, such as `https://8.140.52.117/updates/clawmaster/v2/versions/0.2.2/`. This differs from the synchronizer’s channel-level `--base-url`. Omitting the generator option preserves GitHub download URLs; supplying it preserves filenames and signatures while replacing their URL prefix. Credentials, query strings, fragments and ambiguous path segments are rejected.
...
The v2 migration changes service code, its state path and Nginx aliases; it does not rewrite the legacy manifest or version files. Apply these steps with administrator access:

1. Stop `clawmaster-updates.timer`, stop any active service invocation, and confirm the publication lock is released. Save the existing units, active Nginx include and service scripts in an administrator-owned backup directory; record the legacy manifest, public-key and immutable-file hashes.
...
3. Merge the supplied Nginx locations into the existing virtual host, preserving unrelated routes and all legacy/component/kit aliases. Run `nginx -t` before reloading Nginx.
4. Run one v2 service synchronization. Verify its receipt, source signatures and HTTPS response against the current GitHub release. Compare every retained legacy hash with its pre-migration value, then enable the hourly timer. An older five-target GitHub Latest remains a valid bootstrap source for v2.

On migration failure, stop the v2 service and timer, restore the saved units and Nginx configuration, and revalidate Nginx before reloading it. Retain any published v2 files for recovery and leave the legacy channel unchanged. Keep synchronization stopped until the restored code can accept the selected release. An older stable pointer is not an automatic application downgrade: both the server and client reject lower versions. A corrected release uses a new stable version. Do not overwrite immutable files, disable TLS verification, or replace a signing key to make a rejected release pass.

The [publication decision](../../../.agents/notes/implemented/architecture/2026-09-15-desktop-self-hosted-update-channel.md) records trust ownership. The [versioned-channel decision](../../../.agents/notes/implemented/architecture/2026-09-15-versioned-native-update-targets.md) explains legacy parser compatibility and retained paths.
