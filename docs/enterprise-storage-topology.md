...
ClawMaster uses two deliberately different persistence topologies.

## Local and desktop

Desktop and offline deployments keep SQLite with SQLCipher so they can run
without a network dependency. Exactly one ClawMaster process may write the database,
and the database file must remain on a local filesystem. ClawMaster rejects UNC,
...
The target enterprise topology is stateless ClawMaster Server replicas sharing:
...
ClawMaster Server does not implement database failover itself: every replica uses the
...
$env:CLAWMASTER_POSTGRES_URL = 'postgresql://clawmaster:<password>@postgres-rw.internal/clawmaster'
$env:CLAWMASTER_ENTERPRISE_REPLICA_COUNT = '3'
$env:CLAWMASTER_ATTACHMENT_OBJECT_STORE = 's3'
$env:CLAWMASTER_S3_BUCKET = 'clawmaster-private'
...
$env:CLAWMASTER_ACCOUNT_SYNC_ENCRYPTION_KEY_FILE = 'D:\clawmaster-secrets\account-sync.key'
```
...
```powershell
$env:CLAWMASTER_ENTERPRISE_DATABASE_BACKEND = 'postgresql'
$env:CLAWMASTER_POSTGRES_URL = 'postgresql://clawmaster:password@db.internal/clawmaster'
...
read-only standby or a schema newer than the running ClawMaster version. Its output
...
- Run ClawMaster Server replicas without local authoritative state. Session/rate
...
- Never place a SQLite database or its WAL files on NFS, SMB/CIFS, or another
  shared filesystem for multi-instance writes. ClawMaster rejects known network
...
$env:CLAWMASTER_SQLITE_ATTACHMENT_STORAGE_DIR = 'D:\clawmaster-data\attachments'
$env:CLAWMASTER_SQLITE_ATTACHMENT_ENCRYPTION_KEY_FILE = 'D:\clawmaster-keys\attachment-storage.key'
...
local encrypted copies. ClawMaster rejects this compatibility mount when more than
...
are never sent to ClawMaster Server or the object store.
