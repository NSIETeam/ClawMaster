...
- SQLite schema 25 与 PostgreSQL migration 15 都包含组织级事件唯一键、持久 consumer receipt 和带递增 fence token 的 consumer claim/lease 数据契约；失败处理不会提前确认。
- SQLite 持久化 Watchdog Action/Audit 投影，事务提交后的重放依靠稳定 ID 和唯一约束保持幂等。
- SQLite consumer 原子领取单条事件；活跃租约阻止并发 worker 重复处理，过期租约允许更高 fence token 接管，旧 worker 不能再确认或提交 Watchdog 投影。
- 认证 HTTP 路由支持事件写入、租户级 Watchdog 检查、Action 查询和管理员 Audit 查询；管理员触发检查不会处理其他租户事件。
...
- Watchdog Task 投影，以及 PostgreSQL 异步 consumer 的原子 claim/lease/fencing 实现。
