...
- Watchdog Task 投影，以及并发 consumer claim/lease/fencing 和崩溃后的超时接管。
- Inventory/Cash/Growth Engine、预测校准、真实猫头鹰/知了猴 OAuth/API 连接。
- Desktop 首页与真实 Design Partner 验收；当前测试不替代 live/production 证据。
- 下一阶段应实现 PostgreSQL 异步 repository 和并发消费租约，再把 action 执行接到 policy/approval/workflow/audit 的真实持久路径。
