...
| D7 | **Browser launch 400 regression**: `browser-open.spec.ts` expects 200, gets 400 (reproduces in isolation) | Web startup handshake failure | Unfixed | Investigate the token/host validation change |
| D8 | **Hardcoded version drift**: `packages/server/src/server.ts` appVersion decoupled from the product version (was 0.0.2beta) | Version-consistency gate red | Resolved: the 2026-09-21 version reset, appVersion=0.0.1beta matches desktop 0.0.1-beta | Inject from package.json |
| D9 | **Orphan gate tests**: `scripts/tests/*.test.js` are in no vitest include | CI never runs the release gates | Manually `vitest run --config /tmp/vitest.empty.mjs` | Add them to the vitest include |
