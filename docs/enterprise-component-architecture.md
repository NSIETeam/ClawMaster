...
ClawMaster should support organizations that want their own private agent without forcing them to fork the whole product. The intended shape is a stable, lightweight kernel plus independently replaceable external components.

## Ownership model

| Layer | Owner | Update pattern | Examples |
| --- | --- | --- | --- |
| Kernel | ClawMaster upstream | Regular upstream kernel updates | turn lifecycle, tool execution engine, policy gate, model routing, memory budget, component manifest validation |
...
Enterprise distributions should consume the ClawMaster kernel as a compiled, signed artifact. This gives organizations a stable update path without turning every deployment into a private fork.
...
```json
{
  ...
  "artifact": {
    ...
    "path": "dist/clawmaster-kernel.exe",
    "sha256": "<64-char-sha256>",
    "signature": "ed25519:<detached-signature>",
    "publicKeyId": "clawmaster-kernel-prod-2026",
...
| --- | --- | --- |
| Agent pool and memory accounting | `clawmaster-native/src/agent_pool.rs` | `packages/core/src/core/subAgent.ts`, `packages/core/src/core/agentResourceBudget.ts` |
| Session storage and bounded history | `clawmaster-native/src/session_store.rs` | `packages/core/src/services/sessionManager.ts` |
| Tokenizer and schema/history budget counting | `clawmaster-native/src/tokenizer.rs` | `packages/core/src/core/tokenLimits.ts` callers |
...
Optional components use the versioned `ClawMasterComponentManifest` contract from `packages/core/src/components/componentManifest.ts`.
...
If a requested change requires modifying the kernel, it should answer: "Would every ClawMaster distribution benefit from this?" If not, it probably belongs in a component.
