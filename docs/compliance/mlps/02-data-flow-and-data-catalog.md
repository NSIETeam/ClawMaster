...
```mermaid
flowchart TD
  U["用户输入 / 文件 / 会议"] --> D["ClawMaster Desktop"]
  D -->|"账号、组织、工单、密文消息"| S["ClawMaster Server"]
  D -->|"本机记忆、日志、Skill"| LD[("SQLCipher / 系统安全存储 / 加密磁盘")]
  S --> P[("PostgreSQL")]
  S --> R[("Redis")]
  S --> O[("S3 密文附件")]
  S -->|"最小健康、License、签名用量"| C["ClawMaster Control"]
...
| --- | --- | --- | --- | --- | --- | --- |
| 账号身份 | 姓名、手机号、企业、部门、岗位 | 用户、管理员 | 企业 PostgreSQL/SQLite | ClawMaster Server | 最小权限、TLS、敏感字段加密/哈希 | 账号存续期及法定期限 |
| 认证凭据 | 密码哈希、会话令牌、验证码 | 用户、认证服务 | 企业数据库/缓存 | ClawMaster Server | 强哈希、令牌加密、限流 | 按会话和安全策略 |
...
| 模型请求 | 提示词、上下文、工具结果 | ClawMaster Desktop/Server | 通常不由 ClawMaster 长期保存 | 客户选择的模型供应商 | HTTPS、最小上下文、供应商评审 | 以供应商合同为准 |
| 商业控制 | deploymentId、License、席位、模块、签名用量 | Server/Control | ClawMaster Control | 授权运营人员 | 签名、防重放、RBAC、审计 | 合同和财税期限 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ClawMaster Control | License、健康、签名用量 | 授权和运营 | 待填写 | 通常不含正文 | 合同、管理员启用 | 待填写 | 健康遥测可关闭，License 依套餐 |
