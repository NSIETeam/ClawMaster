...
compatibility ledger inherited from the ClawMaster codebase. It records where the
enterprise schema, capability set, and security integrations came from. Its
`NSIETeam/clawmaster-new` and `internal` fields are historical evidence, not the
...
optional `--verify-git-refs` mode exists only for auditing the historical ClawMaster
