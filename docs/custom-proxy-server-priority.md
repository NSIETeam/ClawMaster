...
本文档详细说明了在 ClawMaster 中配置自定义代理服务器时的优先级规则。
...
- **位置**: 由环境变量 `GEMINI_CLI_SYSTEM_SETTINGS_PATH` 指定或平台默认位置
  - macOS: `/Library/Application Support/ClawMasterCli/settings.json`
...
### 场景 4：系统管理员为所有用户设置代理
```json
// /Library/Application Support/ClawMasterCli/settings.json（macOS 示例）
{
...
1. 检查是否有系统级设置（System Settings）覆盖了你的配置
   ```bash
   # 查看系统设置文件（如果存在）
   cat "/Library/Application Support/ClawMasterCli/settings.json"  # macOS
...
5. 查看启动时的日志输出
   ...
   或
   ```
   [DeepX] Connecting to ClawMaster server: https://api-code.easycode-userlab.ai
   ```
