# ClawMaster 集成 DSH 全特性运行时总体规划（历史冻结）
...
目标仓库：`NSIETeam/clawmaster-new`

ClawMaster 基线：`internal@30e2adab7bf9d647ff69965bbbbcf1044b3e3014`

DSH 基线：`master@b150a551b8d465e31e418e1b2eaf5e79bbb7d28e`（`0.1.1-rc.2`）

## 1. 目的

本规划定义 ClawMaster 如何完整获得 DeepSeek Harness（下称 DSH）的稳定产品能力，同时保留 ClawMaster 已有的桌面端、企业服务、飞书、知识治理、文档专家、RPA、商业控制和安全发布体系。

本项目不是把 DSH 的工具逐个复制进 `packages/core`，也不是用 DSH 重写整个 ClawMaster。目标是建立可替换的 Agent Runtime 边界，把 DSH 作为隔离运行时接入 ClawMaster，并让 ClawMaster 产品层通过稳定协议消费会话事件、控制运行状态和呈现用户交互。

最终形态必须满足以下条件：

1. DSH 负责 Agent 循环、会话事件、工具组合、Profile、插件、后台任务、终端、子 Agent、工作流、压缩和运行时恢复。
2. ClawMaster 负责桌面和企业产品体验、身份、授权、商业策略、飞书、知识治理、文档工作流、RPA、发布、更新和组织级审计。
3. 两者之间只有版本化协议和显式适配层，不允许 Desktop 或 Server 直接导入 DSH 内部源码。
4. ClawMaster 旧运行时在迁移期继续可用，但同一会话创建后不得在两个运行时之间切换。
5. 高风险行为无论由哪个运行时发起，都必须经过 ClawMaster 中央策略、用户确认和审计链路。
...
- 对 DSH Web UI 的像素级复制。功能语义必须等价，视觉仍使用 ClawMaster 设计系统。
- 将 ClawMaster 企业、飞书和商业模块回迁到 DSH 上游。
- 在迁移过程中重写 ClawMaster 已稳定运行的文档、RPA、知识和企业服务实现。

### 2.3 兼容性立场

DSH 仍处于 RC 阶段，没有稳定协议兼容承诺。因此 ClawMaster 必须：

- 固定 DSH 精确提交和产物摘要，禁止无门禁追踪 `master`。
- 通过 Adapter 隔离 DSH 类型，ClawMaster 产品代码只能依赖 `runtime-contracts`。
- 为协议、事件和 Profile 建立黄金快照。
- 每次升级单独提交，附带兼容报告、迁移说明和回滚产物。
- 将 DSH MIT 许可证和第三方声明纳入 ClawMaster Apache-2.0 发行物的 NOTICE/SBOM。

## 3. 当前状态与差距

### 3.1 ClawMaster 已具备的能力

ClawMaster 当前已经具备以下可复用能力：
...
ClawMaster 与 DSH 的主要差距不是工具数量，而是运行时语义：

1. ClawMaster 工具仍由 `Config.createToolRegistry()` 集中硬编码，缺少 Profile/Bundle/Preset 的组合能力。
2. `ClawMasterComponentManifest` 目前主要提供类型和校验，尚未形成统一加载、作用域、卸载和回滚链路。
...
9. ClawMaster 只有 ACP Client，没有完整 ACP Server、通用 JSON-RPC Runtime SDK 和 Headless 产品入口。
...
└───────────────────────────┬────────────────────────────────┘
                            │ runtime-contracts
┌───────────────────────────▼────────────────────────────────┐
│ ClawMaster Runtime Gateway                                       │
│ session control / event projection / policy / audit / auth │
└───────────────┬──────────────────────────┬─────────────────┘
                │                          │
┌───────────────▼──────────────┐ ┌────────▼─────────────────┐
│ Legacy ClawMaster Runtime Adapter │ │ DSH Runtime Adapter       │
│ migration only              │ │ process + protocol owner  │
└──────────────────────────────┘ └────────┬─────────────────┘
                                         │ authenticated local IPC
                              ┌──────────▼──────────────────┐
                              │ Pinned DSH Runtime Process  │
                              │ ClawMaster Profile + DSH plugins  │
                              └─────────────────────────────┘
```

### 4.1 为什么采用独立进程

DSH 使用 pnpm monorepo和 Cordis 插件生命周期，ClawMaster 使用 npm workspaces、Electron、Server 和现有核心对象。直接把 DSH 源码合并进 `packages/core` 会产生依赖解析、生命周期、全局单例、版本升级和崩溃隔离问题。

独立进程方案提供以下保证：

- DSH 升级不会把内部类型扩散到 ClawMaster 产品包。
...
```text
packages/
  runtime-contracts/       # ClawMaster 稳定运行时协议和领域类型
  runtime-gateway/         # 会话路由、策略、事件归一化和运行时选择
  runtime-dsh/             # DSH 进程监管、协议客户端和事件翻译
  runtime-legacy/          # 旧 ClawMaster Client/Turn/ToolRegistry 的适配器
runtime/
  dsh/
    profile/               # ClawMaster 的 profile manifest 和 cordis.patch.yml
    presets/               # standard / minimal / code / enterprise
    plugins/               # ClawMaster 专用 DSH plugins
...
- `enterprise`：标准能力加 ClawMaster 企业知识、飞书、审计和组织策略插件。

Profile 层级顺序固定为：

1. 固定 DSH base。
2. ClawMaster 产品 Bundle。
3. 部署 Bundle。
4. 用户 Profile patch。
5. 会话级 Preset。
6. 受控临时 overlay。
...
保留 ClawMaster 现有工具名称兼容层，同时把执行转交 DSH FS provider。覆盖读取、写入、目录、Glob、Grep、批量读取、字符串替换、Patch 和 MultiEdit。
...
复用 ClawMaster 已有产品配置和 DSH capability：

- Web Search/Fetch provider 可替换并带 SSRF 策略。
- MCP discovery、OAuth、工具同步、断线恢复和响应 guard 保留 ClawMaster 安全能力。
...
## 9. ClawMaster 专用 DSH 插件

下列能力通过 ClawMaster 插件接入 DSH，不能写入 DSH 核心循环：

1. `clawmaster-policy-bridge`：调用 ClawMaster CentralPolicy，生成审批和审计事实。
2. `clawmaster-model-router`：把 SceneManager/企业模型配置投影为 DSH LLM routes。
3. `clawmaster-audit-sink`：把 DSH 事件归一化后写入 ClawMaster 本地或企业审计。
4. `clawmaster-memory-provider`：接入个人记忆、Mem0 和企业知识检索。
5. `clawmaster-skill-provider`：接入 ClawMaster Skill 市场、个人 Skill 和企业发布策略。
6. `clawmaster-feishu-tools`：提供飞书协作能力并保留移动端交互语义。
7. `clawmaster-document-tools`：提供文档、PPT、表格和数据分析能力。
8. `clawmaster-rpa-tools`：提供受策略控制的浏览器/桌面 RPA。
9. `clawmaster-enterprise-context`：注入租户、组织、岗位和授权范围。
10. `clawmaster-worklog`：把可交付成果和工作记录投影到 ClawMaster 产品界面。
...
- ClawMaster 发行时校验运行时与 Profile digest。
...
- Projection reducer。
- Policy bridge。
- DSH-to-ClawMaster event translator。
...
从 DSH `base/web/headless` 已有场景建立 ClawMaster 对应快照：
...
退出条件：原有 ClawMaster 关键产品回归全部通过，插件可独立禁用。

### Phase 7：Web、SDK、ACP 与企业 Worker

交付物：

- ClawMaster Web Host 集成。
- 完整 JSON-RPC SDK。
- ACP Server。
- 企业远程 Runtime/Worker。
- 管理员运行时和插件清单。
...
21. 实现 ClawMaster Memory/Knowledge 插件。
22. 实现 ClawMaster Skills/Marketplace 插件。
23. 实现 ClawMaster Feishu/Enterprise 插件。
24. 实现 ClawMaster Documents/RPA 插件。
...
| DSH 与 ClawMaster 都拥有策略 | 双重审批或策略绕过 | ClawMaster CentralPolicy 为最终权威；DSH policy 作为本地前置防线 |
...
| 直接复用 DSH Web API 内部细节 | 升级脆弱 | ClawMaster Runtime Protocol 包装，不允许产品层依赖内部方法 |
...
只有满足以下全部条件，才可以宣称 ClawMaster 已实现 DSH 全部稳定特性：

- DSH `base + web + headless` 稳定产品能力在 ClawMaster 中有等价入口。
...
- ClawMaster 的飞书、知识、文档、RPA 和企业能力已通过插件接入且无核心边界违规。
