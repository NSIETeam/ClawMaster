...
> **致后端同事**：客户端 (ClawMaster CLI) 已经按 Anthropic 官方协议把 `effort`
...
- 注入到请求体：`packages/core/src/core/ClawMasterServerAdapter.ts::applyGenAIThinkingConfig()`
- 自定义模型直连路径（不走 ClawMaster 服务端，参考用）：`packages/core/src/core/customModelAdapter.ts::callAnthropicModelStream()`
