...
4. 已冻结企业变更窗口，并确认所有 ClawMaster Server/旧企业服务端进程停止。
...
$env:CLAWMASTER_SQLCIPHER_NATIVE_BINDING = 'D:\clawmaster\native\sqlcipher\win32-x64\better_sqlite3.node'
```
...
`clawmaster_sqlite_import_rows` 暂存表，并在 `clawmaster_sqlite_import_tables` 保存列顺序、
...
使任务失败。只有三次校验一致，`clawmaster_sqlite_import_runs.state` 才会变成
