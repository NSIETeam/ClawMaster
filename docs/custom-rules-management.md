...
ClawMaster 支持自定义规则管理，允许您为项目定义特定的编码约定、风格指南和最佳实践。这些规则会自动注入到 AI 助手的上下文中，确保生成的代码符合您的项目标准。

### 快速了解：哪些文件会被加载为规则？

| 文件/目录 | 是否自动加载 | 用途 |
|---------|-------------|------|
| `EASYCODE.md` | ✅ 是 | 主配置文件、全局规则 |
| `AGENTS.md` | ✅ 是 | AI 代理配置、编码规范 |
| `.easycode/rules/*.md` | ✅ 是 | 特定场景的规则文件 |
...
**重要说明**:
- ✅ **自动加载**: `EASYCODE.md`、`AGENTS.md` 和 `.easycode/rules/` 目录中的所有 `.md` 文件
- 💡 **用途建议**:
  - `EASYCODE.md` → 项目级全局规则
  - `AGENTS.md` → AI 代理特定配置
  - `.easycode/rules/*.md` → 特定场景规则
...
2. 输入 "ClawMaster: Manage Custom Rules"
3. 在打开的对话框中管理规则

### 方法 2: 直接编辑文件

1. 在项目根目录创建 `.easycode/rules/` 目录
...
4. 查看 ClawMaster 日志 (`Ctrl+Shift+P` -> "ClawMaster: Open Log File")
