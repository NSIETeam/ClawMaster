...
ClawMaster 后续更新分为四类，默认优先发增量包，只有触及 Electron、安装器、系统权限、驱动级 native 依赖时才发完整安装包。
...
- `skills/<name>` component bundle 安全解包：bundle 必须是 schemaVersion=1 的 JSON，相对路径 + base64 文件内容；安装后同步到 `~/.clawmaster-user/skills/<name>`，下一次 skills 刷新可发现。
