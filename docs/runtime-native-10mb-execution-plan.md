...
- 任何 ClawMaster 命名、图标、默认资源和兼容层可见字符串。旧环境变量只在迁移代码中保留映射，不进入 UI、安装包元数据或新协议。
...
- `micro-bootstrap` 实际安装器 `<10 MiB`，`micro-online` 实际安装器 `<=20 MiB`，均未携带 Node、agent capsule、SQLCipher、浏览器或 ClawMaster 资源。
...
- 只删除字符串搜索命中的 ClawMaster 文本，却保留旧 logo、资源文件名或安装器元数据。
