# ClawMaster 私有服务器联邦网关接入

ClawMaster Server 的 `federation_gateway` 模块负责把两个独立私有部署连接到 ClawMaster Control
...
- ClawMaster Server 只接受 HTTPS 网关地址，测试环境的 loopback HTTP 必须显式开启。
...
1. 启动启用了联邦模块的 ClawMaster Server。
...
3. 把新私钥放到客户服务器的 root/clawmaster 专用只读路径，权限限制为 `0600`，并更新
   `CLAWMASTER_FEDERATION_SIGNING_KEY_FILE` 后重启 ClawMaster Server。
...
两台 staging ClawMaster Server 和 staging Control Federation 网关准备好后，可以运行黑盒验收：
...
export CLAWMASTER_FEDERATION_SMOKE_SERVER_A_URL=https://clawmaster-a-staging.example.com
...
export CLAWMASTER_FEDERATION_SMOKE_SERVER_B_URL=https://clawmaster-b-staging.example.com
