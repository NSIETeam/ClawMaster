...
This document defines the **minimal runtime kernel** — the set of modules that form the irreducible core of ClawMaster's agent runtime. Everything outside this boundary is optional, replaceable, or UI-specific.
...
- Enumerates `ClawMasterEventType` (Content, ToolCallRequest, ToolCallResponse, ToolCallConfirmation, UserCancelled, Error, ChatCompressed, Thought, Reasoning, MemoryContext, MaxSessionTurns, Finished, LoopDetected, TokenUsage).
- Carries structured error types (`ClawMasterErrorEventValue`), tool call request/response shapes, and confirmation outcome enums.
...
- Checkpoint files live in `~/.clawmaster-user/` under `checkpoint-{tag}.json`.
...
- The audit logger is injected into `ToolExecutionEngine` and `ClawMasterChat`.
- Audit log storage is in `~/.clawmaster-user/audit/audit-*.jsonl`.
...
- **File**: `packages/core/src/core/clawmasterChat.ts`
...
- **File**: `packages/core/src/core/clawmasterRequest.ts`
- `ClawMasterCodeRequest` type alias for `PartListUnion` — the kernel's message shape.
...
- Wired into `ClawMasterClient.sendMessageStream()` — on first turn (`sessionTurnCount === 1`), the injector searches memory and prepends the summary to the user message context.
- Emits `ClawMasterEventType.MemoryContext` before the model call so UI surfaces can show "found N related memories" instead of hiding the behavior in logs.
...
|---|---|---|
| `client.ts` | Top-level agent orchestration | `ClawMasterClient` class |
| `turn.ts` | Single LLM round-trip state machine | `Turn`, `ClawMasterEventType`, `ServerTool` |
...
| `clawmasterChat.ts` | Chat session core (forked genai Chat) | `ClawMasterChat` |
| `clawmasterRequest.ts` | Message shape type | `ClawMasterCodeRequest` |
...
| `ClawMasterServerAdapter.ts` | Server API adapter | `ClawMasterServerAdapter` |
...
| `memory/sessionMemoryInjector.ts` | Session memory injection on session start | Called by `ClawMasterClient.sendMessageStream()` on first turn |
