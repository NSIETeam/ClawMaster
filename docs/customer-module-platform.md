...
- Background authorization always installs off. A publisher may declare it, but the user must enable it separately; execution still requires ClawMaster's registered background-task scheduler and cannot be started by raw module timers.
...
- Foreground model calls use a tool-free temporary ClawMaster model session with no MCP discovery or ambient environment context. Cancellation propagates to the provider request; unavailable pricing is displayed as unknown rather than zero.
