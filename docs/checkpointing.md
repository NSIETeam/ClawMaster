...
> ClawMaster-specific checkpointing. This document supersedes the inherited Gemini CLI
> command examples below; ClawMaster does not expose Gemini's `/restore` command or
> use `~/.gemini` storage.

## ClawMaster Runtime Checkpoints

ClawMaster persists two independent recovery records under `~/.clawmaster-user/checkpoints/`:
...
repository under `~/.clawmaster-user/history/<project-hash>`. It must never mutate
