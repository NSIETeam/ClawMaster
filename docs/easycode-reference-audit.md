...
| File rollback and checkpoint UX | Adopt the user-facing advantage, but not a shadow Git repository or repository-wide cleanup. Use bounded encrypted Rust per-file snapshots, post-write conflict detection, standard confirmation and audit, plus a visible Versions workspace. | `packages/desktop/src-tauri/src/native_encrypted_checkpoints.rs`, `packages/desktop/src/renderer/components/FileRecoveryWorkspace.tsx`, `docs/checkpointing.md` |
