...
ClawMaster supports custom model configurations, allowing you to use any OpenAI-compatible API or Claude API endpoint.
...
- Avoids race conditions when multiple ClawMaster instances are running
...
### ClawMaster Custom (`easycode`)

For custom ClawMaster-compatible endpoints (uses OpenAI format):
...
- No credits consumption displayed (custom models don't use ClawMaster credits)
...
3. Restart ClawMaster after configuration changes
...
3. Restart ClawMaster after setting environment variables
