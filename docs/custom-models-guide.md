...
Otto supports custom model configurations, allowing you to use any OpenAI-compatible API or Claude API endpoint.
...
- Avoids race conditions when multiple Otto instances are running
...
### Otto Custom (`easycode`)

For custom Otto-compatible endpoints (uses OpenAI format):
...
- No credits consumption displayed (custom models don't use Otto credits)
...
3. Restart Otto after configuration changes
...
3. Restart Otto after setting environment variables
