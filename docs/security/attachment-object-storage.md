...
ClawMaster's attachment-storage boundary is the asynchronous
...
- An E2EE client encrypts the file before upload. ClawMaster Server and the object
...
ClawMaster Server cannot scan an E2EE attachment's plaintext. If malware inspection
...
multipart uploads after a conservative interval, but must not delete live ClawMaster
...
The runtime rejects local attachment storage when more than one ClawMaster Server
...
CLAWMASTER_S3_BUCKET=clawmaster-private-attachments
