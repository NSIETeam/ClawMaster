...
ClawMaster requires SQLCipher for the enterprise database in every non-test runtime.
...
CLAWMASTER_DATABASE_ENCRYPTION_KEY_FILE=/mounted-custody/clawmaster-database.key
CLAWMASTER_SQLCIPHER_NATIVE_BINDING=/opt/clawmaster/sqlcipher/better_sqlite3.node
```

The initial offline file is exactly 32 cryptographically random bytes. It must
be provisioned outside the ClawMaster data directory, backed up separately, limited
...
ClawMaster will then reject automatic rotation.

On the first managed rotation, ClawMaster converts a writable raw file into an
`clawmaster-sqlcipher-keyring-v1` file. The keyring durably stages the next key before
...
`sqlcipher_export()` path writes a new encrypted file. ClawMaster validates
...
```text
clawmaster-database-encryption rotate --confirm-rotation
```
...
CycloneDX 1.5 SBOM that binds the target, ClawMaster source commit, SQLCipher source
