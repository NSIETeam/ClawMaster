...
ClawMaster enterprise private chat uses a fail-closed, client-side encryption path.
...
being protected by Electron `safeStorage`. ClawMaster refuses to enable private chat
...
## ClawMaster and A2A privacy boundary

The `enterprise_collaboration` tool has no `list_messages` action. ClawMaster cannot
...
in [E2EE production release gate](./e2ee-release-gate.md). A normal ClawMaster build
...
implementation, so ClawMaster must not claim Signal-grade security.

The release-gated MLS candidate pins OpenMLS 0.8.1 and its official Rust
crypto provider in `clawmaster-native`. It creates signed, one-time public MLS 1.0
...
`@clawmaster/native` as a workspace package and reserves its native executable for
ASAR unpacking.
