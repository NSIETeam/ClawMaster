...
ClawMaster separates candidate delivery from production E2EE enablement. Normal
ClawMaster releases run the candidate gate and may include the inactive MLS
...
4. Record the signed `clawmaster-e2ee-external-audit` attestation. It must bind the
...
5. Two different keys sign the `clawmaster-e2ee-production-approval` statement: one
...
The statement type is `clawmaster-e2ee-external-audit`. Required fields are:
...
The statement type is `clawmaster-e2ee-production-approval`. Required fields are:
