...
- the audited OpenMLS version and ClawMaster native MLS boundary;
- a user comparison of the safety number or QR code for new devices;
- external-auditor and ClawMaster release-approval public keys in the release trust
  store;
...
verify safety numbers or QR codes out of band. ClawMaster must not claim automatic
...
- separate ClawMaster security and release approvers sign the production decision;
...
This gate is release assurance, not a claim that ClawMaster authored its own
independent audit.
