...
它将代码分析逻辑（Brain）与编辑器（UI）分离。通过 LSP，Otto 可以调用各语言官方或社区最专业的分析引擎，为 AI 提供极致的代码感知力。
...
在 Otto 中，我们实现了 **自动二进制管理 (BinaryManager)**：

- **按需触发**：当你第一次询问某种语言的问题时，系统才会启动。
- **静默安装**：如果你的电脑没装对应的 LSP 服务端，Otto 会**自动在后台下载并安装**到隔离目录。
