# AtomCode Reuse Boundary for ClawMaster

> Status: adopted on 2026-07-31. This is a design boundary, not a migration
> plan. AtomCode is a reference implementation in another language; ClawMaster keeps
> ownership of its TypeScript runtime and public protocol.

## Decision

ClawMaster may reuse AtomCode's architectural principles, but it must not replace
ClawMaster's runtime, state machine, tool protocol, storage, or UI with AtomCode
code. Any future work starts from an ClawMaster-native seam and requires a focused
acceptance test.

| AtomCode principle | ClawMaster-native owner | Decision |
| --- | --- | --- |
...
| Checkpoint before unsafe progress and resume safely | `core/turnCheckpoint.ts`, `sessions/sessionCheckpointService.ts` | Reuse recovery semantics; preserve ClawMaster replay classes and session format. |
| Tool calls have typed lifecycle and results | `core/toolExecutionEngine.ts`, `core/turn.ts` | Reuse the contract boundary; keep `EngineToolCall` and ClawMaster events canonical. |
| File changes should be recoverable and ordered | `services/fileOperationQueue.ts`, `services/gitService.ts` | Reuse the safety objective; keep ClawMaster's per-file queue and shadow Git history. |
| Results should not become unbounded prompt state | `core/clawmasterChat.ts`, compression services, tool response summaries | Reuse bounded-result thinking; do not introduce a generic external artifact store without a concrete consumer. |
...
### Must be rewritten in ClawMaster terms
...
| Capability | Current ClawMaster status | Decision |
| --- | --- | --- |
...
2. Extend an existing ClawMaster owner when it already owns the state.
...
5. For destructive tools, require ClawMaster confirmation, policy, and audit paths;
