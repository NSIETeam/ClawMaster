...
| 日常办公 | 企业工作 Agent | Agent chip | Existing `clawmaster-enterprise-work` profile |
...
Personal edition does not reuse the enterprise template. Its initial template is built from personal `BASE_AGENT_PROFILES` plus currently available personal capabilities (for example ClawMaster and Auto Skill when enabled). It must never render a disabled enterprise shell merely to preserve the two-group visual. If only one useful personal group exists, render one group and the “添加功能组” action.
...
npm test --workspace=packages/desktop -- \
  ...
  src/renderer/state/useClawMasterStore.test.ts \
...
Use the storage key helper from Task 1. Keep this hook independent of `useClawMasterStore`; it owns UI preference state only.
...
```ts
window.clawmaster.enterpriseKnowledgeList
window.clawmaster.enterpriseWorkLogRecent
window.clawmaster.enterpriseKnowledgeRecord
window.clawmaster.enterpriseKnowledgeReview
window.clawmaster.enterpriseKnowledgeRevise
window.clawmaster.enterpriseKnowledgeRevisions
```
...
- Modify: `packages/desktop/src/renderer/state/useClawMasterStore.ts`
- Modify: `packages/desktop/src/renderer/state/useClawMasterStore.test.ts`
...
```bash
npm test --workspace=packages/desktop -- src/renderer/components/Composer.test.tsx src/renderer/components/ChatView.test.tsx src/renderer/state/useClawMasterStore.test.ts
```
...
```bash
git add packages/desktop/src/renderer/App.tsx packages/desktop/src/renderer/components/ChatView.tsx packages/desktop/src/renderer/components/ChatView.test.tsx packages/desktop/src/renderer/components/Composer.tsx packages/desktop/src/renderer/components/Composer.test.tsx packages/desktop/src/renderer/state/useClawMasterStore.ts packages/desktop/src/renderer/state/useClawMasterStore.test.ts
